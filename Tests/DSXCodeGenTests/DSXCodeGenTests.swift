...
internal struct DSXCodeGenTests {
  ...
  internal func groups() throws {
    ...
    let generated = generate(profile).source
  ...
  internal func text() throws {
    ...
    let multiline = kValidProfile
      .replacingOccurrences(of: "title: General",
                            with: "title: \"General\\nRegisters\"")
    let lines = try decode(multiline)
    try validate(lines)
    let escaped = generate(lines).source
  ...
  internal func positions() throws {
    ...
    let empty = try generate(decode(kValidProfile)).source
    ...
    let framed = try generate(decode(frame)).source
  ...
  internal func capacity() throws {
    ...
    let registers = (0 ..< 5).map { index in
      parts[1]
        .replacingOccurrences(of: "id: 0", with: "id: \(index)")
    ...
    let profile =
        try decode(parts[0] + separator + registers.joined(separator: "\n"))
    try validate(profile)
    let generated = generate(profile).source
  ...
  internal func spelling() throws {
    ...
    #expect(try generate(decode(custom)).source.contains("storage: \"wire\""))
    let empty = source.replacingOccurrences(of: "gdb: alias", with: "gdb: ''")
    #expect(throws: DSXCodeGenError.self) { try validate(decode(empty)) }
  }
  ...
  internal func leaf(_ name: String) {
    ...
    #expect(throws: DSXCodeGenError.self) { try packets(definition) }
  }
    ...
    for enabled in [false, true] {
      let definition = source
        .replacingOccurrences(of: "role: \(role)",
                              with: "role: \(role)\n    expedited: \(enabled)")
      let profile = try decode(definition)
      try validate(profile)
      ...
      #expect(generate(profile).source.contains(word))
    }
  }
