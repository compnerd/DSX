...
internal struct ConnectionTests {
  ...
  internal func descriptor() throws {
    ...
    guard case let .descriptor(descriptor) = connection else {
  ...
  internal func device() throws {
    ...
    guard case let .device(path) = connection else {
    ...
    guard case let .device(path) = serial else {
  ...
  internal func pipe() throws {
    ...
    guard case let .pipe(path) = connection else {
  ...
  internal func unix() throws {
    ...
    guard case let .unix(path, reverse) = listening else {
    ...
    guard case let .unix(path, reverse) = connecting else {
      Issue.record("expected a Unix socket connection")
      return
    }
    #expect(path == "/tmp/dsx.sock")
  ...
  private func check(_ connection: Connection, host: String?, port: UInt16,
                     reverse: Bool) {
    guard case let .network(address, service, direction) = connection else {
