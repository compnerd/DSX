...
internal struct RegisterTests {
  ...
  internal func configuration() throws {
    ...
    #expect(throws: Debuggee.Error.self) {
      try RegisterDescription { () throws(Debuggee.Error) in
        resolutions += 1
        throw .thread
      }
    }
    ...
    let description = RegisterDescription(configuration)
    #expect(description.configuration.vector == 64)
    #expect(description.configuration.authentication)
#else
    ...
    #expect(description.count == RegisterDescription().count)
#endif
  }
