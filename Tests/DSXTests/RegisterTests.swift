...
private enum RegisterInfoPacket: GDBPacketHandler {
  ...
  internal static func handle(_ payload: borrowing Span<UInt8>,
    ...
    try RegisterDescription().info(payload, state: state, writer: &writer)
    return .reply
  }
}
...
private enum RegisterFeaturesPacket: GDBPacketHandler {
  ...
  internal static func handle(_ payload: borrowing Span<UInt8>,
      ...
      throws(GDBHandlerError) -> GDBPacketDisposition {
    let request =
        try GDBTransferRequest(.features, payload: payload.extracting(0...),
                               state: state)
    var reader = GDBPacketReader(request.annex)
    guard reader.consume("target.xml"), reader.empty else {
      throw .unsupported
    }
    try writer.emit(RegisterDescription(), offset: request.offset,
                    length: request.length, compatibility: state.compatibility)
    return .reply
  }
}
...
internal struct RegisterTests {
  ...
  internal func exceptions(_ vector: Bool) throws {
    var profile = RegisterDescription()
    profile.configuration = RegisterConfiguration(vector: vector)
    let first = vector ? 73 : 57
    let offset = vector ? 792 : 536
    let names = ["trapno", "err", "faultvaddr"]
    let offsets = [offset, offset + 2, offset + 6]
    for index in names.indices {
      let record = try #require(profile.register(first + index))
      #expect(string(profile.name(record)) == names[index])
      #expect(record.identifier.rawValue == 75 + index)
      #expect(record.offset == offsets[index])
      for compatibility in [CompatibilityMode.gdb, .lldb] {
        let selected =
            try #require(profile.register(first + index,
                                          compatibility: compatibility))
        #expect(selected.identifier == record.identifier)
      }
    }
    #expect(profile.count == first + names.count)
  }
#endif
  ...
  internal func positions() throws {
    ...
    let configurations = [0, 16, 64, 256].flatMap { vector in
      [false, true].map { authentication in
        RegisterConfiguration(vector: vector, authentication: authentication)
      }
    }
#else
    ...
    for configuration in configurations {
      ...
      var pc: RegisterIdentifier?
      var fp: RegisterIdentifier?
      var expected = Array<RegisterIdentifier>()
      for index in 0 ..< description.count {
        guard let record = description.register(index) else {
          continue
        }
        if case .program = record.role {
          pc = record.identifier
        }
        if case .frame = record.role {
          fp = record.identifier
        }
        if record.expedited {
          expected.append(record.identifier)
        }
      }
      #expect(description.pc?.identifier == pc)
      #expect(description.fp?.identifier == fp)
      var actual = Array<RegisterIdentifier>()
      for slot in 0 ..< RegisterDescription.expedited.count {
        let index = RegisterDescription.expedited[slot]
        guard let record = description.record(Int(index)) else {
          continue
        }
        let visible = try #require(description.register(record.identifier))
        #expect(record.index == visible.index)
        #expect(record.bits == visible.bits)
        #expect(record.offset == visible.offset)
        actual.append(record.identifier)
      }
  ...
  @Test
  internal func configuration() throws {
    var resolutions = 0
    let description = try? RegisterDescription { () throws(Debuggee.Error) in
      resolutions += 1
      throw .thread
    }
...
#endif
    #expect((description == nil) == (resolutions == 1))
#if (os(Android) || os(Linux)) && arch(arm64)
    let configuration = RegisterConfiguration(vector: 64, authentication: true)
    let scalable = RegisterDescription(configuration)
    #expect(scalable.configuration.vector == 64)
    #expect(scalable.configuration.authentication)
#endif
  }

  @Test(arguments: [false, true])
  internal func expedited(_ enabled: Bool) {
    let flag: UInt64 = enabled ? 0x0001_0000_0000_0000 : 0
    let storage = RegisterStorage(0, flag | 64, 0, relations: 0)
    let record = RegisterRecord(storage: storage, index: 0)
    #expect(record.expedited == enabled)
    #expect(record.bits == 64)
    let layout = storage.layout(bits: 128, offset: 512, bias: 0)
    #expect(layout.expedited == enabled)
    #expect(layout.bits == 128)
    #expect(layout.offset == 512)
  }

  @Test
  internal func bytes() {
    let cases = [(0, 0), (1, 1), (7, 1), (8, 1), (9, 2), (15, 2), (16, 2),
                 (17, 3), (65528, 8191), (65529, 8192), (65535, 8192),
    ]
    for (bits, bytes) in cases {
      let storage = RegisterStorage(0, UInt64(bits), 0, relations: 0)
      let record = RegisterRecord(storage: storage, index: 0)
      #expect(record.bits == bits)
  ...
  internal func profile() throws {
    ...
    #expect(profile.sets == 5)
    #expect(profile.features == 5)
...
#endif
#if os(Android) || os(Linux)
    #expect(profile.types == 7)
#else
    #expect(profile.types == 5)
#endif
    #expect(profile.fields == 36)
    let first = try #require(profile.register(0))
    #expect(string(profile.name(first)) == "x0")
#elseif arch(i386)
    #expect(profile.count == 41 + (profile.configuration.vector ? 8 : 0))
    #expect(profile.sets == 3)
...
#elseif arch(x86_64)
#if os(Android) || os(Linux)
    #expect(profile.count == 59)
    #expect(profile.features == 4)
    let base = try #require(profile.register(57))
    #expect(string(profile.name(base)) == "fs_base")
    guard case .thread = base.role else {
      Issue.record("FS base must identify the thread pointer")
      return
    }
    #expect(base.bits == 64)
    #expect(base.offset == 536)
    #expect(base.numbers.dwarf == 58)
#elseif os(anyAppleOS)
    #expect(profile.count == 60 + (profile.configuration.vector ? 16 : 0))
    #expect(profile.features == 4)
#else
    #expect(profile.count == 57 + (profile.configuration.vector ? 16 : 0))
    #expect(profile.features == 3)
#endif
#if os(anyAppleOS)
    #expect(profile.sets == 4)
#else
    #expect(profile.sets == 3)
#endif
    #expect(profile.types == 3)
    ...
    let index = 24
#endif
    let stack = try #require(profile.register(index))
    #expect(string(profile.name(stack)) == "st0")
    #expect(stack.bits == 80)
    #expect(stack.encoding == .vector)
    #expect(stack.format == .vector)
#endif
#if arch(arm64)
...
#if arch(i386)
  @Test
  internal func numbering() throws {
    let profile = RegisterDescription()
    let names = [
      "eax", "ebx", "ecx", "edx", "edi", "esi", "ebp", "esp",
      "eip", "eflags", "cs", "fs", "gs", "ss", "ds", "es",
    ]
    let numbers = [0, 3, 1, 2, 7, 6, 5, 4, 8, 9, 10, 14, 15, 11, 12, 13]
    for index in names.indices {
      let lldb = try #require(profile.register(index, compatibility: .lldb))
      let gdb =
          try #require(profile.register(numbers[index], compatibility: .gdb))
      #expect(string(profile.name(lldb)) == names[index])
      #expect(lldb.identifier == gdb.identifier)
      #expect(lldb.offset == numbers[index] * 4)
      #expect(profile.number(lldb, compatibility: .lldb) == index)
      #expect(profile.number(gdb, compatibility: .gdb) == numbers[index])
    }
  }
#endif
  ...
  internal func layout() throws(Debuggee.Error) {
    ...
    for index in 0 ..< expected.count {
      ...
      let layout = try WindowsRegisterState.layout(identifier)
    ...
    #expect(throws: Debuggee.Error.register) {
      try WindowsRegisterState.layout(invalid)
    }
  }
#endif
  ...
  internal func scalarbytes() throws {
    ...
    let value = try bytes.span.register(as: UInt32.self)
  ...
  internal func offsetbytes() throws {
    ...
    try bytes.span.write(offset: 8, to: &storage)

    let output =
        try withUnsafeTemporaryAllocation(of: UInt8.self,
                                          capacity: 4) { buffer in
      var output = OutputSpan(buffer: buffer, initializedCount: 0)
      try output.append(storage, offset: 8, size: 4)
      return Array(buffer[0 ..< output.count])
    }
    #expect(output == bytes)
  }
  ...
  internal func extendedbytes() throws {
    ...
    try bytes.span.narrow(offset: 2, native: 2, size: 4, to: &storage)

    let output =
        try withUnsafeTemporaryAllocation(of: UInt8.self,
                                          capacity: 4) { buffer in
      var output = OutputSpan(buffer: buffer, initializedCount: 0)
      try output.extend(storage, offset: 2, native: 2, size: 4)
      return Array(buffer[0 ..< output.count])
    }
  ...
  @Test
  internal func gdb() throws {
    var session = TestRegisterSession()
    let capacity = String(Tuning.Packet.capacity, radix: 16)
    let packet = "qXfer:features:read:target.xml:0,\(capacity)"
    let response =
        try response(RegisterFeaturesPacket.self, packet: packet,
                     session: &session, compatibility: .gdb)
    let xml = String(decoding: response.dropFirst(), as: UTF8.self)
    #expect(response.first == UInt8(ascii: "l"))
    #expect(xml.contains("group=\"general\""))
    #expect(xml.contains("group=\"General Purpose Registers\"") == false)
    #expect(xml.contains("type=\"code_ptr\""))
#if arch(i386) || arch(x86_64)
    #expect(xml.contains("org.gnu.gdb.i386.387") == false)
    #expect(xml.contains("org.gnu.gdb.i386.sse"))
    #expect(xml.contains("type=\"i387_ext\""))
    #expect(xml.contains("type=\"data_ptr\""))
    let profile = RegisterDescription()
    let core = try #require(profile.feature(0))
    let floating = [
      "st0", "st1", "st2", "st3", "st4", "st5", "st6", "st7",
      "fctrl", "fstat", "ftag", "fiseg", "fioff", "foseg", "fooff", "fop",
    ]
    for index in 0 ..< profile.count {
      let record = try #require(profile.register(index))
      let name = string(profile.name(record))
      if floating.contains(name) {
        #expect(record.feature == core.identifier)
      }
    }
#endif
  }
