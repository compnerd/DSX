...
internal struct DarwinX86TrapTests {
  ...
  internal func access() {
    for index: UInt32 in 0 ..< 78 {
      let register = RegisterIdentifier(rawValue: index)
      let access: RegisterAccess = switch index {
      case 19 ... 21: .unavailable
      case 75 ... 77: .immutable
      default: .mutable
      }
      #expect(DarwinX86RegisterState.access(register) == access)
    }
  }
...
internal struct DarwinX86WatchpointTests {
  ...
  @Test(arguments: 0 ..< 4)
  internal func pending(_ slot: Int) throws(Debuggee.Error) {
    let thread =
        ProcessThreadIdentifier(process: ProcessIdentifier(rawValue: 1),
                                thread: ThreadIdentifier(rawValue: 2))
    var sites = ActiveBreakpoints()
    for index in 0 ..< 4 {
      let address = Debuggee.Address(rawValue: 0x1000 + UInt64(index) * 4)
      let site =
          BreakpointSite(address: address, size: 4, kind: .watchpoint(.write))
      sites.update(site, thread: nil, enabled: true)
    }
    var state = try x86_debug_state64_t(sites, thread: thread)
    state.__dr6 = 1 << slot
    let empty = ActiveBreakpoints()
    #expect(try state.configure(empty, thread: thread) == false)
    #expect(state.__dr6 == 1 << slot)
    let stop = Debuggee.Stop(thread: thread, reason: .trace)
    let address = Debuggee.Address(rawValue: 0x1000 + UInt64(slot) * 4)
    let trap = try state.trap(stop, breakpoints: sites)
    #expect(trap.reason == .watchpoint(.write, address))

    // Once consumed, a fresh configuration retires both the hit and removed
    // comparators. A single-step status alone must not block configuration.
    state = try x86_debug_state64_t(empty, thread: thread)
    #expect(state.__dr6 == 0)
    #expect(state.__dr7 == 0)
    state.__dr6 = 1 << 14
    #expect(try state.configure(sites, thread: thread))
    #expect(state.__dr6 == 0)
    #expect(state.__dr7 > 0)
  }
}
