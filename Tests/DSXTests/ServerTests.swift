...
internal struct ServerTests {
  ...
  internal func notification() throws {
    ...
    try PortNotification.descriptor(descriptors[1]).write(1234)
  ...
  internal func logging() throws {
    let configuration =
        try LogConfiguration("gdb-remote packets:lldb process:warning")
    ...
    let stepping = try LogConfiguration("lldb step:gdb-remote packets")
    #expect(stepping.level == .trace)
    #expect(stepping.channels == channels)
    let loader = try LogConfiguration("gdb-remote packets:lldb dyld")
    #expect(loader.level == .trace)
    #expect(loader.channels == channels)
    #expect(throws: LogConfigurationError.self) {
      try LogConfiguration("unknown")
    }
  }
}
...
private func frame(_ message: borrowing Span<UInt8>) -> Array<UInt8> {
  ...
  frame.append(addingCapacity: capacity) { output in
    GDBPacketEncoding.text.frame(message, output: &output)
  }
  return frame
}
