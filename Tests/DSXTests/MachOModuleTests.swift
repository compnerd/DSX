...
internal struct MachOModuleTests {
  @Test
  internal func traversal() throws {
    var bytes = image(0x0100_000c, subtype: 0, identifier: 0x10)
    bytes.append(contentsOf: repeatElement(UInt8(0), count: 24))
    store(2, at: 16, into: &bytes)
    store(48, at: 20, into: &bytes)
    store(0x32, at: 56, into: &bytes)
    store(24, at: 60, into: &bytes)
    store(6, at: 64, into: &bytes)
    do {
      let module = try MachOModule(bytes.span)
      let platform = try module?.platform
      let system = try module?.system
      let identity = try module?.identity
      #expect(platform == 6)
      #expect(system == "maccatalyst")
      guard case let .unique(identifier) = identity else {
        Issue.record("UUID command was not found")
        return
      }
      #expect(identifier == "10111213-1415-1617-1819-1A1B1C1D1E1F")
    }
    store(7, at: 60, into: &bytes)
    #expect(throws: Debuggee.Error.process) {
      _ = try MachOModule(bytes.span)?.platform
    }
  }

  @Test
  internal func systems() {
    let names = ["macosx", "ios", "tvos", "watchos", "bridgeos", "maccatalyst",
                 "iossimulator", "tvossimulator", "watchossimulator",
                 "driverkit",]
    for index in names.indices {
      #expect(String(platform: UInt32(index + 1)) == names[index])
    }
    #expect(String(platform: 0) == nil)
    #expect(String(platform: UInt32.max) == nil)
  }

  @Test(arguments: [UInt32(1), 2, 6, 7, 11])
  internal func platform(_ platform: UInt32) throws {
    var bytes = image(0x0100_000c, subtype: 0, identifier: 0)
    store(0x32, at: 32, into: &bytes)
    store(platform, at: 40, into: &bytes)
    let actual = try MachOModule(bytes.span)?.platform
    #expect(actual == platform)
    store(8, at: 36, into: &bytes)
    #expect(throws: Debuggee.Error.process) {
      _ = try MachOModule(bytes.span)?.platform
    }
  }

  @Test(arguments: [UInt32(0x24), 0x25, 0x2f, 0x30])
  internal func legacy(_ command: UInt32) throws {
    var bytes = image(0x0100_000c, subtype: 0, identifier: 0)
    store(command, at: 32, into: &bytes)
    let expected: UInt32 = switch command {
    case 0x24: 1
    case 0x25: 2
    case 0x2f: 3
    default: 4
  ...
  internal func unspecified() throws {
    let bytes = image(0x0100_000c, subtype: 0, identifier: 0)
    let platform = try MachOModule(bytes.span)?.platform
    #expect(platform == nil)
  }
  ...
  internal func thin() throws {
    ...
    let identity = try module?.identity
    guard case let .unique(value)? = identity else {
  ...
  internal func universal() throws {
    ...
    let fallback = try MachOModule(bytes.span)
    #expect(fallback?.offset == 0x80)
    #expect(throws: Debuggee.Error.unsupported) {
      _ = try MachOModule(bytes.span, requested: "x86_64")
    }
  ...
  internal func digest(_ little: Bool) throws {
    ...
    guard case let .digest(expected)? = identity else {
    ...
    guard case let .digest(actual)? = try module?.identity else {
