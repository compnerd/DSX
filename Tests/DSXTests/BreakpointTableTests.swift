...
internal struct BreakpointTableTests {
  ...
  internal func ownership() throws {
    ...
    let native = try table.insert(fixture.process, site, native: true,
                                   context: &control)
    let client = try table.insert(fixture.process, site, context: &control)
    ...
    let stop = Debuggee.Stop(thread: fixture.thread, reason: .breakpoint,
                             fault: fault)
    #expect(try table.hit(stop, context: control) == client)
    try table.remove(fixture.process, client, context: &control)
    #expect(table.find(fixture.process, site) == nil)
    #expect(fixture.bytes != original)
    #expect(try table.hit(stop, context: control) == native)
    try table.remove(fixture.process, native, context: &control)
    #expect(fixture.bytes == original)
  }
  ...
  internal func maintenance() throws {
    ...
    for stepping in [false, true] {
      ...
      let identifier =
          try session.breakpoints.insert(fixture.process, site, native: true)
      let stop = Debuggee.Stop(thread: fixture.thread, reason: .breakpoint,
      ...
      guard case .stopped(let result) = try session.next(global: true) else {
  ...
  internal func packets() throws {
    ...
    try withUnsafeTemporaryAllocation(of: UInt8.self, capacity: 16) { buffer in
      ...
      for _ in 0 ..< 2 {
        try session.insert(payload.utf8Span.span, state: state,
                           writer: &writer)
      }
      #expect(session.breakpoints.find(fixture.process, site) != nil)
      #expect(fixture.bytes != original)
      for _ in 0 ..< 2 {
        try session.remove(payload.utf8Span.span, state: state,
                           writer: &writer)
        #expect(session.breakpoints.find(fixture.process, site) == nil)
        #expect(fixture.bytes == original)
      }
  ...
  internal func detach() throws {
    ...
    let site = fixture.site()
    _ = try session.breakpoints.insert(fixture.process, site,
  ...
  internal func relinquishment(shared: Bool, active: Bool) throws {
    ...
    let identifier = try table.insert(fixture.process, native, native: true)
    if active {
      try table.enable(identifier, context: &control)
    }
    _ = try table.insert(fixture.process, client, context: &control)
    let armed = fixture.bytes
    try table.clear(fixture.process, preserving: true, context: &control)
