...
internal struct PEModuleTests {
  @Test
  internal func coff() throws {
    var bytes = Array<UInt8>(repeating: 0, count: 20)
    bytes[0] = 0x64
    bytes[1] = 0x86
    #expect(try String(coff: bytes.span) == "x86_64")
    for size in 0 ..< 20 {
      #expect(throws: Debuggee.Error.self) {
        _ = try String(coff: bytes.span.extracting(..<size))
      }
    }
    bytes[16] = 1
    #expect(throws: Debuggee.Error.self) {
      _ = try String(coff: bytes.span)
    }
    bytes[16] = 0
    bytes[2] = 1
    #expect(throws: Debuggee.Error.self) {
    ...
    bytes.append(contentsOf: repeatElement(0, count: 40))
    #expect(try String(coff: bytes.span) == "x86_64")
    bytes[12] = 1
    #expect(throws: Debuggee.Error.self) {
      _ = try String(coff: bytes.span)
    ...
    bytes[8] = 60
    bytes.append(contentsOf: repeatElement(0, count: 18))
    #expect(try String(coff: bytes.span) == "x86_64")
  }
  ...
  internal func debuglink(_ wide: Bool) throws {
    ...
    bytes.append(contentsOf: repeatElement(0, count: 256))
    store(1, at: 0x46, count: 2, into: &bytes)
    store(476, at: 0x4c, count: 4, into: &bytes)
    store(2, at: 0x50, count: 4, into: &bytes)
    let section = 0x58 + (wide ? 240 : 224)
    bytes[section] = UInt8(ascii: "/")
    bytes[section + 1] = UInt8(ascii: "4")
    store(128, at: section + 16, count: 4, into: &bytes)
    store(384, at: section + 20, count: 4, into: &bytes)
    let name = Array(".gnu_debuglink\0".utf8)
    store(UInt64(name.count + 4), at: 512, count: 4, into: &bytes)
    bytes.replaceSubrange(516 ..< (516 + name.count), with: name)
    bytes.replaceSubrange(384 ..< 390, with: "a.dbg\0".utf8)
    store(0x1234_5678, at: 392, count: 4, into: &bytes)
    #expect(try PEModule(bytes.span)?.identifier == "78563412")

    // The CRC follows the aligned filename, not raw section padding.
    store(0xffff_ffff, at: 508, count: 4, into: &bytes)
    #expect(try PEModule(bytes.span)?.identifier == "78563412")
    store(7, at: section + 16, count: 4, into: &bytes)
    #expect(throws: Debuggee.Error.process) {
      _ = try PEModule(bytes.span)?.identifier
    }
    store(128, at: section + 16, count: 4, into: &bytes)
    bytes[516] = UInt8(ascii: "x")
  ...
  internal func machine(_ fixture: (UInt64, String)) throws {
    #expect(try String(pe: fixture.0) == fixture.1)
  }
  ...
  internal func codeview(_ wide: Bool, _ age: UInt64) throws {
    ...
    // CodeView takes precedence without consulting the COFF string table.
    store(UInt64(UInt32.max), at: 0x4c, count: 4, into: &bytes)
    #expect(try PEModule(bytes.span)?.identifier == expected)
    store(0, at: 0x4c, count: 4, into: &bytes)
