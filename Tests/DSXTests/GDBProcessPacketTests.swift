...
private struct PacketSession: Sendable {
  ...
  fileprivate var debuggee: Debuggee
  private let storage: PacketStorage
  ...
  internal static func handle(_ payload: borrowing Span<UInt8>,
    ...
    let action =
        Debuggee.Continuation(selection: state.selection.resume,
                              ...
                              address: address)
    do {
      if case let .thread(thread) = state.selection.resume {
        let fallback =
...
private enum InterruptPacket: PacketHandler {
  internal static func handle(_ payload: borrowing Span<UInt8>,
    ...
    let process = try selection.process(in: session.debuggee)
    do {
...
private enum ThreadOptionPacket: GDBPacketHandler {
  ...
  internal static func handle(_ payload: borrowing Span<UInt8>,
                              ...
                              writer: inout GDBPacketWriter)
      throws(GDBHandlerError) -> GDBPacketDisposition {
    try state.options(payload, session: session, writer: &writer)
    return .reply
  }
}
...
private enum SelectThreadPacket: PacketHandler {
  internal static func handle(_ payload: borrowing Span<UInt8>,
    ...
    let identifier =
        try Debuggee.Thread.Selection(payload.extracting(1...),
...
private enum InsertBreakpointPacket: PacketHandler {
  internal static func handle(_ payload: borrowing Span<UInt8>,
    ...
    let process = try state.selection.general.process(in: session.debuggee)
    do {
...
private enum SaveRegisterStatePacket: PacketHandler {
  internal static func handle(_ payload: borrowing Span<UInt8>,
    ...
    let thread = try state.selection.thread(nil, in: session.debuggee)
...
private enum WorkingDirectoryPacket: GDBPacketHandler {
  ...
  internal static func handle(_ payload: borrowing Span<UInt8>,
                              ...
                              writer: inout GDBPacketWriter)
      throws(GDBHandlerError) -> GDBPacketDisposition {
    try session.launch.directory(payload, writer: &writer)
    return .reply
  }
}
...
private enum EnvironmentPacket: GDBPacketHandler {
  ...
  internal static func handle(_ payload: borrowing Span<UInt8>,
                              ...
                              writer: inout GDBPacketWriter)
      throws(GDBHandlerError) -> GDBPacketDisposition {
    try session.launch.environment(payload, writer: &writer)
    return .reply
  }
}
...
private enum EnvironmentResetPacket: GDBPacketHandler {
  ...
  internal static func handle(_ payload: borrowing Span<UInt8>,
                              ...
                              writer: inout GDBPacketWriter)
      throws(GDBHandlerError) -> GDBPacketDisposition {
    try session.launch.environment(reset: payload, writer: &writer)
    return .reply
  }
}
...
private enum EnvironmentUnsetPacket: GDBPacketHandler {
  ...
  internal static func handle(_ payload: borrowing Span<UInt8>,
                              ...
                              writer: inout GDBPacketWriter)
      throws(GDBHandlerError) -> GDBPacketDisposition {
    try session.launch.environment(unset: payload, writer: &writer)
    return .reply
  }
}
...
private enum ArgumentsPacket: GDBPacketHandler {
  ...
  internal static func handle(_ payload: borrowing Span<UInt8>,
                              ...
                              writer: inout GDBPacketWriter)
      throws(GDBHandlerError) -> GDBPacketDisposition {
    try session.launch.arguments(payload, writer: &writer)
    return .reply
  }
}
...
private enum RunPacket: GDBPacketHandler {
  ...
  internal static func handle(_ payload: borrowing Span<UInt8>,
                              ...
                              writer: inout GDBPacketWriter)
      throws(GDBHandlerError) -> GDBPacketDisposition {
    try session.launch.run(payload)
    try writer.append("OK")
    return .reply
  }
}
...
internal struct GDBProcessPacketTests {
  ...
  internal func signal() throws {
    #expect(CompatibilityMode.gdb.signal(.interrupt) == 0x02)
#if os(Windows)
    #expect(CompatibilityMode.lldb.signal(.interrupt) == 2)
#elseif os(Android) || os(Linux)
    #expect(CompatibilityMode.lldb.signal(.interrupt) == 19)
#else
    #expect(CompatibilityMode.lldb.signal(.interrupt) == 17)
#endif
    #expect(CompatibilityMode.gdb.signal(.exception(0x91)) == 0x91)
    #expect(CompatibilityMode.gdb.signal(.exception(0x90)) == 0x05)
#if os(Android) || os(Linux)
    #expect(CompatibilityMode.gdb.signal(10) == 30)
    #expect(CompatibilityMode.gdb.native(30) == 10)
    #expect(CompatibilityMode.gdb.signal(12) == 31)
    #expect(CompatibilityMode.gdb.native(31) == 12)
#else
    #expect(CompatibilityMode.gdb.signal(29) == 142)
    #expect(CompatibilityMode.gdb.native(142) == 29)
#endif
    #expect(CompatibilityMode.lldb.signal(10) == 10)
    #expect(CompatibilityMode.lldb.native(10) == 10)
  ...
  internal func metadata(_ mask: Int) throws {
    let storage = PacketStorage()
    var session = DebugSession(debuggee: PacketSession(storage).debuggee)
    let identifier =
        ProcessThreadIdentifier(process: ProcessIdentifier(rawValue: 1),
                                thread: ThreadIdentifier(rawValue: 2))
    let data: Debuggee.ExceptionData? = if mask & 1 == 1 {
      Debuggee.ExceptionData(count: 2) { UInt64($0 + 1) }
    } else {
      nil
    }
    let code: UInt64? = mask & 2 == 2 ? 6 : nil
    let fault = Debuggee.Fault(address: Debuggee.Address(rawValue: 0),
                               code: code, data: data, domain: .mach)
    let stop =
        Debuggee.Stop(thread: identifier, reason: .exception(17), fault: fault)
    session.debuggee.observe(.stopped(stop))
    var state = GDBRemoteSessionState(compatibility: .lldb)
    let reply =
        try response(GDBThreadsInfoPacket.self, packet: "jThreadsInfo",
                     session: &session, state: &state)
    let type = mask == 3 ? 6 : 17
    let values = data == nil ? "" : "1,2"
    let expected = "[{\"tid\":2,\"reason\":\"exception\",\"metype\":\(type)," +
        "\"medata\":[\(values)]}]"
    #expect(reply == Array(expected.utf8))
    let stopped =
        try response(GDBStopPacket.self, packet: "?", session: &session,
                     state: &state)
    let fields = data == nil ? "" : "medata:1;medata:2;"
    let count = data?.count ?? 0
    let hexadecimal = String(type, radix: 16)
    let text = "T05thread:2;metype:\(hexadecimal);mecount:\(count);\(fields)"
    #expect(stopped == Array(text.utf8))
  }

  @Test(arguments: [Debuggee.StopReason.breakpoint, .trace])
  internal func classified(_ reason: Debuggee.StopReason) throws {
    let storage = PacketStorage()
    var session = DebugSession(debuggee: PacketSession(storage).debuggee)
    let identifier =
        ProcessThreadIdentifier(process: ProcessIdentifier(rawValue: 1),
    ...
    let fault = Debuggee.Fault(address: Debuggee.Address(rawValue: 0),
                               code: 6, data: data, domain: .mach)
    let stop = Debuggee.Stop(thread: identifier, reason: reason, fault: fault)
    session.debuggee.observe(.stopped(stop))
    var state = GDBRemoteSessionState(compatibility: .lldb)
    let name = reason == .trace ? "trace" : "breakpoint"
    let reply =
        try response(GDBStopPacket.self, packet: "?", session: &session,
                     state: &state)
    #expect(reply == Array("T05thread:2;reason:\(name);".utf8))
    let threads =
        try response(GDBThreadsInfoPacket.self, packet: "jThreadsInfo",
                     session: &session, state: &state)
    #expect(threads == Array("[{\"tid\":2,\"reason\":\"\(name)\"}]".utf8))
  }
  ...
  internal func fault() throws {
    ...
    let expected = "T0bthread:2;reason:signal;description:" +
        ...
        "61646265656629;"
#elseif os(Windows)
    let expected = "T0bthread:2;reason:signal;"
#else
    let text = "signal SIGSEGV (fault address=0xdeadbeef)"
    let encoded = text.utf8.map { String($0, radix: 16) }.joined()
    let expected = "T0bthread:2;reason:signal;description:\(encoded);"
#endif
    #expect(reply == Array(expected.utf8))
    let threads =
        try response(GDBThreadsInfoPacket.self, packet: "jThreadsInfo",
...
#else
    let listing = "[{\"tid\":2,\"reason\":\"signal\",\"signal\":11," +
        "\"description\":\"signal SIGSEGV (fault address=0xdeadbeef)\"}]"
#endif
    #expect(threads == Array(listing.utf8))
  ...
  internal func launch() throws {
    ...
    #expect(session.launch.directory == "/tmp")
  ...
  @Test(arguments: [(Debuggee.StopReason.breakpoint, "breakpoint"),
                    (.create, "create"), (.executed, "exec"),
                    (.exception(17), "exception"), (.fork, "fork"),
                    (.interrupt, "signal"), (.signal(2), "signal"),
                    (.library, "shared-library-event"), (.spawn, "fork"),
                    (.syscall(1, true), "trace"), (.trace, "trace"),
                    (.vfork, "vfork"), (.vforkdone, "vforkdone"),
  ])
  internal func reasons(_ reason: Debuggee.StopReason, _ name: String) throws {
    let identifier =
        ProcessThreadIdentifier(process: ProcessIdentifier(rawValue: 1),
                                thread: ThreadIdentifier(rawValue: 2))
    let stop = Debuggee.Stop(thread: identifier, reason: reason)
    ...
    try withUnsafeTemporaryAllocation(of: UInt8.self, capacity: 256) { buffer in
      ...
      try session.emit(stop, state: state, writer: &writer, registers: false)
      let output = writer.finish()
      let text = String(decoding: output.span, as: UTF8.self)
      #expect(text.contains(";reason:\(name);"))
    }
  }

  @Test
  internal func library() throws {
    for mode in [CompatibilityMode.gdb, .lldb] {
      ...
      let identifier =
          ProcessThreadIdentifier(process: ProcessIdentifier(rawValue: 1),
                                  thread: ThreadIdentifier(rawValue: 2))
      let stop = Debuggee.Stop(thread: identifier, reason: .library)
      session.debuggee.observe(.stopped(stop))
      var state =
          GDBRemoteSessionState(compatibility: mode, features: [.libraries])
      state.negotiation.advertise()
      let reply =
          try response(GDBStopPacket.self, packet: "?", session: &session,
                       state: &state)
      let expected = mode == .gdb ? "T05thread:2;library:;"
          : "T00thread:2;reason:shared-library-event;library:1;"
      #expect(reply == Array(expected.utf8))
      if mode == .lldb {
        let threads =
            try response(GDBThreadsInfoPacket.self, packet: "jThreadsInfo",
                         session: &session, state: &state)
        let expected = "[{\"tid\":2,\"reason\":\"shared-library-event\"}]"
        #expect(threads == Array(expected.utf8))
      }
    }
  }
  ...
  internal func compatibility() throws {
    ...
    let listing =
        ...
        "\(CompatibilityMode.lldb.signal(.interrupt))}]"
    #expect(threads == Array(listing.utf8))
    ...
    if case let .extended(process) = state.termination {
