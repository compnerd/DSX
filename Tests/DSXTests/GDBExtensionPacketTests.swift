...
internal struct GDBExtensionPacketTests {
  ...
  internal func launch() throws {
    ...
    let detach = try response { writer throws(Failure) in
      ...
      _ = try launch.detach(payload.span, writer: &writer)
    }
    ...
    let terminal = try response { writer throws(Failure) in
      ...
      _ = try launch.terminal(payload.span, writer: &writer)
    }
  ...
  internal func checksum() throws {
    ...
    try checksum.update(Array("abc".utf8).span)
  ...
  internal func files() throws {
    ...
    let checksum = try response { writer throws(Failure) in
      ...
      _ = try files.handle(payload.span, writer: &writer)
    }
  ...
  internal func core() throws {
    ...
    #expect(try GDBPacketReader(absent.span).hint() == nil)
    let encoded = Array(";path-hint:2f746d702f636f7265".utf8)
    #expect(try GDBPacketReader(encoded.span).hint() == "/tmp/core")
  }
}
...
private func encode(_ value: String) -> String {
  ...
  for byte in value.utf8 {
    encoded.append((byte >> 4).hexadecimal)
    encoded.append(byte.hexadecimal)
  }
...
private func response(capacity size: Int = Tuning.Packet.capacity,
