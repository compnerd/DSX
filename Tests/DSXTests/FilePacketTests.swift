...
internal struct FilePacketTests {
  ...
  @Test
  internal func closed() throws {
    var files = FileSystem()
    let file = try files.open(#filePath, options: [.read], mode: 0)
    try files.close(file)
    let bytes = Array<UInt8>()
    #expect(throws: Debuggee.Error.file(.descriptor)) {
      try files.write(file, offset: 0, bytes: bytes.span)
    }
    #expect(throws: Debuggee.Error.file(.descriptor)) {
      try files.size(file)
    }
    #expect(throws: Debuggee.Error.file(.descriptor)) {
      try files.status(file)
    }
    let buffer = UnsafeMutableBufferPointer<UInt8>(start: nil, count: 0)
    var output = OutputSpan(buffer: buffer, initializedCount: 0)
    #expect(throws: Debuggee.Error.file(.descriptor)) {
      try files.read(file, offset: 0, size: 0, into: &output)
    }
    #expect(throws: Debuggee.Error.file(.invalid)) {
      try files.read(file, offset: 0, size: -1, into: &output)
    }
  }
