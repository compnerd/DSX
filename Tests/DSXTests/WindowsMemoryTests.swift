...
internal struct WindowsMemoryTests {
  ...
  internal func enumeration() throws {
    ...
    var mapped = false
    while address < UInt64.max {
      let location = Debuggee.Address(rawValue: address)
      let region = try WindowsMemory.region(identifier, address: location)
      #expect(region.address.rawValue == address)
      let (end, overflow) =
          region.address.rawValue.addingReportingOverflow(region.size)
      try #require(overflow == false && end > address)
  ...
  internal func progress() throws {
    ...
    let breakpoint = try session.breakpoints.install(identifier, site,
