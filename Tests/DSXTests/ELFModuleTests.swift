...
internal struct ELFModuleTests {
  ...
  internal func dynamic(_ wide: Bool, _ little: Bool) throws {
    ...
    let fields = [
      ...
      (64, 4, UInt64(1)),
      (64 + (wide ? 16 : 8), width, UInt64(0x10000)),
      (64 + (wide ? 32 : 16), width, UInt64(bytes.count)),
      (64 + stride, 4, UInt64(2)),
      (64 + stride + (wide ? 16 : 8), width, UInt64(0x12000)),
      (64 + stride + (wide ? 40 : 20), width, UInt64(0x80)),
    ]
    ...
    #expect(try module.dynamic(at: 0x40000) == 0x42000 ..< 0x42080)
    #expect(throws: Debuggee.Error.process) {
      _ = try module.dynamic(at: UInt64.max)
    }
  }
  ...
  @Test(arguments: [false, true], [false, true])
  internal func programs(_ wide: Bool, _ little: Bool) throws {
    let count = 0x10000
    let stride = wide ? 56 : 32
    let width = wide ? 8 : 4
    let section = 64 + count * stride
    var bytes = Array<UInt8>(repeating: 0, count: section + 64)
    bytes.replaceSubrange(0 ..< 6, with: [
      0x7f, 0x45, 0x4c, 0x46, wide ? 2 : 1, little ? 1 : 2,
    ])
    func field(_ value: UInt64, at offset: Int, size: Int) {
    ...
    field(64, at: wide ? 32 : 28, size: width)
    field(UInt64(stride), at: wide ? 54 : 42, size: 2)
    field(0xffff, at: wide ? 56 : 44, size: 2)
    field(UInt64(section), at: wide ? 40 : 32, size: width)
    field(wide ? 64 : 40, at: wide ? 58 : 46, size: 2)
    field(UInt64(count), at: section + (wide ? 44 : 28), size: 4)
    for index in [0, count - 1] {
      let start = 64 + index * stride
      field(1, at: start, size: 4)
      field(index == 0 ? 0x1000 : 0x9000, at: start + (wide ? 16 : 8),
            size: width)
      field(0x1000, at: start + (wide ? 40 : 20), size: width)
    }
    #expect(try ELFModule(bytes.span)?.extent == 0x1000 ..< 0xa000)
    field(0, at: wide ? 40 : 32, size: width)
    #expect(throws: Debuggee.Error.process) {
      _ = try ELFModule(bytes.span)?.extent
    }
    field(UInt64(section), at: wide ? 40 : 32, size: width)
    field(1, at: wide ? 58 : 46, size: 2)
    #expect(throws: Debuggee.Error.process) {
      _ = try ELFModule(bytes.span)?.extent
    }
    field(wide ? 64 : 40, at: wide ? 58 : 46, size: 2)
