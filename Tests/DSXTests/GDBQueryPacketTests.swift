...
private enum QueryProcessInfoPacket: GDBPacketHandler {
  ...
  internal static func handle(_ payload: borrowing Span<UInt8>,
    ...
    try GDBProcessInfoPacket.write(info, hex: false, writer: &writer)
    return .reply
  }
}
...
private enum QueryModulesPacket: GDBPacketHandler {
  ...
  internal static func handle(_ payload: borrowing Span<UInt8>,
    ...
    while let request = try reader.next() {
      ...
      try GDBModulesPacket.write(request, module: module,
                                 identifier: identity.value, writer: &writer)
      comma = true
    }
    try writer.append(UInt8(ascii: "]"))
    return .reply
...
internal struct GDBQueryPacketTests {
...
#if os(Windows)
    let path = try GDBModulePacket.resolve("module.dll", working: #"C:\tmp"#)
    #expect(path == #"C:\tmp\module.dll"#)
#else
    let path = try GDBModulePacket.resolve("module.so", working: "/tmp")
  ...
  internal func executable() throws {
    ...
    let request = try GDBTransferPacket.parse(.executable,
...
private func expected() -> String {
  ...
  response += "endian:little;ptrsize:\(ABI.width / 8);"
  return response
}
