...
private let kCapacity = String(Tuning.Packet.capacity, radix: 16)
...
private struct QueryImages: Sendable {
  private var images: Array<Debuggee.Image>
  private var failure: Debuggee.Error?
...
private enum QueryImagePacket: GDBPacketHandler {
  ...
  internal static func handle(_ payload: borrowing Span<UInt8>,
    ...
    while true {
      ...
      let address =
          try session.debuggee.address(payload, images: images.span,
                                       state: state)
      if let address {
...
private enum QueryRouter {
  ...
  internal static func dispatch(_ leaf: GDBPacketLeaf,
    ...
    case .supported:
      try state.supported(packet, writer: &writer)
      return .reply
    default:
      throw .unsupported
    }
...
internal struct GDBQueryPacketTests {
  ...
  internal func module() throws {
    ...
    let binary = try Debuggee.Module(path: CommandLine.arguments[0])
    let architecture = try #require(binary.architecture)
    let request = GDBModuleRequest(path: binary.path, triple: architecture)
    let matching = try Debuggee.Module(request, directory: nil)
    #expect(matching.architecture == architecture)
    let mismatch = GDBModuleRequest(path: binary.path, triple: "invalid")
    #expect(throws: GDBHandlerError.code(GDBErrorCode.invalid)) {
      _ = try Debuggee.Module(mismatch, directory: nil)
    }
  }
  ...
  internal func modulepath() throws {
    let binary = try Debuggee.Module(path: CommandLine.arguments[0])
    let source = binary.path
    let separator = try #require(source.lastIndex { $0 == "/" || $0 == "\\" })
    let name = String(source[source.index(after: separator)...])
    let directory = String(source[..<separator])
    let request =
        GDBModuleRequest(path: name, triple: binary.architecture ?? "")
    let module = try Debuggee.Module(request, directory: directory)
    #expect(try module.path == NativeFileSystem.canonical(source))
    #expect(module.architecture == binary.architecture)
  }

#if os(Android)
  @Test(arguments: ["libc.so", "libm.so", "libdl.so"])
  internal func library(_ name: String) throws {
    let binary = try Debuggee.Module(path: CommandLine.arguments[0])
    let architecture = try #require(binary.architecture)
    let request = GDBModuleRequest(path: name, triple: architecture)
    let module = try Debuggee.Module(request, directory: "/data/local/tmp")
    #expect(module.path.first == "/")
    #expect(module.architecture == architecture)
    #expect(module.identity != nil)
    let mismatch = GDBModuleRequest(path: module.path, triple: "invalid")
    #expect(throws: GDBHandlerError.code(GDBErrorCode.invalid)) {
      _ = try Debuggee.Module(mismatch, directory: nil)
    }
  }
  ...
  internal func executable() throws {
    ...
    let empty = request.annex.isEmpty
    #expect(empty)
