...
internal struct GDBPacketChannelTests {
  @Test(arguments: Array<Duration?>([.zero, .microseconds(1), .seconds(1),
                                    .seconds(UInt64.max), nil]))
  internal func readiness(_ timeout: Duration?) throws {
    ...
    #expect(try channel.wait(timeout: .seconds(1), events: Span()) == .channel)
    #expect(try channel.wait(timeout: timeout, events: Span()) == .channel)
  }
  ...
  internal func event() throws {
    ...
    #expect(try channel.wait(timeout: .seconds(1), events: events.span)
        == .timeout)
#else
    #expect(try channel.wait(timeout: .seconds(1), events: events.span)
        == .event)
