...
internal struct GDBPacketChannelTests {
  ...
  internal func readiness() throws {
    ...
    #expect(try channel.wait(timeout: 1000, events: Span()) == .channel)
  }
  ...
  internal func event() throws {
    ...
    #expect(try channel.wait(timeout: 1000, events: events.span) == .timeout)
#else
    #expect(try channel.wait(timeout: 1000, events: events.span) == .event)
#endif
  }
