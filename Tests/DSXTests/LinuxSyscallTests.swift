...
internal struct LinuxSyscallTests {
  ...
  internal func exiting() throws {
    ...
    guard case let .exited(identifier, _) =
  ...
  internal func snapshot() throws {
    ...
    let other = ProcessIdentifier(rawValue: 2)
    var control = LinuxDebugControl()
    control.threads[1] = LinuxThreadState(process: process, stopped: true)
    control.threads[2] = LinuxThreadState(process: other, stopped: true)
    control.threads[3] = LinuxThreadState(process: process, exiting: true)
    control.threads[4] = LinuxThreadState(process: process, newborn: true)
    let expected =
        ProcessThreadIdentifier(process: process,
                                thread: ThreadIdentifier(rawValue: 1))
    #expect(try control.threads(process) == [expected])
    #expect(control.threads.count == 4)
  }
  ...
  internal func redirection(_ address: UInt64) {
    ...
    #expect(registers.pc == address)
    #expect(registers.origin == .max)
    #expect(saved.origin == 35)
    #expect(saved.pc == 0x1000)
  }
#endif
  ...
  internal func injection(_ blocked: Bool) throws {
...
#endif
    let address =
        try LinuxMemory.allocate(process, size: 4096,
    ...
    #expect(throws: Debuggee.Error.state) {
      try LinuxMemory.allocate(process, size: 4096,
                               permissions: [.read, .write], control: &control)
    }
    guard case let .stopped(stop) = try control.event() else {
