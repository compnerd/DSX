...
internal struct WindowsThreadTests {
  @Test
  internal func inventory() throws {
    ...
    control.threads[thread] = WindowsDebugThread(handle: GetCurrentThread())
    #expect(try control.threads(process) == [identifier])
    control.threads.removeValue(forKey: thread)
    // The system thread still exists, but it no longer belongs to the
    // debugger's event inventory and must not be rediscovered by a snapshot.
    #expect(try control.threads(process).isEmpty)
    #expect(throws: Debuggee.Error.process) {
      try control.threads(ProcessIdentifier(rawValue: UInt64.max))
    }
  }
  ...
  @Test("Exited threads retain ownership without requiring a context",
        arguments: [false, true])
  internal func comparators(_ suspended: Bool) throws {
    let raw = CreateThread(nil, 0, { _ in 0 }, nil, 0, nil)
    let handle = try #require(raw)
    defer { _ = CloseHandle(handle) }
    try #require(WaitForSingleObject(handle, 1000) == DSX::WAIT_OBJECT_0)
    let process = ProcessIdentifier(rawValue: UInt64(GetCurrentProcessId()))
    let native = GetThreadId(handle)
    let thread = ProcessThreadIdentifier(native, process: process)
    var control = WindowsDebugControl()
    control.process = process
    control.threads[native] =
        WindowsDebugThread(handle: handle, suspended: suspended)
    let site = BreakpointSite(address: Debuggee.Address(rawValue: 0x1000),
                              size: 1, kind: .watchpoint(.write))
    for enabled in [true, false] {
      try control.breakpoint(process, site: site, thread: nil, enabled: enabled)
      try control.breakpoint(process, site: site, thread: thread,
                             enabled: enabled)
    }
    // Keep ownership until EXIT_THREAD_DEBUG_EVENT is continued. Resuming or
    // stepping another thread must not require this thread's defunct context.
    let selected = try #require(control.threads[native])
    try control.prepare(selected, stepping: true)
    try control.threads[native]?.activate()
    let actions = [Debuggee.Continuation(selection: .process(process),
                                         operation: .step)]
    try control.resume(actions.span)
    #expect(control.threads[native]?.handle == handle)
  }
...
#if arch(i386)
    let raw = DSX::OpenThread(access, false, thread)
#else
    let raw = WinSDK.OpenThread(access, false, thread)
#endif
    let handle = try #require(raw)
    defer { _ = CloseHandle(handle) }
    var control = WindowsDebugControl()
