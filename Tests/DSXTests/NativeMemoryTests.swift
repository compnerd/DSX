...
internal struct NativeMemoryTests {
  ...
  internal func cleanup() throws {
    ...
    session.control.process = foreign
    session.debuggee.insert(Debuggee.Process(identifier: foreign))
    let invalid = MemoryAllocation(process: foreign,
                                   address: Debuggee.Address(rawValue: 1),
                                   size: 4096)
    session.allocations.append(invalid)
    _ = try session.allocate(process, size: 4096, permissions: [.read, .write])
    for _ in 0 ..< 2 {
      #expect(throws: Debuggee.Error.self) {
        try session.close(cause: .normal)
      }
      #expect(session.allocations.count == 1)
      #expect(session.allocations.first?.process == foreign)
      let failed = session.failed
      #expect(failed)
    }
    session.control.process = nil
    #expect(throws: Debuggee.Error.self) {
      try session.close(cause: .normal)
    }
    #expect(session.allocations.isEmpty)
  }
  ...
  internal func release() throws {
    ...
    defer { try? session.deallocate(process) }
    let address =
        try session.allocate(process, size: 4096, permissions: [.read, .write])
  ...
  internal func frames(_ count: Int, _ capacity: Int) throws {
    ...
    try session.settle()
    let address =
        try session.allocate(process, size: UInt64(size),
                             permissions: [.read, .write])
#else
    ...
    try frames.withUnsafeMutableBufferPointer { frames in
      ...
      try withUnsafeTemporaryAllocation(of: UInt8.self, capacity: capacity,
        ...
        let expected = capacity > 32 ? min(count, FrameCursor.frames) : 0
