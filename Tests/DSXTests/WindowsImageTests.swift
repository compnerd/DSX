...
internal struct WindowsImageTests {
  ...
  internal func delivery() throws {
    ...
    for _ in 0 ..< 2 {
      ...
      guard case let .image(image) = control.translated else {
    ...
    guard case let .image(image) = delivered else {
  ...
  internal func startup(_ initial: Bool, _ libraries: Bool) throws {
    ...
    if initial, libraries {
      guard case let .stopped(stop) = control.deferred else {
  ...
  internal func offsets() throws {
    ...
    guard case let .sections(text, data) = offsets else {
