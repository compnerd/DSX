...
internal struct WindowsProcessTests {
  ...
  internal func core(_ named: Bool) throws {
    ...
    while true {
      if case .stopped = try control.event(output: false) {
        break
      }
      try #require(deadline.remaining(at: Host.time) > .zero)
#if arch(i386)
      DSX::Sleep(1)
#else
      WinSDK.Sleep(1)
    ...
    let name = "dsx-core-\(process.rawValue).dmp"
    let hint = try named
        ? NativeFileSystem.resolve(name, directory: directory) : nil
    let path = try CoreDump.dump(process, hint: hint)
    defer { try? NativeFileSystem.remove(path) }
    if let hint {
      #expect(path != hint)
      #expect(try WindowsPath.parent(path) == WindowsPath.parent(hint))
    }
    let mapping = try WindowsMappedFile(path)
    let bytes = mapping.span()
    try #require(bytes.count >= 32)
    #expect(bytes[0] == UInt8(ascii: "M"))
    #expect(bytes[1] == UInt8(ascii: "D"))
    #expect(bytes[2] == UInt8(ascii: "M"))
    #expect(bytes[3] == UInt8(ascii: "P"))
    #expect(CoreDump.capabilities.contains(.core))
    // A hint must never cause an existing file to be overwritten.
    let second = try CoreDump.dump(process, hint: path)
    defer { try? NativeFileSystem.remove(second) }
    #expect(second != path)
  ...
  internal func surrogate(_ count: Int) throws {
    ...
    try text.withUnsafeBufferPointer { input in
      ...
      var expected = String(prefix.prefix(Tuning.Output.capacity))
      if count + 4 <= Tuning.Output.capacity {
        expected += "😀"
        if count + 5 <= Tuning.Output.capacity {
          expected += "z"
        }
      }
  ...
  internal func notification(_ capacity: Int) throws {
    ...
    let count = min(Tuning.Output.capacity, max(1, (capacity - 7) / 2))
    pending.count = count
  ...
  internal func capture(_ fixture: (String, String)) throws(Debuggee.Error) {
    ...
    try bytes.append(addingCapacity: 64) { output throws(Debuggee.Error) in
      status =
          try Host.execute(fixture.0, directory: nil, timeout: .seconds(2),
                           into: &output)
    }
  ...
  internal func drain() throws(Debuggee.Error) {
    ...
    try bytes.append(addingCapacity: 32768) { output throws(Debuggee.Error) in
      let status =
          try Host.execute(command, directory: nil, timeout: .seconds(5),
                           into: &output)
      #expect(status == .completed(.exited(0)))
    }
  ...
  internal func redirect() throws {
    ...
    #expect(try child.status(timeout: .milliseconds(5000)) == .exited(0))
  ...
  internal func server() throws {
    let path = try WindowsPath.canonical(CommandLine.arguments[0])
