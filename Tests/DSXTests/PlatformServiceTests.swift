...
internal struct PlatformServiceTests {
  ...
  internal func threads() throws {
    ...
    #expect(try threads[0].info.thread == threads[0])
  }
  ...
  internal func memory() throws {
    ...
    let address =
        try NativeMemory.allocate(process, size: 4096,
  ...
  @Test(arguments: UInt8(0) ... UInt8(7))
  internal func permissions(_ mask: UInt8) throws {
    let process = ProcessIdentifier(rawValue: UInt64(getpid()))
    var control = NativeDebugControl()
    let permissions = MemoryPermissions(rawValue: mask)
    var reference: mach_vm_address_t = 0
    let task = mach_task_self_
    try #require(mach_vm_allocate(task, &reference, 4096, VM_FLAGS_ANYWHERE)
                 == KERN_SUCCESS)
    defer { _ = mach_vm_deallocate(task, reference, 4096) }
    let status = mach_vm_protect(task, reference, 4096, 0, vm_prot_t(mask))
    if status == KERN_SUCCESS {
      let address =
          try NativeMemory.allocate(process, size: 4096,
                                    permissions: permissions, control: &control)
      defer {
        try? NativeMemory.deallocate(process, address: address, size: 4096,
                                     control: &control)
      }
      let region = try NativeMemory.region(process, address: address)
      #expect(region.readable == permissions.contains(.read))
      #expect(region.writable == permissions.contains(.write))
      #expect(region.executable == permissions.contains(.execute))
    } else {
      #expect(throws: Debuggee.Error(virtual: status)) {
        try NativeMemory.allocate(process, size: 4096, permissions: permissions,
                                  control: &control)
      }
    }
  }
  ...
  internal func services() throws {
    ...
    try bytes.append(addingCapacity: 64) { output in
      status = try Host.execute("printf DSX", directory: nil,
                                timeout: .seconds(2), into: &output)
    }
