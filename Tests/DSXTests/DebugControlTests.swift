...
internal struct LinuxDebugControlTests {
  ...
  internal func interrupt() throws {
    ...
    guard case let .stopped(stop) = try control.event() else {
  ...
  internal func termination() throws {
    ...
    guard case let .terminated(thread, 0) = try control.event() else {
  ...
  internal func exit() throws {
    ...
    guard case let .exited(identifier, .exited(0)) = try control.event() else {
...
internal struct DarwinDebugControlTests {
  ...
  internal func attach(_ stopped: Bool, signal: CInt) throws {
    ...
    if case let .stopped(stop) = try control.event(until: deadline) {
      ...
      if signal == SIGUSR1 || signal == SIGKILL {
        guard case let .exited(exited, .signalled(delivered)) =
      ...
      try control.interrupt(process)
      guard case let .stopped(interrupted) =
          try control.event(until: deadline) else {
  ...
  internal func signal(_ delivery: (Disposition, CInt), original: CInt) throws {
    ...
    try control.attach(process)
    let deadline = try Deadline(.seconds(10), now: Host.time)
    guard case let .stopped(initial) = try control.event(until: deadline) else {
      Issue.record("attach did not report a stop")
      return
    }
    ...
    if original == SIGUSR2 {
      ...
      guard case let .stopped(stop) = try control.event(until: deadline) else {
    ...
    guard case let .exited(exited, status) =
        try control.event(until: deadline) else {
  ...
  internal func output() throws {
    ...
    guard case let .output(identifier) = event else {
  ...
  internal func exit() throws {
    ...
    #expect(captured == bytes)
    guard case let .exited(exited, .exited(0)) = try control.event() else {
