// Copyright © 2026 Saleem Abdulrasool <compnerd@compnerd.org>. All rights reserved.
// SPDX-License-Identifier: BSD-3-Clause

#if os(Windows) && (arch(i386) || arch(x86_64))
internal import Testing
internal import WinSDK
@testable internal import DSX

@Suite
internal struct WindowsBreakpointTests {
#if arch(x86_64)
  @Test
  internal func preservation() throws {
    var control = WindowsDebugControl()
    control.handle = GetCurrentProcess()
    var destination = CONTEXT()
    destination.ContextFlags = DSX::CONTEXT_ALL
    destination.Rip = 0x1234
    destination.Rax = 0x5678
    destination.EFlags = 0x0202
    destination.Dr0 = 0x1000
    destination.Dr7 = 0x0001
    let original = withUnsafeBytes(of: destination) { Array($0) }
    var source = CONTEXT()
    source.Dr0 = 0x1000
    source.Dr1 = 0x2000
    source.Dr2 = 0x3000
    source.Dr3 = 0x4000
    source.Dr6 = 0x0ff0
    source.Dr7 = 0x0055
    try withUnsafeMutablePointer(to: &destination) { pointer in
      source.Rcx = UInt64(UInt(bitPattern: pointer))
      try control.preserve(source)
    }
    let mask = DSX::CONTEXT_DEBUG_REGISTERS ^ DSX::CONTEXT_AMD64
    #expect(destination.ContextFlags == DSX::CONTEXT_ALL & ~mask)
    let start =
        try #require(MemoryLayout<CONTEXT>.offset(of: \CONTEXT.ContextFlags))
    let end = start + MemoryLayout<DWORD>.size
    withUnsafeBytes(of: destination) { bytes in
      #expect(Array(bytes[..<start]) == Array(original[..<start]))
      #expect(Array(bytes[end...]) == Array(original[end...]))
    }
    destination.ContextFlags = DSX::CONTEXT_CONTROL
    destination.Dr0 = 0
    try withUnsafeMutablePointer(to: &destination) { pointer in
      source.Rcx = UInt64(UInt(bitPattern: pointer))
      try control.preserve(source)
    }
    #expect(destination.Dr0 == 0)
    source.Rcx = 1
    #expect(throws: Debuggee.Error.self) {
      try control.preserve(source)
    }
  }
#else
  @Test
  internal func preservation() throws {
    var control = WindowsDebugControl()
    control.handle = GetCurrentProcess()
    var destination = CONTEXT()
    destination.ContextFlags = DSX::CONTEXT_ALL
    destination.Eip = 0x1234
    destination.Eax = 0x5678
    destination.Dr0 = 0x1000
    destination.Dr7 = 0x0001
    let original = withUnsafeBytes(of: destination) { Array($0) }
    var source = CONTEXT()
    try withUnsafeMutablePointer(to: &destination) { pointer in
      var stack: InlineArray<2, UInt> = [0, UInt(bitPattern: pointer)]
      try withUnsafePointer(to: &stack) { stack in
        source.Esp = DWORD(UInt(bitPattern: stack))
        try control.preserve(source)
      }
    }
    let mask = DSX::CONTEXT_DEBUG_REGISTERS ^ DSX::CONTEXT_i386
    #expect(destination.ContextFlags == DSX::CONTEXT_ALL & ~mask)
    let start =
        try #require(MemoryLayout<CONTEXT>.offset(of: \CONTEXT.ContextFlags))
    let end = start + MemoryLayout<DWORD>.size
    withUnsafeBytes(of: destination) { bytes in
      #expect(Array(bytes[..<start]) == Array(original[..<start]))
      #expect(Array(bytes[end...]) == Array(original[end...]))
    }
    source.Esp = 0
    #expect(throws: Debuggee.Error.self) {
      try control.preserve(source)
    }
    source.Esp = DWORD.max
    #expect(throws: Debuggee.Error.self) {
      try control.preserve(source)
    }
  }
#endif

  @Test
  internal func exception() throws(Debuggee.Error) {
    var event = DEBUG_EVENT()
    event.dwDebugEventCode = DSX::EXCEPTION_DEBUG_EVENT
    event.dwProcessId = 7
    event.dwThreadId = 11
    event.u.Exception.ExceptionRecord.ExceptionCode = DSX::EXCEPTION_BREAKPOINT
    event.u.Exception.ExceptionRecord.NumberParameters = 3
    event.u.Exception.ExceptionRecord.ExceptionAddress =
        UnsafeMutableRawPointer(bitPattern: 0x1000)
    event.u.Exception.ExceptionRecord.ExceptionInformation.1 = 0x2000
    let process = ProcessIdentifier(rawValue: 7)
    var control = WindowsDebugControl()
    let translated = try control.translate(&event, process: process)
    switch translated {
    case let .stopped(stop):
      #expect(stop.reason == .breakpoint)
      #expect(stop.fault?.address.rawValue == 0x1000)
      #expect(stop.fault?.data?[1] == 0x2000)
    default:
      Issue.record("expected an x86 breakpoint stop")
    }
  }

  @Test
  internal func watchpoints() throws {
    let process = ProcessIdentifier(rawValue: 7)
    var control = WindowsDebugControl()
    #expect(throws: Debuggee.Error.process) {
      try control.watchpoints(process)
    }
    control.process = process
    #expect(try control.watchpoints(process) == 4)
    #expect(control.pending == nil)
    #expect(control.threads.isEmpty)
  }

  @Test
  internal func partial() throws {
    let primary =
        CreateThread(nil, 0, { _ in 0 }, nil, DWORD(CREATE_SUSPENDED), nil)
    let first = try #require(primary)
    let secondary =
        CreateThread(nil, 0, { _ in 0 }, nil, DWORD(CREATE_SUSPENDED), nil)
    let second = try #require(secondary)
    defer {
      for handle in [first, second] {
        _ = ResumeThread(handle)
        _ = WaitForSingleObject(handle, 1000)
        _ = CloseHandle(handle)
      }
    }
    let process = ProcessIdentifier(rawValue: UInt64(GetCurrentProcessId()))
    var control = WindowsDebugControl()
    control.process = process
    control.handle = GetCurrentProcess()
    control.threads[GetThreadId(first)] = WindowsDebugThread(handle: first)
    control.threads[GetThreadId(second)] = WindowsDebugThread(handle: second)
    let keys = Array(control.threads.keys)
    let good = try #require(control.threads[keys[0]]?.handle)
    let saved = try #require(control.threads[keys[1]])
    // Replacing a value retains dictionary traversal order. The second update
    // fails after the first thread's debug registers have been programmed.
    control.threads[keys[1]] =
        WindowsDebugThread(handle: HANDLE(bitPattern: 1)!)
    var table = BreakpointTable()
    let site = BreakpointSite(address: Debuggee.Address(rawValue: 0x1000),
                              size: 4, kind: .watchpoint(.write))
    let identifier = try table.record(process, site)
    for _ in 0 ..< 2 {
      #expect(throws: Debuggee.Error.self) {
        try table.enable(identifier, control: &control)
      }
      #expect(control.breakpoints.count == 1)
    }
    let flags = DSX::CONTEXT_DEBUG_REGISTERS
    let context = try CONTEXT(good, flags: flags)
    #expect(context.Dr7 & 0xff == 1)
    #expect(throws: Debuggee.Error.self) {
      try table.remove(process, identifier, control: &control)
    }
    #expect(table.find(process, site) == identifier)
    control.threads[keys[1]] = saved
    try table.remove(process, identifier, control: &control)
    #expect(control.breakpoints.isEmpty)
    for handle in [first, second] {
      let context = try CONTEXT(handle, flags: flags)
      #expect(context.Dr7 & 0xff == 0)
    }
  }

  @Test
  internal func invalid() throws {
    let thread =
        CreateThread(nil, 0, { _ in 0 }, nil, DWORD(CREATE_SUSPENDED), nil)
    let handle = try #require(thread)
    defer {
      _ = ResumeThread(handle)
      _ = WaitForSingleObject(handle, 1000)
      _ = CloseHandle(handle)
    }
    let process = ProcessIdentifier(rawValue: UInt64(GetCurrentProcessId()))
    var control = WindowsDebugControl()
    control.process = process
    control.handle = GetCurrentProcess()
    control.threads[GetThreadId(handle)] = WindowsDebugThread(handle: handle)
    var table = BreakpointTable()
    let site = BreakpointSite(address: Debuggee.Address(rawValue: 0x1000),
                              size: 3, kind: .watchpoint(.write))
    #expect(throws: Debuggee.Error.self) {
      _ = try table.install(process, site, control: &control)
    }
    #expect(table.find(process, site) == nil)
    #expect(control.breakpoints.isEmpty)
  }

  @Test(arguments: [true, false])
  internal func sharing(_ reverse: Bool) throws {
    let thread =
        CreateThread(nil, 0, { _ in 0 }, nil, DWORD(CREATE_SUSPENDED), nil)
    let handle = try #require(thread)
    defer {
      _ = ResumeThread(handle)
      _ = WaitForSingleObject(handle, 1000)
      _ = CloseHandle(handle)
    }
    let process = ProcessIdentifier(rawValue: UInt64(GetCurrentProcessId()))
    var control = WindowsDebugControl()
    control.process = process
    control.handle = GetCurrentProcess()
    control.threads[GetThreadId(handle)] = WindowsDebugThread(handle: handle)
    var table = BreakpointTable()
    let address = Debuggee.Address(rawValue: 0x1000)
    let read =
        BreakpointSite(address: address, size: 4, kind: .watchpoint(.read))
    let both =
        BreakpointSite(address: address, size: 4, kind: .watchpoint(.readwrite))
    let first = try table.install(process, read, capacity: 1, control: &control)
    let second =
        try table.install(process, both, capacity: 1, control: &control)
    let flags = DSX::CONTEXT_DEBUG_REGISTERS
    var snapshot = try CONTEXT(handle, flags: flags)
    #expect(snapshot.Dr7 & 0xff == 1)
    #expect(control.breakpoints.count == 1)
    try table.remove(process, reverse ? second : first, control: &control)
    snapshot = try CONTEXT(handle, flags: flags)
    #expect(snapshot.Dr7 & 0xff == 1)
    try table.remove(process, reverse ? first : second, control: &control)
    snapshot = try CONTEXT(handle, flags: flags)
    #expect(snapshot.Dr7 & 0xff == 0)
    #expect(control.breakpoints.isEmpty)
  }

  @Test
  internal func capacity() throws {
    let process = ProcessIdentifier(rawValue: UInt64(GetCurrentProcessId()))
    var table = BreakpointTable()
    let address = Debuggee.Address(rawValue: 0x1000)
    let read =
        BreakpointSite(address: address, size: 4, kind: .watchpoint(.read))
    let both =
        BreakpointSite(address: address, size: 4, kind: .watchpoint(.readwrite))
    _ = try table.record(process, read, capacity: 2)
    _ = try table.record(process, both, capacity: 2)
    let other = BreakpointSite(address: Debuggee.Address(rawValue: 0x2000),
                               size: 4, kind: .watchpoint(.write))
    _ = try table.record(process, other, capacity: 2)
  }

  @Test
  internal func slots() throws {
    let thread =
        CreateThread(nil, 0, { _ in 0 }, nil, DWORD(CREATE_SUSPENDED), nil)
    let handle = try #require(thread)
    defer {
      _ = ResumeThread(handle)
      _ = WaitForSingleObject(handle, 1000)
      _ = CloseHandle(handle)
    }
    let address = Debuggee.Address(rawValue: 0x1000)
    let execution = BreakpointSite(address: address, size: 1, kind: .hardware)
    let byte =
        BreakpointSite(address: address, size: 1, kind: .watchpoint(.write))
    let word =
        BreakpointSite(address: address, size: 4, kind: .watchpoint(.write))
    try WindowsDebugThread(handle: handle).configure(execution, enabled: true)
    try WindowsDebugThread(handle: handle).configure(byte, enabled: true)
    try WindowsDebugThread(handle: handle).configure(word, enabled: true)
    let flags = DSX::CONTEXT_DEBUG_REGISTERS
    var context = try CONTEXT(handle, flags: flags)
    #expect(context.Dr7 & 0x3f == 0x15)
    #expect(context.Dr0 == address.rawValue)
    #expect(context.Dr1 == address.rawValue)
    #expect(context.Dr2 == address.rawValue)
    // Windows may discard synthetic DR6 hit bits on context round trips.
    // Check the cleared slot here; the register test checks preservation.
    context.Dr6 |= 0x07
    try context.commit(to: handle)
    try WindowsDebugThread(handle: handle).configure(byte, enabled: false)
    context = try CONTEXT(handle, flags: flags)
    #expect(context.Dr7 & 0x3f == 0x11)
    #expect(context.Dr0 == address.rawValue)
    #expect(context.Dr1 == 0)
    #expect(context.Dr2 == address.rawValue)
    #expect(context.Dr6 & 0x02 == 0)
    try WindowsDebugThread(handle: handle).configure(execution, enabled: true)
    context = try CONTEXT(handle, flags: flags)
    #expect(context.Dr6 & 0x01 == 0)
    try WindowsDebugThread(handle: handle).configure(execution, enabled: false)
    try WindowsDebugThread(handle: handle).configure(word, enabled: false)
  }
}
#endif
