...
internal struct GDBPlatformPacketTests {
  ...
  internal func identity() throws {
    let identifier = try UInt64(decimal: Array("42".utf8).span)
  ...
  internal func lifecycle() throws {
    ...
    let request = try GDBLaunchRequest(packet.span)
    ...
    let parsed =
        try ProcessIdentifier(rawValue: UInt64(decimal: Array("17".utf8).span))
  ...
  internal func processes() throws {
    ...
    let packet = try response { writer throws(Failure) in
      _ = try state.processes(empty.span, writer: &writer)
    }
    ...
    let missing = try response { writer throws(Failure) in
      _ = try state.processes(filter.span, writer: &writer)
    }
    #expect(missing == Array("E04".utf8))
    let exhausted = try response { writer throws(Failure) in
      try state.processes(writer: &writer)
    }
    #expect(exhausted == Array("E04".utf8))
  }

  @Test(arguments: [false, true])
  internal func malformed(_ active: Bool) throws {
    var state = GDBRemoteSessionState(compatibility: .lldb)
    if active {
      state.enumeration.processes =
          try GDBProcessEnumeration(filter: "__dsx_missing_process__")
    }
    let payload = Array(":name:zz;".utf8)
    #expect(throws: GDBHandlerError.malformed) {
      try response { writer throws(Failure) in
        try state.processes(payload.span, writer: &writer)
      }
    }
    let packet = try response { writer throws(Failure) in
      try state.processes(writer: &writer)
    }
    #expect(packet == Array("E04".utf8))
  }

  @Test
  internal func capability() throws {
    let supported = try response { writer throws(Failure) in
      _ = try writer.append("OK")
    }
    #expect(supported == Array("OK".utf8))

    let watchpoints = try response { writer throws(Failure) in
      try writer.watchpoints(4)
    }
  ...
  internal func shell() throws {
    ...
    let request = try GDBShellRequest(payload.span)
    #expect(request.command == "echo")
    #expect(request.timeout == .seconds(5))
    #expect(request.directory == nil)

    let unlimited = try GDBShellRequest(Array("6563686f,0".utf8).span)
    #expect(unlimited.timeout == nil)
    let large =
        try GDBShellRequest(Array("6563686f,ffffffffffffffff".utf8).span)
    #expect(large.timeout == .seconds(UInt64.max))

    let empty = try GDBShellRequest(Array("6563686f,5,".utf8).span)
    #expect(empty.directory == "")
    let directory = try GDBShellRequest(Array("6563686f,5,2f746d70".utf8).span)
    #expect(directory.directory == "/tmp")

    let response = try response { writer throws(Failure) in
      try writer.append("F,00000000,00000000,out")
      writer.patch(.completed(.exited(7)))
    }
  ...
  internal func directory() throws {
    ...
    let request = try GDBDirectoryRequest(payload.span)
    #expect(request.path == "/tmp/debug")
    #expect(request.mode == 0x01c0)

    let permissions = try GDBDirectoryRequest(payload.span)
  ...
  internal func permissions() throws {
    let payload = Array("1c0,".utf8)
    let request = try GDBDirectoryRequest(payload.span)
    #expect(request.path.isEmpty)
    do {
      _ = try response { writer throws(Failure) in
        try request.permissions(writer: &writer)
      }
      Issue.record("permissions accepted an empty path")
  ...
  internal func modules() throws {
    ...
    let response = try response { writer throws(Failure) in
      _ = try session.launch.handle(.modules, payload: payload.span,
