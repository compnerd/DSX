...
private struct MemoryFixture: ~Copyable {
  ...
  fileprivate func response(_ packet: String) throws -> Array<UInt8> {
    ...
    let capacity = Tuning.Packet.capacity
...
#if os(anyAppleOS)
  @Test
  internal func mappings() throws {
    let fixture = try MemoryFixture()
    try #require(mprotect(fixture.memory + fixture.page, fixture.page,
                          PROT_READ) == 0)
    let raw = fixture.address + UInt64(fixture.page - 1)
    let address = Debuggee.Address(rawValue: raw)
    let bytes: Array<UInt8> = [0x12, 0x34]
    var count = 0
    try NativeMemory.write(fixture.process, address: address, bytes: bytes.span,
                           count: &count)
    #expect(count == 2)
    #expect(fixture.memory[fixture.page - 1] == 0x12)
    #expect(fixture.memory[fixture.page] == 0x34)
    let next = Debuggee.Address(rawValue: raw + 1)
    let region = try NativeMemory.region(fixture.process, address: next)
    #expect(region.readable)
    #expect(region.writable == false)
  }
#endif
