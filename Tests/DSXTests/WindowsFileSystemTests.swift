...
internal struct WindowsFileSystemTests {
  ...
  internal func notification() throws {
    let name = "dsx-port-\(GetCurrentProcessId())"
    let path = try NativeFileSystem.resolve(name, directory: temporary())
    defer { try? NativeFileSystem.remove(path) }
    for port: UInt16 in [65535, 0, 1, 9, 10, 99, 100, 999, 1000, 9999, 10000] {
      try PortNotification.file(path).write(port)
      let handle = try NativeFileSystem.open(path, options: [.read], mode: 0)
      defer { try? NativeFileSystem.close(handle) }
      var bytes = Array<UInt8>()
      try bytes.append(addingCapacity: 6) { output throws(Debuggee.Error) in
        try NativeFileSystem.read(handle, offset: 0, size: 6, into: &output)
      }
      #expect(bytes.elementsEqual("\(port)\n".utf8))
      #expect(try NativeFileSystem.size(handle) == UInt64(bytes.count))
    }
  }
