...
private struct TestRemote<Channel: ByteChannel & ~Copyable, Router: TestRouter>:
    ~Copyable {
  fileprivate var core: GDBRemoteCore<Channel>
  fileprivate var session: Router.Context

  fileprivate init(channel: consuming Channel,
...
private final class TestChannelStorage: @unchecked Sendable {
  fileprivate var input: Array<UInt8>
  fileprivate var offset: Int
  fileprivate var output: Array<UInt8>

  fileprivate init(_ input: Array<UInt8>) {
    self.input = input
    offset = 0
    output = Array<UInt8>()
  }
...
private struct TestChannel: ~Copyable, ByteChannel {
  private let storage: TestChannelStorage

  fileprivate init(_ storage: TestChannelStorage) {
    self.storage = storage
  }

  internal mutating func read(into buffer: inout OutputSpan<UInt8>)
      throws(TransportError) {
    let remaining = storage.input.count - storage.offset
    guard remaining > 0 else {
      return
    }
    let count = min(buffer.freeCapacity, remaining, 2)
    for index in 0 ..< count {
      buffer.append(storage.input[storage.offset + index])
    }
    storage.offset += count
  }

  internal mutating func write(_ buffer: borrowing Span<UInt8>)
      throws(TransportError) -> Int {
    for index in 0 ..< buffer.count {
      storage.output.append(buffer[index])
    }
    return buffer.count
  }
}
...
internal struct GDBRemoteTests {
  @Test
  internal func close() {
    let storage = TestChannelStorage([])
    var remote = GDBRemote(channel: TestChannel(storage),
                           session: DebugSession(), compatibility: .lldb)
  ...
  internal func logical() throws {
    ...
    let storage = TestChannelStorage(frame(payload.span, encoding: .binary))
    var channel = GDBPacketChannel(channel: TestChannel(storage))
  ...
  internal func singular() throws {
    ...
    let storage = TestChannelStorage(frame(payload.span))
    var channel = GDBPacketChannel(channel: TestChannel(storage))
    var packet = false
  ...
  internal func growth() throws {
    ...
    let storage = TestChannelStorage(frame(packet.span, encoding: .binary))
    var remote =
        TestRemote(channel: TestChannel(storage), session: DebugSession(),
  ...
  internal func overflow() throws {
    ...
    let storage = TestChannelStorage(frame(packet.span))
    var remote =
        TestRemote(channel: TestChannel(storage), session: DebugSession(),
  ...
  @Test
  internal func inbound() {
    let input = Array("$aaaaaaaaaaaa".utf8)
    let storage = TestChannelStorage(input)
    var remote =
        TestRemote(channel: TestChannel(storage), session: DebugSession(),
  ...
  internal func legacy() throws {
    ...
    let storage = TestChannelStorage(input)
    var remote =
        GDBRemote(channel: TestChannel(storage), session: DebugSession(),
                  compatibility: .gdb)
    for _ in 0 ..< 4 {
      try remote.step()
    }
  ...
  internal func identity() throws {
    ...
    let storage = TestChannelStorage(input)
    var remote =
        GDBRemote(channel: TestChannel(storage),
    ...
    let debuggee =
        Debuggee(processes: [
          Debuggee.Process(identifier: process, state: .stopped, threads: [
                          Debuggee.Thread(identifier: identifier,
                                          state: .running),
                        ], current: first),
        ])
    var input = Array<UInt8>()
  ...
  internal func event() throws {
    ...
    let storage = TestChannelStorage([])
    var remote = GDBRemote(channel: TestChannel(storage),
                           session: DebugSession(), compatibility: .gdb)
  ...
  internal func termination() throws {
    let process = ProcessIdentifier(rawValue: 7)
    let legacy = TestChannelStorage(Array<UInt8>())
    var remote =
        GDBRemote(channel: TestChannel(legacy), session: DebugSession(),
                  compatibility: .lldb)
    remote.core.state.termination = .legacy(process)
    try remote.handle(event: .exited(process, .signalled(9)))

    let exited = Array("X09".utf8)
    ...
    let extended = TestChannelStorage(Array<UInt8>())
    var session =
        GDBRemote(channel: TestChannel(extended), session: DebugSession(),
                  compatibility: .lldb)
    ...
    let nonstop = TestChannelStorage(Array<UInt8>())
    var channel =
        GDBRemote(channel: TestChannel(nonstop), session: DebugSession(),
                  compatibility: .lldb)
  ...
  internal func notifications() throws {
    for compatibility in [CompatibilityMode.gdb, .lldb] {
      ...
      for reply in ["W02", "OK"] {
        storage.input.append(contentsOf: frame(Array("vStopped".utf8).span))
        try remote.step()
  ...
  internal func snapshot() throws {
    ...
    for query in ["?", "vStopped", "vStopped"] {
      storage.input.append(contentsOf: frame(Array(query.utf8).span))
      try remote.step()
    }
  ...
  internal func running() throws {
    ...
    let storage = TestChannelStorage(frame(Array("?".utf8).span))
    var remote = GDBRemote(channel: TestChannel(storage),
                           session: DebugSession(debuggee: debuggee),
                           compatibility: .gdb)
    remote.core.state.nonstop = true
    try remote.step()
    #expect(storage.output == [UInt8(ascii: "+")] +
        frame(Array("OK".utf8).span))
  }
...
#endif
    #expect(storage.output == frame(Array(expected.utf8).span))
  }
  ...
  internal func platform() throws {
    ...
    let storage = TestChannelStorage(input)
    var remote = PlatformRemote(channel: TestChannel(storage),
  ...
  internal func transfer() throws {
    ...
    let storage = TestChannelStorage(frame(query.span))
    var remote =
        TestRemote(channel: TestChannel(storage), session: TransferSession(),
  ...
  internal func errors() throws {
    ...
    let storage = TestChannelStorage(frame(query.span))
    var remote =
        TestRemote(channel: TestChannel(storage), session: TestSession(),
    ...
    let malformed = TestChannelStorage(frame(query.span))
    var invalid =
        TestRemote(channel: TestChannel(malformed), session: TestSession(),
  ...
  internal func interrupt() throws {
    let storage = TestChannelStorage([0x03])
    var remote =
        TestRemote(channel: TestChannel(storage), session: DebugSession(),
                   router: DefaultRouter.self, compatibility: .gdb)
    try remote.step()
  ...
  internal func waitdisconnect() throws {
    let request =
        Array("vAttachWait;6473782d6e6f2d737563682d70726f63657373".utf8)
    let storage = TestChannelStorage(frame(request.span))
    var remote =
        GDBRemote(channel: TestChannel(storage), session: DebugSession(),
                  compatibility: .lldb)
    do {
