...
private func extract(_ frame: inout Array<UInt8>, cursor: inout Int,
  ...
  return switch frame {
  case let .control(range), let .packet(range): range
  }
}
...
internal struct GDBRemoteTests {
  ...
  internal func fragmented(_ split: Int) throws {
    ...
    #expect(packets == 0)
    #expect(try channel.wait(timeout: .zero, events: Span()) == .timeout)
  ...
  internal func recoveredfragment() throws {
    ...
    #expect(try channel.wait(timeout: .zero, events: Span()) == .channel)
    try channel.receive { _, _, _, _ in packets += 1 }
    #expect(packets == 1)
  }
  ...
  internal func multimemory() throws {
    ...
    let payload = Array<UInt8>([UInt8(ascii: "4"), UInt8(ascii: ";"),
                               ...
                               UInt8(ascii: "}"), UInt8(ascii: "*")])
    let body: GDBPacketResponse = { writer throws(GDBHandlerError) in
      try writer.append(payload.span)
      return .reply
    }
  ...
  internal func echo() throws {
    ...
    var expected = Array<UInt8>()
    for value in ["qEcho:hello", "data:1234", "OK"] {
      expected.append(0x2b)
      let packet = Array(value.utf8)
  ...
  internal func identity() throws {
    ...
    supported +=
        ...
        "swbreak+;hwbreak+"
    if DebugCapabilities.current.contains(.core) {
      supported += ";qSaveCore+"
    }
    supported +=
        ";QNonStop+;jMultiBreakpoint+;binary-upload+;" +
  ...
  internal func lifecycle() throws {
    for compatibility in [CompatibilityMode.gdb, .lldb] {
      ...
      if case let .stopped(stop) = state {
  ...
  internal func platform() throws {
    var input = Array<UInt8>()
    for query in ["qSupported", "qEcho:", "qEcho:123:456", "jModulesInfo:[]",
                  "?"] {
    ...
    let supported =
        "PacketSize=\(capacity)\(kWatchpoints);QStartNoAckMode+;qEcho+"
    var expected = Array<UInt8>()
    for reply in [supported, "qEcho:", "qEcho:123:456", "[]", ""] {
      expected.append(0x2b)
  ...
  @Test(arguments: [CompatibilityMode.gdb, .lldb])
  internal func stoppedinterrupt(_ compatibility: CompatibilityMode) throws {
    let process = ProcessIdentifier(rawValue: 7)
    let thread = ThreadIdentifier(rawValue: 0)
    let identifier = ProcessThreadIdentifier(process: process, thread: thread)
    let stop = Debuggee.Stop(thread: identifier, reason: .breakpoint)
    ...
    let storage = try TestConnection([0x03, 0x03])
    var remote = GDBRemote(channel: storage.connect(), session: session,
                           compatibility: compatibility)
    try remote.step()
    try remote.step()
    #expect(storage.output.isEmpty)
    #expect(remote.core.state.interruption == process)

    // A queued interrupt must not leak into a replacement debuggee.
    remote.session.debuggee.processes.removeAll()
    try storage.send(frame(Array("qUnsupported".utf8).span))
    try remote.step()
    #expect(remote.core.state.interruption == nil)
  }
