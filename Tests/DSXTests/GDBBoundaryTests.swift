...
internal struct GDBBoundaryTests {
  ...
  internal func retry() throws {
    ...
    var attempts = 0
    let result = channel.response(1, encoding: .text, limit: 2,
    ...
    #expect(attempts == 2)
    guard case .success(.close) = result else {
      Issue.record("response retry lost its completion disposition")
      return
    }
    try channel.send()
    #expect(storage.output == Array("$OK#9a".utf8))
  }

  @Test(arguments: ["0,1000,1", "1,1000,1", "2,1000,1", "3,1000,1",
                    "4,1000,1",])
  internal func removal(_ payload: String) throws {
    let process = ProcessIdentifier(rawValue: 7)
    var session = DebugSession()
    session.debuggee.insert(Debuggee.Process(identifier: process))
    let site = try BreakpointSite(payload.utf8Span.span)
    ...
    try withUnsafeTemporaryAllocation(of: UInt8.self, capacity: 8) { buffer in
      ...
      let supported = HardwareBreakpoint.supports(site.kind)
      for _ in 0 ..< 2 {
        if site.kind == .software || supported {
      ...
      let expected = site.kind == .software || supported ? "OKOK" : ""
      let output = writer.finish()
      #expect(String(decoding: output.span, as: UTF8.self) == expected)
    }
  }
