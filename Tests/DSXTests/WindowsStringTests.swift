...
internal struct WindowsStringTests {
  ...
  @Test(arguments: [0, 259, 260, 4096])
  internal func errors(_ count: Int) {
    let value = String(repeating: "x", count: count)
    let expected: DWORD = 0x1234
    withUTF16CString(value) { _ in
#if arch(i386)
      DSX::SetLastError(expected)
#else
      WinSDK.SetLastError(expected)
#endif
    }
