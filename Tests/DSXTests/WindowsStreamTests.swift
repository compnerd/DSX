...
internal struct WindowsStreamTests {
  ...
  internal func readiness() throws {
    ...
    #expect(try WindowsStream.wait(handle, timeout: .zero,
                                   events: Span()) == .timeout)
    let start = GetTickCount64()
    #expect(try WindowsStream.wait(handle, timeout: .milliseconds(20),
                                   events: Span()) == .timeout)
    ...
    #expect(try WindowsStream.wait(handle, timeout: .zero,
                                   events: Span()) == .channel)
    #expect(try WindowsStream.receive(handle, into: &byte, capacity: 1) == 1)
    #expect(byte == 42)
    #expect(try WindowsStream.wait(handle, timeout: .zero,
                                   events: Span()) == .timeout)
  }
  ...
  internal func closure() throws {
    ...
    #expect(try WindowsStream.wait(handle, timeout: .zero,
                                   events: Span()) == .channel)
    var byte: UInt8 = 0
    #expect(try WindowsStream.receive(handle, into: &byte, capacity: 1) == 0)
  }
}
#endif
