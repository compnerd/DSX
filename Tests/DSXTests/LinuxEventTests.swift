...
internal struct LinuxEventTests {
    ...
    for _ in 0 ..< 2 {
      ...
      let ready =
          try events.wait(control) { delay, fds throws(GDBRemoteError) in
            ...
            return try channel.wait(timeout: 0, events: fds)
          }
      #expect(ready == .event)
      try events.drain()
      let drained = try events.wait(control) { _, fds throws(GDBRemoteError) in
        try channel.wait(timeout: 0, events: fds)
      }
  ...
  internal func output() throws {
    ...
    _ = events.wait(control) { delay, fds in
      #expect(delay == Configuration.DebuggeePollInterval)
  ...
  internal func outputPrecedesCompletion(_ exiting: Bool, _ queued: Bool)
    ...
    while captured.count < bytes.count {
      guard case .output(let identifier) = try control.event() else {
    ...
    if exiting {
      guard case .exited(let identifier, .exited(0)) = event else {
      ...
      guard case .stopped(let result) = event else {
  ...
  internal func outputFailureRetainsStop() throws {
    ...
    #expect(control.deferred != nil)
    guard case .stopped(let result) = try control.event(output: false) else {
