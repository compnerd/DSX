...
static inline BOOL dsx_MiniDumpWriteDump(MiniDumpWriteDump_t function,
                                         HANDLE process, DWORD identifier,
                                         HANDLE file, MINIDUMP_TYPE type,
                                         PMINIDUMP_EXCEPTION_INFORMATION info,
                                         PMINIDUMP_USER_STREAM_INFORMATION user,
                                         PMINIDUMP_CALLBACK_INFORMATION hook) {
  return function(process, identifier, file, type, info, user, hook);
}

// Preserve NTAPI on x86 when invoking a dynamically resolved export.
static inline NTSTATUS dsx_NtContinue(NtContinue_t function, PCONTEXT context,
                                      BOOLEAN alert) {
  return function(context, alert);
}
...
static inline HMODULE dsx_LoadLibraryExW(LPCWSTR path, HANDLE file,
                                         DWORD flags) {
  return LoadLibraryExW(path, file, flags);
}

static inline DWORD dsx_GetTempPathW(DWORD length, LPWSTR path) {
  return GetTempPathW(length, path);
}

static inline UINT dsx_GetTempFileNameW(LPCWSTR path, LPCWSTR prefix,
                                        UINT unique, LPWSTR file) {
  return GetTempFileNameW(path, prefix, unique, file);
}

static inline BOOL dsx_DeleteFileW(LPCWSTR path) {
...
static inline FARPROC dsx_GetProcAddress(HMODULE module, LPCSTR name) {
...
static inline DWORD64 dsx_GetEnabledXStateFeatures(void) {
  return GetEnabledXStateFeatures();
}

static inline BOOL dsx_InitializeContext(PVOID buffer, DWORD flags,
                                         PCONTEXT *context, PDWORD size) {
  return InitializeContext(buffer, flags, context, size);
}

static inline BOOL dsx_SetXStateFeaturesMask(PCONTEXT context, DWORD64 mask) {
  return SetXStateFeaturesMask(context, mask);
}

static inline BOOL dsx_GetXStateFeaturesMask(PCONTEXT context, PDWORD64 mask) {
  return GetXStateFeaturesMask(context, mask);
}

static inline PVOID dsx_LocateXStateFeature(PCONTEXT context, DWORD feature,
                                            PDWORD size) {
  return LocateXStateFeature(context, feature, size);
}
...
static inline NTSTATUS dsx_BCryptCreateHash(BCRYPT_ALG_HANDLE algorithm,
                                            BCRYPT_HASH_HANDLE *hash,
                                            PUCHAR object, ULONG size,
                                            PUCHAR secret, ULONG length,
                                            ULONG flags) {
  return BCryptCreateHash(algorithm, hash, object, size, secret, length, flags);
}

static inline NTSTATUS dsx_BCryptHashData(BCRYPT_HASH_HANDLE hash, PUCHAR input,
                                          ULONG size, ULONG flags) {
  return BCryptHashData(hash, input, size, flags);
}

static inline NTSTATUS dsx_BCryptFinishHash(BCRYPT_HASH_HANDLE hash,
                                            PUCHAR output, ULONG size,
                                            ULONG flags) {
  return BCryptFinishHash(hash, output, size, flags);
}

static inline NTSTATUS dsx_BCryptDestroyHash(BCRYPT_HASH_HANDLE hash) {
  return BCryptDestroyHash(hash);
}
#endif
...
static inline void *dsx_RTLD_DEFAULT(void) {
#if defined(__GLIBC__) && !defined(RTLD_DEFAULT)
  // Glibc gates the null default-scope handle behind _GNU_SOURCE.
  return NULL;
#else
  return RTLD_DEFAULT;
#endif
...
typedef int
    (*posix_spawn_file_actions_addchdir_np_t)(posix_spawn_file_actions_t *,
                                            const char *);

// Keep the opaque spawn-action pointer's imported nullability inside C.
static inline int
dsx_spawn_chdir(posix_spawn_file_actions_addchdir_np_t function, void *actions,
                const char *path) {
  return function((posix_spawn_file_actions_t *)actions, path);
}
...
mach_port_t dsx_exception_port(const dsx_exception_context *context);
...
kern_return_t dsx_exception_receive(dsx_exception_context *context,
                                    ...
                                    boolean_t *received);
/// Reply to all pending exceptions in receive order.
kern_return_t dsx_exception_drain(dsx_exception_context *context);
/// Reply to the most recently received pending exception with the given result.
...
// Darwin's thread-directed signal syscall accepts a Mach thread send right.
int __pthread_kill(mach_port_t thread, int signal);
