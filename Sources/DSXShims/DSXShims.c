...
kern_return_t dsx_exception_update(dsx_exception_context *context,
  ...
  if (status != KERN_SUCCESS)
    return status;
  if (context->retained)
    mach_port_mod_refs(mach_task_self(), context->task, MACH_PORT_RIGHT_SEND,
                       -1);
  context->task = task;
  context->retained = TRUE;
...
kern_return_t dsx_exception_restore(dsx_exception_context *context) {
  ...
  for (mach_msg_type_number_t index = 0; index < context->saved; ++index) {
    kern_return_t status =
        task_set_exception_ports(context->task, context->masks[index],
                                 context->ports[index],
                                 context->behaviors[index],
                                 context->flavors[index]);
...
kern_return_t dsx_exception_destroy(dsx_exception_context *context) {
  if (context == NULL)
    return KERN_SUCCESS;
  kern_return_t result = dsx_exception_restore(context);
  for (;;) {
    if (context->count > 0) {
    ...
    dsx_exception_record record;
    kern_return_t status = dsx_exception_receive(context, &record, &received);
    if (status != KERN_SUCCESS) {
      if (result == KERN_SUCCESS)
        result = status;
      break;
  ...
  if (context->port != MACH_PORT_NULL) {
    mach_port_mod_refs(mach_task_self(), context->port, MACH_PORT_RIGHT_SEND,
                       -1);
    mach_port_mod_refs(mach_task_self(), context->port, MACH_PORT_RIGHT_RECEIVE,
                       -1);
  }
  if (context->retained)
    mach_port_mod_refs(mach_task_self(), context->task, MACH_PORT_RIGHT_SEND,
...
kern_return_t dsx_exception_receive(dsx_exception_context *context,
  ...
  *received = FALSE;
  ...
  kern_return_t status =
      mach_msg(&entry.request.header, MACH_RCV_MSG | MACH_RCV_TIMEOUT, 0,
               sizeof(entry.request), context->port, timeout, MACH_PORT_NULL);
  if (status == MACH_RCV_TIMED_OUT)
    return KERN_SUCCESS;
  if (status != KERN_SUCCESS)
    return status;
  dsx_exception_request *request = (dsx_exception_request *)&entry.request;
...
void dsx_exception_detached(dsx_exception_context *context) {
  // XNU's PT_DETACH sigwait path consumes this task suspension while the
  // signal exception is pending (bsd/kern/mach_process.c).
...
NTSYSAPI NTSTATUS NTAPI NtSuspendThread(HANDLE thread, PULONG count);

LONG dsx_NtSuspendThread(HANDLE thread, ULONG *count) {
...
int dsx_openat2(int directory, const char *path, const struct open_how *how,
                  size_t size) {
  return (int)syscall(SYS_openat2, directory, path, how, size);
}
...
#if defined(__ANDROID__)
  // The deployment target does not describe the running device's APIs.
  // Resolve this optional API without raising the minimum Android version.
  int (*action)(posix_spawn_file_actions_t *, const char *) =
      (int (*)(posix_spawn_file_actions_t *, const char *))
          dlsym(RTLD_DEFAULT, "posix_spawn_file_actions_addchdir_np");
  return action ? action(raw, path) : ENOTSUP;
