...
internal struct Server: ~Copyable, Sendable {
  ...
  internal var bound: UInt16? {
    borrowing get {
      switch endpoint {
      case .none: nil
      case let .some(endpoint): endpoint.bound
      }
    }
  }
  ...
  internal mutating func start() throws(ServerError) {
    ...
    do throws(TransportError) {
      ...
      if let notification {
        try notification.write(endpoint?.bound)
      }
    } catch {
  ...
  internal borrowing func wait(timeout: Duration?,
      ...
      throws(ServerError) -> WaitResult {
    switch endpoint {
    case let .some(endpoint):
      do throws(TransportError) {
        return try endpoint.wait(timeout: timeout, events: events)
      } catch {
        throw .transport(error)
      }
  ...
  internal mutating func accept() throws(ServerError) -> ConnectionTransport {
    guard case let .some(current) = endpoint.take() else {
      throw .state
    }
