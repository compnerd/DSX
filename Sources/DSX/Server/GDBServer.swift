...
internal struct GDBServer: ~Copyable, Sendable {
  ...
  internal mutating func accept(events: borrowing NativeEventSource =
                                    NativeEventSource()) throws(ServerError)
      -> GDBRemote {
    DSX.log("waiting for client", level: .trace, channel: .transport)
    if let interval = NativeDebugControl.interval {
      while try wait(timeout: interval) != .channel {
      }
    }
    let transport = try server.accept()
    let session: DebugSession
    do throws(Debuggee.Error) {
      session = try DebugSession(initial, events: events)
    } catch {
  ...
  private borrowing func wait(timeout: Int32) throws(ServerError)
      -> WaitResult {
    switch server.endpoint {
    case .some(let endpoint):
      do throws(TransportError) {
        return try endpoint.wait(timeout: timeout, events: Span())
      } catch {
  ...
  internal mutating func run(daemonize: Bool = false,
                             events mode: DSX.Events = .polling)
      throws(ServerError) {
    guard try server.start(daemonize: daemonize) else {
      return
    }
    let events: NativeEventSource
    do throws(Debuggee.Error) {
      events = try NativeEventSource(mode)
    } catch {
      throw .debuggee(error)
    }
    var remote = try accept(events: events)
    try remote.serve(events: events)
  }
}

extension GDBRemote {
  @inline(never)
  internal mutating func serve(events: borrowing NativeEventSource =
                                   NativeEventSource()) throws(ServerError) {
