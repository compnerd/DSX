...
extension LogConfigurationError: CustomStringConvertible {
  internal var description: String {
    switch self {
    case let .channel(name): "unknown log channel or level '\(name)'"
    case let .destination(code): "unable to open log destination (\(code))"
...
private func channel(_ value: Substring) -> LogChannel? {
  ...
  case "dyld", "events", "step", "thread", "threads": .process
  case "packets": .packet
