...
extension DSX {
  ...
  internal static func logging(_ channels: borrowing String? = nil,
    ...
    let configuration = try LogConfiguration(channels)
    switch file {
    case .some(let file):
      if file.isEmpty {
        kLogger.redirect(NativeLog.error, close: false, colour: .auto)
      } else {
        let descriptor: CInt
        do throws(LogError) {
          descriptor = try NativeLog.open(file, append: false)
        } catch {
          switch error {
          case .open(let code):
