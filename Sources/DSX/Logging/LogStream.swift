...
internal struct LogStream: ~Copyable, Sendable {
  ...
  internal init(descriptor: CInt, level: LogLevel = .info,
    ...
    case .auto: LogSystem.terminal(descriptor)
    case .always: true
    case .never: false
    }
  ...
  internal init(path: String, append: Bool = true, level: LogLevel = .info,
    ...
    let descriptor = try LogSystem.open(path, append: append)
  ...
  internal init(level: LogLevel = .info, channels: UInt64 = LogChannel.all,
    ...
    self.init(descriptor: LogSystem.error, level: level, channels: channels,
              colour: colour)
  }
  ...
  deinit {
    state.withLock { state in
      if state.owned {
        LogSystem.close(state.descriptor)
      }
    }
  }
