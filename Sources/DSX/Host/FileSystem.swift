...
internal func basename(_ path: borrowing Span<UInt8>) -> Range<Int> {
  var start = 0
...
internal func equivalent(_ lhs: borrowing Span<UInt8>,
                         _ rhs: borrowing Span<UInt8>) -> Bool {
  guard lhs.count == rhs.count else {
    return false
  }
  for index in 0 ..< lhs.count {
    guard Host.equivalent(lhs[index], rhs[index]) else {
      return false
    }
  }
  return true
