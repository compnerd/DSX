...
internal struct FileSystem: ~Copyable, Sendable {
  ...
  private borrowing func handle(_ file: FileIdentifier) throws(Debuggee.Error)
...
extension FileSystem {
  ...
  internal borrowing func checksum(_ path: String, directory: String?)
      throws(Debuggee.Error) -> InlineArray<16, UInt8> {
    let path = try NativeFileSystem.resolve(path, directory: directory)
    let file =
        try NativeFileSystem.open(path, options: [.read], mode: 0, root: root)
    defer {
    ...
    var offset: UInt64 = 0
    let capacity = Tuning.Transfer.capacity
    while true {
      let count =
        ...
        try NativeFileSystem.read(file, offset: offset, size: buffer.count,
                                  into: &output)
