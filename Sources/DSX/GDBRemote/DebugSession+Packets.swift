  ...
  internal mutating func handle(_ packet: GDBPacketLeaf,
    ...
    case .H:
      try GDBSelectThreadPacket.handle(payload, session: &self, state: &state,
                                       writer: &writer)
    case .M:
      try GDBMemoryPacket.write(payload, session: &self, state: &state,
                                writer: &writer)
    case .MultiMemRead:
    ...
    case .alive:
      try GDBAlivePacket.handle(payload, session: &self, state: &state,
                                writer: &writer)
    case .X:
      try GDBBinaryMemoryPacket.write(payload, session: &self, state: &state,
                                      writer: &writer)
    case .Z:
    ...
    case .qRcmd:
      ...
      return .close
    case .qSearch:
      try GDBMemoryPacket.search(payload, session: self, state: state,
                                 writer: &writer)
    case .qShlibInfoAddr:
