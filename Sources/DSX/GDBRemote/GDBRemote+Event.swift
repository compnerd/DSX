...
extension GDBRemote {
  internal mutating func handle(event incoming: consuming Debuggee.Event)
    ...
    if notification {
      ...
      do {
        if case let .output(process) = event {
