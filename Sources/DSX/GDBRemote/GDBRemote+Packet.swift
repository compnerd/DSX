...
extension GDBRemote {
  internal mutating func packet() throws(GDBRemoteError) {
    ...
    do throws(GDBRemoteError) {
      guard let result =
        ...
        case .session:
          try route(match.leaf, payload: data, launch: &session.launch,
                    files: &session.files, relative: false, writer: &sink)
        case .mode:
  ...
  private mutating func wait() throws(GDBRemoteError) {
    while case .waiting = session.phase {
      let result =
          try core.channel.wait(timeout: Configuration.AttachWaitInterval,
                                events: Span())
      if result == .channel {
        var interrupt = false
        do throws(GDBRemoteError) {
