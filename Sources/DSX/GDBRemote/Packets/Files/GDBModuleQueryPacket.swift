...
extension Debuggee.Module {
  @inline(never)
  internal init(_ request: borrowing GDBModuleRequest, directory: String?)
      throws(GDBHandlerError) {
    let arch = request.architecture
    let path = try translate(NativeFileSystem.module(request.path,
                                                     directory: directory,
                                                     architecture: arch))
    do {
      self = try Debuggee.Module(path: path, architecture: arch)
    } catch .unsupported {
      throw .code(GDBErrorCode.invalid)
    } catch {
      throw .debuggee(error)
    }
    if let candidate = self.architecture,
        candidate.matches(architecture: arch) == false {
      throw .code(GDBErrorCode.invalid)
    }
  }
}
...
extension GDBPacketWriter {
  internal mutating func emit(_ modules: inout GDBModulesReader,
    ...
    while let request = try modules.next() {
      ...
      do {
        module = try Debuggee.Module(request, directory: directory)
      } catch {
        continue
      }
