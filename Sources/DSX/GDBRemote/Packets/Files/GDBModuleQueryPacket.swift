...
extension Debuggee.Module {
  ...
  internal init(_ request: borrowing GDBModuleRequest, directory: String?)
    ...
    let path =
        try translate(NativeFileSystem.resolve(request.path,
                                               directory: directory))
    let architecture = request.architecture
    do {
      self = try Debuggee.Module(path: path, architecture: architecture)
    } catch .unsupported {
    ...
    if let candidate = self.architecture,
        candidate.matches(architecture: architecture) == false {
      throw .code(GDBErrorCode.invalid)
    }
  }
}
