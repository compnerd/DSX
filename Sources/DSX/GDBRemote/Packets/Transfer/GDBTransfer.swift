...
internal enum GDBTransferPacket {
  internal static func handle(_ object: GDBTransferObject,
    ...
    let request = try GDBTransferRequest(object, payload: payload, state: state)
...
extension GDBTransferRequest {
  internal init(_ object: GDBTransferObject, payload: borrowing Span<UInt8>,
    ...
    self.init(object: object, annex: annex, offset: offset, length: length)
  }
}
