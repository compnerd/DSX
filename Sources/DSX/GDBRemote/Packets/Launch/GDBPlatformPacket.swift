...
internal enum GDBLaunchServerPacket {
  internal static func handle(_ payload: borrowing Span<UInt8>,
    ...
    try write(child, writer: &writer)
  }

  internal static func parse(_ payload: borrowing Span<UInt8>)
      throws(GDBHandlerError) -> GDBLaunchRequest {
    ...
    return GDBLaunchRequest(host: host, port: port)
  }

  internal static func write(_ child: HostProcess.Information,
                             writer: inout GDBPacketWriter)
      throws(GDBHandlerError) {
    try writer.append("pid:")
    try writer.decimal(child.process.rawValue)
    try writer.append(";port:")
    try writer.decimal(UInt64(child.port))
...
internal enum GDBPlatformPermissionsPacket {
  ...
  internal static func parse(_ payload: borrowing Span<UInt8>)
      throws(GDBHandlerError) -> GDBDirectoryRequest {
    var reader = GDBPacketReader(payload.extracting(0...))
    ...
    return GDBDirectoryRequest(path: path, mode: UInt32(mode))
  }
}
...
internal enum GDBPlatformProcessInfoPacket {
  internal static func handle(_ payload: borrowing Span<UInt8>,
    ...
    try GDBProcessInfoPacket.write(info, hex: true, writer: &writer)
  }
}
...
internal enum GDBPlatformShellPacket {
  ...
  internal static func parse(_ payload: borrowing Span<UInt8>)
      throws(GDBHandlerError) -> GDBShellRequest {
    var reader = GDBPacketReader(payload.extracting(0...))
    ...
    let working = try working(reader.remaining(), separated: separated)
    return GDBShellRequest(command: command, working: working, timeout: timeout)
  }
...
private func working(_ payload: borrowing Span<UInt8>, separated: Bool)
