...
extension DebugSession {
  internal mutating func insert(_ site: borrowing BreakpointSite,
    ...
    do {
      ...
      _ = try breakpoints.install(process, site, capacity: capacity,
                                  control: &control)
    } catch {
  ...
  internal mutating func remove(_ site: borrowing BreakpointSite,
    ...
    guard let identifier = breakpoints.find(process, site) else {
      // Tolerate retransmission of a supported Z/z pair. An absent site
      // needs no removal, but an unsupported mechanism remains unsupported.
      switch site.kind {
      case .software:
        return
      case .hardware, .watchpoint:
        guard HardwareBreakpoint.supports(site.kind) else {
          throw .unsupported
        }
        return
      }
    }
    try translate(breakpoints.remove(process, identifier, control: &control))
  }
}
