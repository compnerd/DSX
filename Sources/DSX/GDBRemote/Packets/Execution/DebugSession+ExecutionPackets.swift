...
extension DebugSession {
  private typealias ResumeActionStorage = InlineArray<64, Debuggee.Continuation>
...
extension DebugSession {
  internal mutating func interrupt(_ payload: borrowing Span<UInt8>,
    ...
    if active {
      ...
      // RSP queues interrupts received while stopped until the next resume.
      // Repeating the previous stop would create a second reply to one run.
      state.interruption = process
    }
    return .none
  }
}
...
extension DebugSession {
  internal mutating func vcont(_ payload: borrowing Span<UInt8>,
    ...
    var actions = ResumeActionStorage { _ in empty }
...
extension DebugSession {
  internal mutating func resume(address: Debuggee.Address?,
    ...
    if operation == .resume, case let .thread(thread) = selection {
      let fallback =
