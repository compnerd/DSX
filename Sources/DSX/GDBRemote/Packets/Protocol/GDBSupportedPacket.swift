...
extension GDBRemoteFeatures {
  ...
  private var hidden: Bool {
    switch self {
    case .binary, .events, .executable, .execute, .hwbreak, .map, .options,
        .randomization, .reset, .swbreak, .syscalls, .threads, .unset, .vcont:
      true
    default: false
    }
  }
...
private let kDescriptors: InlineArray<33, GDBFeatureDescriptor> = [
  ...
  GDBFeatureDescriptor(.echo, "qEcho", request: false),
]
