...
extension DebugSession {
  private typealias ThreadStorage = InlineArray<64, UInt64?>

  private borrowing func reply(_ stop: borrowing Debuggee.Stop,
...
extension GDBPacketWriter {
  fileprivate mutating func expedited(_ thread: ProcessThreadIdentifier,
    ...
    do {
      snapshot = try NativeRegisterState(thread, control: control)
    } catch {
...
extension GDBPacketWriter {
  fileprivate mutating func fault(_ stop: borrowing Debuggee.Stop,
    ...
    if let code = fault.exception(stop.reason) {
    ...
    guard case let .signal(signal) = stop.reason else {
      return
    }
...
extension Debuggee.Fault {
  internal func exception(_ reason: Debuggee.StopReason) -> UInt64? {
    // A classified breakpoint or step is not an unclassified Mach exception.
    guard case let .exception(value) = reason else {
      return nil
    }
    if domain == .mach, case .some = data, let code {
      return code
    }
    return value
  }
...
extension DebugSession {
  private borrowing func reason(_ stop: borrowing Debuggee.Stop,
                                ...
                                writer: inout GDBPacketWriter)
      throws(GDBHandlerError) {
    if case let .watchpoint(access, address) = stop.reason {
    ...
    if stop.fault?.domain == .mach, case .exception = stop.reason {
      return
    }
    try writer.append("reason:")
    let reason: StaticString = switch stop.reason {
    case .breakpoint: "breakpoint"
    case .create: "create"
    case .executed: "exec"
    case .exception: "exception"
    case .fork, .spawn: "fork"
    case .interrupt, .signal: "signal"
    case .library: "shared-library-event"
    case .syscall, .trace: "trace"
    case .vfork: "vfork"
    case .vforkdone: "vforkdone"
...
extension DebugSession {
  internal borrowing func status(_ payload: borrowing Span<UInt8>,
    ...
    for process in debuggee.processes {
      if case let .exited(status) = process.state {
...
extension DebugSession {
  ...
  internal borrowing func emit(_ event: Debuggee.Event,
    ...
    case let .exited(process, status):
    ...
    case let .stopped(stop):
      try reply(stop, state: &state, writer: &writer)
    case let .terminated(thread, status):
...
extension DebugSession {
  internal borrowing func emit(_ stop: borrowing Debuggee.Stop,
                               state: borrowing GDBRemoteSessionState,
                               writer: inout GDBPacketWriter,
    ...
    do {
      info = try stop.thread.info(control: control)
    } catch {
    ...
    if registers {
      try writer.expedited(stop.thread, control: control, state: state)
    }
    if state.negotiation.enabled.contains(.stopthreads) {
      ...
      try writer.append(UInt8(ascii: ";"))
      var programs = ThreadStorage { _ in nil }
      var count = 0
      var complete = true
      collect: for process in debuggee.processes
        ...
        for thread in process.threads
          ...
          do {
            let registers =
                try NativeRegisterState(thread.identifier, control: control)
