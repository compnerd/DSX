...
extension DebugSession {
  internal mutating func handle(_ packet: GDBPacketLeaf,
    ...
    case .loader:
      try loader(payload, state: state, writer: &writer)
    case .threads:
    ...
    case .cache:
      try cache(payload, state: state, writer: &writer)
    case .kill:
    ...
    case .qFileLoadAddress:
      try load(payload, state: state, writer: &writer)
    case .qGetPid:
    ...
    case .qMemoryRegionInfo:
      try region(payload, state: state, writer: &writer)
    case .qOffsets:
    ...
    case .qShlibInfoAddr:
      try library(payload, state: state, writer: &writer)
    case .qThreadExtraInfo:
      try thread(payload, writer: &writer)
    case .qSpeedTest:
