...
internal struct DebugSession: ~Copyable, Sendable {
  ...
  internal enum State: Sendable {
    ...
    internal var origin: Origin? {
      switch self {
      case let .starting(origin), let .pending(origin),
          let .terminating(origin), let .stopped(origin),
          let .exited(origin, _):
        origin
      case let .failed(origin): origin
      case .absent, .waiting: nil
      }
    }
  }
  ...
  internal var snapshots = RegisterSnapshots()
  internal var signals = SignalSet()
  internal var deferred = Array<Debuggee.Event>()
  internal var continuations = Array<Debuggee.Continuation>()
