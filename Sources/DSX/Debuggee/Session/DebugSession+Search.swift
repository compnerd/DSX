...
extension DebugSession {
  internal func search(_ process: ProcessIdentifier, address: Debuggee.Address,
    ...
    let chunk = if length < UInt64(Tuning.Transfer.capacity) {
      Int(length)
    } else {
      Tuning.Transfer.capacity
    }
    ...
    return try withUnsafeTemporaryAllocation(of: UInt8.self, capacity: capacity,
      ...
      while consumed < length {
        ...
        let region =
            try NativeMemory.region(process, address: next, control: control)
        ...
        var available = min(region.size - offset, remaining)
        guard available > 0 else {
          throw .memory
        }
        if region.readable == false {
          consumed += available
          retained = 0
          continue
        }
        while available > 0 {
          ...
          let (raw, overflow) =
              address.rawValue.addingReportingOverflow(consumed)
          if overflow {
            throw .memory
          }
          let next = Debuggee.Address(rawValue: raw)
          ...
          if let offset = output.span.first(pattern: pattern) {
...
extension Span where Element == UInt8 {
  fileprivate func first(pattern: borrowing Span<UInt8>) -> Int? {
    guard pattern.count <= count else {
      return nil
    }
    for offset in 0 ... count - pattern.count {
      var matched = true
      for index in 0 ..< pattern.count {
        if self[offset + index] != pattern[index] {
          matched = false
          break
        }
      }
