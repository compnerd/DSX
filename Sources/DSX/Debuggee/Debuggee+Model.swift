...
extension Debuggee {
  ...
  internal borrowing func contains(_ identifier: ProcessIdentifier) -> Bool {
    index(identifier) == nil ? false : true
  }
  ...
  internal borrowing func process(_ selection: Debuggee.Thread.Selection)
      -> ProcessIdentifier? {
    switch selection {
    case .all, .any: processes.first?.identifier
    case let .process(process): process
    case let .thread(thread): thread.process
    }
  }
...
extension Debuggee {
  internal borrowing func validate(_ actions: borrowing Debuggee.Continuations)
  ...
  internal mutating func observe(_ event: borrowing Debuggee.Event,
    ...
    case let .forked(fork):
      ...
      if let index = index(fork.parent.process) {
        processes[index].observe(parent, global: global)
      }
      let stop =
          Debuggee.Stop(thread: fork.child, reason: fork.vfork ? .vfork : .fork)
      let thread =
          Debuggee.Thread(identifier: fork.child, state: .stopped(stop))
      insert(Debuggee.Process(identifier: fork.child.process, state: .stopped,
                              threads: [thread], current: fork.child.thread))
    case .image, .output:
      break
