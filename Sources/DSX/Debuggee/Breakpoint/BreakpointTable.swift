...
internal struct BreakpointTable: ~Copyable, Sendable {
  ...
  internal mutating func record(_ process: ProcessIdentifier,
    ...
    case let (.some(capacity), _): capacity
  ...
  internal mutating func install(_ process: ProcessIdentifier,
                                 _ site: BreakpointSite, capacity: Int? = nil,
                                 native: Bool = false,
    ...
    if let identifier = find(process, site, native: native) {
      try enable(identifier, control: &control)
      return identifier
    }
    let identifier =
        try record(process, site, capacity: capacity, native: native,
                   control: control)
    do {
      try enable(identifier, control: &control)
      return identifier
    } catch {
      try remove(process, identifier, control: &control)
      throw error
    }
  }
  ...
  internal mutating func remove(_ process: ProcessIdentifier,
    ...
    try configure(index, enabled: false, control: &control)
  ...
  internal mutating func enable(_ identifier: BreakpointIdentifier,
    ...
    try configure(index, enabled: true, thread: thread, control: &control)
  }
  ...
  internal mutating func disable(_ identifier: BreakpointIdentifier,
    ...
    try configure(index, enabled: false, thread: thread, control: &control)
  }
  ...
  internal func hit(_ stop: Debuggee.Stop,
    ...
    for entry in entries {
      ...
      if try entry.handle.hit(stop, entry.site, control: control) {
        if entry.native {
  ...
  internal func classify(_ stop: consuming Debuggee.Stop,
    ...
    guard let identifier = try hit(stop, control: control) else {
  ...
  internal mutating func prepare(_ process: ProcessIdentifier,
    ...
    for index in entries.indices {
      if entries[index].process == process,
        ...
        try configure(index, enabled: true, control: &control)
      }
    }
  }
  ...
  internal func restore(_ process: ProcessIdentifier, address: Debuggee.Address,
    ...
    for entry in entries where entry.active && entry.process == process {
      guard case let .software(handle) = entry.handle,
