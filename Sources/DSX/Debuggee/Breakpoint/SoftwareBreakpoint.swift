...
internal struct SoftwareBreakpoint: Sendable {
  ...
  internal init(_ process: ProcessIdentifier, _ breakpoint: BreakpointSite,
    ...
    try withUnsafeMutableBytes(of: &original) { data throws(Debuggee.Error) in
  ...
  internal func enable(_ process: ProcessIdentifier,
    ...
    return try withUnsafeTemporaryAllocation(of: UInt8.self, capacity: size,
      ...
      do throws(Debuggee.Error) {
        ...
        if count > 0 {
          try disable(process, breakpoint, thread: thread, control: control)
        }
        throw error
      }
  ...
  internal func disable(_ process: ProcessIdentifier,
    ...
    do {
      ...
      // Unloading an image also removes its traps. Do not retain a stale
      // installation, but preserve failures for inaccessible mapped memory.
      if let region =
          try? NativeMemory.region(process, address: address, control: control),
          region.mapped == false {
        return
      }
      throw error
