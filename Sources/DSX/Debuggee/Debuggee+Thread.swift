  ...
  internal struct Thread: Sendable {
    ...
    internal enum Selection: Equatable, Sendable {
      ...
      internal func applies(to process: ProcessIdentifier) -> Bool {
        switch self {
        case .all, .any: true
        case let .process(candidate): candidate == process
        case let .thread(candidate): candidate.process == process
        }
      }
    }
    ...
    internal struct Context: Sendable {
      ...
      internal let queue: UInt64?
      internal let quality: Quality?
    }

    internal struct Quality: Sendable {
      internal let value: UInt32
      internal let constant: StaticString
