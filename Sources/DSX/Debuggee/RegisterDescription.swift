...
internal struct RegisterText: Sendable {
  ...
  internal init(storage: StaticString, range: UInt32) {
    self.init(storage: storage,
              range: UInt64(range & 0xffff) | UInt64(range >> 16) << 32)
  }
...
internal struct RegisterNumbering: Sendable {
  ...
  private func number(bias: UInt64) -> Int? {
    ...
    return raw > Int16.min ? Int(raw) : nil
  }
}
...
internal struct RegisterFieldRecord: Sendable {
  ...
  internal var type: RegisterTypeIdentifier? {
    ...
    return encoded > 0 ? RegisterTypeIdentifier(rawValue: encoded - 1) : nil
  }
...
internal struct RegisterStorage: Sendable {
  ...
  internal func layout(bits: Int, offset: Int, bias: Int) -> RegisterStorage {
    let location = location & ~UInt64(UInt32.max)
                 | UInt64(UInt16(bits)) | UInt64(UInt16(offset)) << 16
    let gdb = UInt16(truncatingIfNeeded: numbersraw) &- UInt16(bias)
    let lldb = UInt16(truncatingIfNeeded: numbersraw >> 16) &- UInt16(bias)
    let numbers = numbersraw & ~UInt64(UInt32.max)
                | UInt64(gdb) | UInt64(lldb) << 16
    return RegisterStorage(metadata, location, numbers, relations: relationsraw)
  }
  ...
  internal var expedited: Bool {
    location & 0x0001_0000_0000_0000 > 0
  }
  ...
  internal var type: RegisterTypeIdentifier? {
    ...
    return value < UInt16.max ? RegisterTypeIdentifier(rawValue: value) : nil
  }
...
extension RegisterDescription {
  internal init(_ configuration: RegisterConfiguration) {
    self.init()
    self.configuration = configuration
  }

  @inline(__always)
  internal func type(_ identifier: RegisterTypeIdentifier)
      -> RegisterTypeRecord? {
    for index in 0 ..< types {
      if let type = type(index), type.identifier == identifier {
        return type
      }
    }
    return nil
...
extension RegisterDescription {
  internal func contains(_ feature: RegisterFeatureIdentifier) -> Bool {
    for index in 0 ..< count {
      if let register = register(index),
          configuration.feature(register.feature) == feature {
        return true
      }
    }
    return false
