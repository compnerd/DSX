  ...
  internal enum Event: Sendable {
    ...
    internal var process: ProcessIdentifier {
      ...
      case .executed(let thread), .started(let thread),
        ...
        thread.process
      case .exited(let process, _), .output(let process): process
      case .forked(let fork): fork.parent.process
      case .image(let image): image.process
      case .stopped(let stop): stop.thread.process
      }
    }
