...
extension Debuggee {
  ...
  internal struct Stop: Sendable {
    ...
    internal func refined(reason: StopReason, fault: Fault?,
                          breakpoint: BreakpointIdentifier?) -> Stop {
      Stop(thread: thread, reason: reason, core: core, fault: fault,
           breakpoint: breakpoint, child: child, snapshot: snapshot,
           chance: chance)
    }
  }
  ...
  internal enum Exit: Equatable, Sendable {
    ...
    internal var code: CInt {
      switch self {
      case let .exited(code), let .signalled(code): code
      }
    }
  }
  ...
  internal enum Event: Sendable {
    ...
    internal var completion: Bool {
      switch self {
      case .executed, .exited, .forked, .stopped: true
      case .image, .output, .started, .terminated: false
      }
    }

    internal var lifecycle: Bool {
      switch self {
      case .started, .terminated: true
      case .executed, .exited, .forked, .image, .output, .stopped: false
      }
    }

    internal var process: ProcessIdentifier {
      switch self {
      case let .executed(thread), let .started(thread),
          let .terminated(thread, _):
        thread.process
      case let .exited(process, _), let .output(process): process
      case let .forked(fork): fork.parent.process
      case let .image(image): image.process
      case let .stopped(stop): stop.thread.process
      }
    }
    ...
    internal var refreshable: Bool {
      switch self {
      case .executed, .forked, .started, .stopped: true
      case .exited, .image, .output, .terminated: false
      }
    }
  }
}
