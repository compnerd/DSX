...
extension Debuggee {
  ...
  internal struct Launch: Sendable {
    ...
    /// The child's working directory; nil inherits the parent's directory.
    ...
    /// Reject path truncation before opening launch redirections. Argument and
    /// environment encoders validate their respective native representations.
    @inline(never)
    internal func validate() throws(Debuggee.Error) {
    ...
    internal init(executable: consuming String? = nil,
      ...
      self.directory = consume directory
...
extension String {
  @inline(never)
  fileprivate func validate() throws(Debuggee.Error) {
    let bytes = utf8Span.span
    for index in 0 ..< bytes.count where bytes[index] == 0 {
      throw .process
    }
  }
