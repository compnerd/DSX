...
internal enum WindowsFileSystem {
  ...
  internal static func complete(_ path: String, directories: Bool)
    ...
    repeat {
      let name = withUnsafeBytes(of: &data.cFileName) {
        String(native: $0)
      }
      if name == "." || name == ".." {
        continue
  ...
  internal static func status(_ path: String, link: Bool,
    ...
    let handle = try WindowsHandle(path, reparse: link)
    let file =
  ...
  internal static func destination(_ path: String,
                                   root _: borrowing Root? = nil)
      throws(Debuggee.Error) -> String {
    let handle = try WindowsHandle(path, reparse: true)
  ...
  internal static func status(_ handle: WindowsFileHandle)
    ...
    return FileStatus(device: UInt64(info.dwVolumeSerialNumber), inode: inode,
                      ...
                      access: info.ftLastAccessTime.timestamp,
                      modification: info.ftLastWriteTime.timestamp,
                      change: info.ftCreationTime.timestamp)
  }
}

extension WindowsHandle {
  fileprivate init(_ path: String, reparse: Bool) throws(Debuggee.Error) {
    ...
    let raw = withUTF16CString(path) { path in
      ...
      return CreateFileW(path, 0, sharing, nil, OPEN_EXISTING,
                         FILE_FLAG_BACKUP_SEMANTICS | flag, nil)
    }
