...
internal enum WindowsPath {
  ...
  internal static func canonical(_ path: String) throws(Debuggee.Error)
    ...
    guard result >= 0 else {
      throw Debuggee.Error(hresult: result)
    }
...
private func error(_ code: DWORD, process: Bool) -> Debuggee.Error {
  let invalid: Debuggee.Error =
      process ? .process : .system(CInt(bitPattern: code))
  return Debuggee.Error(windows: code, invalid: invalid)
}
#endif
