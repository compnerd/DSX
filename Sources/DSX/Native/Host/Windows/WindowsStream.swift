    ...
    if GetFileType(native) == FILE_TYPE_PIPE {
      ...
      while true {
        ...
        let remaining = timeout < 0 ? UInt64(interval)
                                    : deadline.remaining(at: GetTickCount64())
        if remaining == 0 {
          return .timeout
        }
...
extension WindowsStreamHandle {
  internal init(_ endpoint: StreamEndpoint) throws(TransportError) {
    ...
    case .descriptor(let handle): try WindowsStreamHandle(descriptor: handle)
    case .device(let path): try WindowsStreamHandle(path: path)
