...
internal struct WindowsProcess: ~Copyable {
  ...
  internal borrowing func byte(timeout: Duration) throws(Debuggee.Error)
      -> UInt8? {
    ...
    if available == 0 {
      Sleep(DWORD(min(timeout.rounded(to: .milliseconds(1),
  ...
  internal borrowing func status(timeout: Duration) throws(Debuggee.Error)
      -> Debuggee.Exit? {
    let milliseconds = timeout.rounded(to: .milliseconds(1), rule: .up)
    let duration = DWORD(min(milliseconds, UInt64(INFINITE - 1)))
    let result = WaitForSingleObject(handle, duration)
    switch result {
    case WAIT_OBJECT_0:
      var code: DWORD = 0
