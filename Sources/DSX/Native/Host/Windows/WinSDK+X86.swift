...
internal func LoadLibraryExW(_ path: LPCWSTR?, _ file: HANDLE?, _ flags: DWORD)
    -> HMODULE? {
  dsx_LoadLibraryExW(path, file, flags)
}

internal func GetTempPathW(_ length: DWORD, _ path: LPWSTR?) -> DWORD {
  dsx_GetTempPathW(length, path)
}

internal func GetTempFileNameW(_ path: LPCWSTR?, _ prefix: LPCWSTR?,
                               _ unique: UINT, _ file: LPWSTR?) -> UINT {
  dsx_GetTempFileNameW(path, prefix, unique, file)
}

internal func DeleteFileW(_ path: LPCWSTR?) -> Bool {
  dsx_DeleteFileW(path)
}

internal func BCryptCreateHash(_ algorithm: BCRYPT_ALG_HANDLE?,
                               _ hash: UnsafeMutablePointer<BCRYPT_HANDLE?>?,
                               _ object: PUCHAR?, _ size: ULONG,
                               _ secret: PUCHAR?, _ length: ULONG,
                               _ flags: ULONG) -> NTSTATUS {
  dsx_BCryptCreateHash(algorithm, hash, object, size, secret, length, flags)
}

internal func BCryptHashData(_ hash: BCRYPT_HASH_HANDLE?, _ input: PUCHAR?,
                             _ size: ULONG, _ flags: ULONG) -> NTSTATUS {
  dsx_BCryptHashData(hash, input, size, flags)
}

internal func BCryptFinishHash(_ hash: BCRYPT_HASH_HANDLE?, _ output: PUCHAR?,
                               _ size: ULONG, _ flags: ULONG) -> NTSTATUS {
  dsx_BCryptFinishHash(hash, output, size, flags)
}

internal func BCryptDestroyHash(_ hash: BCRYPT_HASH_HANDLE?) -> NTSTATUS {
  dsx_BCryptDestroyHash(hash)
}
...
internal func GetProcAddress(_ module: HMODULE?, _ name: UnsafePointer<CChar>?)
    -> FARPROC? {
  dsx_GetProcAddress(module, name)
}
...
internal func GetEnabledXStateFeatures() -> DWORD64 {
  dsx_GetEnabledXStateFeatures()
}

internal func InitializeContext(_ buffer: UnsafeMutableRawPointer?,
                                _ flags: DWORD,
                                _ context: UnsafeMutablePointer<PCONTEXT?>?,
                                _ size: UnsafeMutablePointer<DWORD>) -> Bool {
  dsx_InitializeContext(buffer, flags, context, size)
}

internal func SetXStateFeaturesMask(_ context: PCONTEXT, _ mask: DWORD64)
    -> Bool {
  dsx_SetXStateFeaturesMask(context, mask)
}

internal func GetXStateFeaturesMask(_ context: PCONTEXT,
                                    _ mask: UnsafeMutablePointer<DWORD64>)
    -> Bool {
  dsx_GetXStateFeaturesMask(context, mask)
}

internal func LocateXStateFeature(_ context: PCONTEXT, _ feature: DWORD,
                                  _ size: UnsafeMutablePointer<DWORD>)
    -> UnsafeMutableRawPointer? {
  dsx_LocateXStateFeature(context, feature, size)
}
