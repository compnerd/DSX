...
internal enum WindowsSocket {
  ...
  internal static func accept(_ handle: SOCKET, network: Bool)
      throws(TransportError) -> SOCKET {
    let peer = DSX::accept(handle, nil, nil)
  ...
  internal static func open(_ endpoint: NetworkEndpoint, listening: Bool)
    ...
    while let info = address?.pointee {
      ...
      guard bind(handle, info.ai_addr, CInt(info.ai_addrlen)) == 0 else {
        failure = .bind(code())
        close(handle, path: nil)
        address = info.ai_next
        continue
  ...
  internal static func open(_ endpoint: UnixEndpoint, listening: Bool)
    ...
    do {
      try address(endpoint.path) { address, length in
        status = listening ? bind(handle, address, length)
            : DSX::connect(handle, address, length)
      }
    } catch {
      close(handle, path: nil)
      throw error
  ...
  internal static func wait(_ handle: SOCKET, timeout: Duration?,
    ...
    while true {
      ...
      let status = WSAPoll(&descriptor, 1, milliseconds)
      guard status >= 0 else {
        throw .read(code())
      }
      if status > 0 {
  ...
  internal static func receive(_ handle: SOCKET,
    ...
    let result = recv(handle, buffer, CInt(min(capacity, Int(CInt.max))), 0)
    guard result >= 0 else {
      throw .read(code())
    }
    return Int(result)
  ...
  internal static func transmit(_ handle: SOCKET, from buffer: UnsafeRawPointer,
                                count: Int) throws(TransportError) -> Int {
    let result = send(handle, buffer, CInt(min(count, Int(CInt.max))), 0)
    guard result >= 0 else {
      throw .write(code())
    }
    return Int(result)
  ...
  private static func responsive(_ handle: SOCKET) throws(TransportError) {
    var enabled: CInt = 1
    let status = withUnsafePointer(to: &enabled) { enabled in
      enabled.withMemoryRebound(to: CChar.self, capacity: 1) {
        setsockopt(handle, IPPROTO_TCP, TCP_NODELAY, $0,
                   CInt(MemoryLayout<CInt>.size))
      }
    }
    guard status == 0 else {
