...
#if arch(i386)
@_transparent
internal var CONTEXT_i386: DWORD { DWORD(WinSDK.CONTEXT_i386) }
#elseif arch(x86_64)
@_transparent
internal var CONTEXT_AMD64: DWORD { DWORD(WinSDK.CONTEXT_AMD64) }
#elseif arch(arm64)
@_transparent
internal var CONTEXT_ARM64: DWORD { DWORD(WinSDK.CONTEXT_ARM64) }
#endif

#if arch(i386) || arch(x86_64)
@_transparent
internal var CONTEXT_XSTATE: DWORD { DWORD(WinSDK.CONTEXT_XSTATE) }

@_transparent
internal var XSTATE_AVX: DWORD { DWORD(WinSDK.XSTATE_AVX) }

@_transparent
internal var XSTATE_MASK_AVX: DWORD64 { 1 << XSTATE_AVX }
#endif

internal func NtSuspendThread(_ thread: HANDLE,
                              _ count: UnsafeMutablePointer<ULONG>?) -> LONG {
  dsx_NtSuspendThread(thread, count)
}

internal func NtQueryInformationThread(_ thread: HANDLE, _ information: CInt,
...
@_transparent
internal var STATUS_THREAD_IS_TERMINATING: LONG {
  LONG(dsx_STATUS_THREAD_IS_TERMINATING)
}

@_transparent
internal func connect(_ socket: SOCKET, _ address: UnsafePointer<SOCKADDR>?,
                      _ length: CInt) -> CInt {
#if arch(i386)
  dsx_connect(socket, address, length)
#else
  WinSDK.connect(socket, address, length)
#endif
}

@_transparent
internal func listen(_ socket: SOCKET, _ backlog: CInt) -> CInt {
#if arch(i386)
  dsx_listen(socket, backlog)
#else
  WinSDK.listen(socket, backlog)
#endif
}

@_transparent
internal func accept(_ socket: SOCKET,
                     _ address: UnsafeMutablePointer<SOCKADDR>?,
                     _ length: UnsafeMutablePointer<CInt>?) -> SOCKET {
#if arch(i386)
  dsx_accept(socket, address, length)
#else
  WinSDK.accept(socket, address, length)
#endif
}

@_transparent
internal var BCRYPT_MD5_ALG_HANDLE: BCRYPT_ALG_HANDLE {
  dsx_md5_algorithm()
}
...
@_transparent
internal var STATUS_WX86_SINGLE_STEP: DWORD { 0x4000001e }

@_transparent
internal var STATUS_WX86_BREAKPOINT: DWORD { 0x4000001f }
...
#if arch(i386)
@_transparent
internal var CONTEXT_EXTENDED_REGISTERS: DWORD {
  DWORD(WinSDK.CONTEXT_EXTENDED_REGISTERS)
}
#endif
...
#if arch(i386)
  // The x86 register layout reads the FXSAVE area in ExtendedRegisters.
...
@_transparent
internal var PROCESS_QUERY_INFORMATION: DWORD {
  DWORD(WinSDK.PROCESS_QUERY_INFORMATION)
}

@_transparent
internal var LOAD_LIBRARY_SEARCH_SYSTEM32: DWORD {
  DWORD(WinSDK.LOAD_LIBRARY_SEARCH_SYSTEM32)
}
