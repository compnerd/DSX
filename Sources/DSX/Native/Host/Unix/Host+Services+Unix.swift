...
extension Host {
  ...
  internal static func spawn(_ config: borrowing Debuggee.Launch)
    ...
    do {
      ...
      // Older libc versions cannot express chdir as a spawn action. Reuse
      // child-side setup without tracing or changing the server's directory.
      let descriptors = UnixDescriptors(reader: -1, writer: -1)
      let child = try config.spawn(descriptors: descriptors, tracing: false)
      let identifier = ProcessIdentifier(rawValue: UInt64(child))
