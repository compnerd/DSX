...
#if os(Android)
private typealias posix_spawn_file_actions_addchdir_npBinding =
    LazyBinding<DSXShims.posix_spawn_file_actions_addchdir_np_t>

extension posix_spawn_file_actions_addchdir_npBinding {
  fileprivate func callAsFunction(_ actions: SpawnActions, _ path: SpawnPath)
      -> CInt {
    dsx_spawn_chdir(function, actions, path)
  }
}

// Resolve optional libc support once without raising the deployment target.
private let posix_spawn_file_actions_addchdir_np = {
  let name: StaticString = "posix_spawn_file_actions_addchdir_np"
  return posix_spawn_file_actions_addchdir_npBinding(module: RTLD_DEFAULT, name)
}()
#endif
...
#if os(Android)
  guard let posix_spawn_file_actions_addchdir_np else {
    return ENOTSUP
  }
