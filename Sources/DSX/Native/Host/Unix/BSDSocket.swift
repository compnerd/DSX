...
internal enum BSDSocket {
  private typealias Failure = TransportError

  internal typealias Handle = CInt

  internal static func accept(_ handle: CInt, network: Bool)
  ...
  internal static func open(_ endpoint: UnixEndpoint, listening: Bool)
    ...
    do {
      address = try sockaddr_un(endpoint.path)
    } catch {
      close(handle, path: nil)
      throw error
    }
  ...
  internal static func wait(_ handle: CInt, timeout: Duration?,
  ...
  internal static func receive(_ handle: CInt,
    ...
    while true {
      let result = recv(handle, buffer, capacity, 0)
      if result >= 0 {
        return result
      }
      guard errno == EINTR else {
  ...
  internal static func transmit(_ handle: CInt, from buffer: UnsafeRawPointer,
...
#endif
    while true {
      let result = send(handle, buffer, count, flags)
      if result >= 0 {
        return result
      }
      guard errno == EINTR else {
  ...
  private static func configure(_ handle: CInt, family: CInt)
    ...
    if family == AF_INET6 {
      ...
      let dual = withUnsafePointer(to: &disabled) { disabled in
        setsockopt(handle, IPPROTO_IPV6, IPV6_V6ONLY, disabled,
                   socklen_t(MemoryLayout<CInt>.size))
      }
      guard dual == 0 else {
        throw .option(errno)
  ...
  private static func responsive(_ handle: CInt) throws(TransportError) {
    var enabled: CInt = 1
    let status = withUnsafePointer(to: &enabled) { enabled in
      setsockopt(handle, IPPROTO_TCP, CInt(TCP_NODELAY), enabled,
                 socklen_t(MemoryLayout<CInt>.size))
    }
    guard status == 0 else {
      throw .option(errno)
  ...
  private static func prepare(_ handle: CInt) throws(TransportError) {
...
#if os(anyAppleOS)
    var enabled: CInt = 1
    guard setsockopt(handle, SOL_SOCKET, SO_NOSIGPIPE, &enabled,
