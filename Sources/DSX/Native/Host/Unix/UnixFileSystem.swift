...
internal enum UnixFileSystem {
  private static let capacity = 131_072
  ...
  internal static func module(_ path: String, directory: String?,
                              architecture: String) throws(Debuggee.Error)
      -> String {
#if os(Android)
    let component = switch architecture {
    case "aarch64", "arm64", "x86_64", "amd64": "lib64"
    default: "lib"
    }
    let defaults = "/vendor/\(component):/system/\(component)"
    let paths = if let value = getenv("LD_LIBRARY_PATH") {
      String(cString: value) + ":" + defaults
    } else {
      defaults
    }
    return try library(path, directory: directory, paths: paths)
#else
    try resolve(path, directory: directory)
#endif
  }

  internal static func library(_ path: String, directory: String?,
                               paths: String) throws(Debuggee.Error) -> String {
    let resolved = try resolve(path, directory: directory)
    // An explicit path never falls back to a different library of that name.
    ...
    for entry in paths.split(separator: ":") {
      let parent = try resolve(String(entry), directory: directory)
      let candidate = try resolve(path, directory: parent)
