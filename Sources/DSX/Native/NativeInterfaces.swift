...
private func validate(_ control: inout NativeDebugControl,
  ...
  typealias Region = Debuggee.MemoryRegion

  let _: (ProcessIdentifier, Debuggee.Address, Int, Debuggee.MemoryRegion?,
          inout OutputSpan<UInt8>,
          borrowing NativeDebugControl) throws(Failure) -> Void =
      NativeMemory.read(_:address:size:mapping:into:control:)
  // Writes report the committed prefix through count, including on failure.
  let _: (ProcessIdentifier, Debuggee.Address, borrowing Span<UInt8>, inout Int,
          borrowing NativeDebugControl) throws(Failure) -> Void =
      NativeMemory.write(_:address:bytes:count:control:)
  let _: (ProcessIdentifier, ProcessThreadIdentifier?, Debuggee.Address,
          borrowing Span<UInt8>, inout Int,
          borrowing NativeDebugControl) throws(Failure) -> Void =
      NativeMemory.patch(_:thread:address:bytes:count:control:)
  let _: (ProcessIdentifier, Debuggee.Address,
          borrowing NativeDebugControl) throws(Failure) -> Region =
      NativeMemory.region(_:address:control:)
  let _: (ProcessIdentifier, UInt64, MemoryPermissions,
          inout NativeDebugControl) throws(Failure) -> Debuggee.Address =
      NativeMemory.allocate(_:size:permissions:control:)
  ...
  let _: (ProcessThreadIdentifier,
          borrowing NativeDebugControl) throws(Failure) -> Void =
      NativeRegisterState.synchronize(_:control:)
  ...
  let _: (UnsafeRawPointer, Int) throws(TransportError) -> Int =
      NativeStream.output(_:count:)

  let _: (StreamEndpoint) throws(TransportError) -> StreamHandle =
      NativeStream.open
  let _: (StreamHandle) -> Void = NativeStream.close
  let _: (StreamHandle, Duration?,
          borrowing Span<WaitHandle>) throws(TransportError) -> WaitResult =
  ...
  let _: Array<ProcessThreadIdentifier> = try control.threads(process)
  ...
  let _: Debuggee.Process.Info? = try processes.read()

  // MARK: - Child Process

  let _: ProcessIdentifier = child.process
  let _: UInt8? = try child.byte(timeout: .zero)
  let _: Debuggee.Exit? = try child.status(timeout: .zero)
  ...
  let _: Duration? = NativeDebugControl.polling
  ...
  let _: BreakpointSite? = try control.checkpoint(process)
