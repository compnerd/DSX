...
extension Debuggee.Event {
  internal init(_ record: borrowing dsx_exception_record,
    ...
    let thread = try ThreadIdentifier(mach: record.thread)
    ...
    case EXC_BREAKPOINT:
      ...
      let event = Debuggee.Event.stopped(Debuggee.Stop(thread: identifier,
                                                       reason: .trace,
                                                       fault: fault))
      let translated =
          try DarwinDebugControl.trap(status, event: event, stepping: true,
                                      thread: record.thread, code: codes.0,
                                      threads: threads,
                                      breakpoints: breakpoints)
      guard case let .stopped(stop) = translated else {
        throw .state
      }
