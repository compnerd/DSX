...
extension ProcessIdentifier {
  ...
  internal var address: Debuggee.Address {
    get throws(Debuggee.Error) {
      try address(control: DarwinDebugControl())
    }
  }

  internal func address(control: borrowing NativeDebugControl)
  ...
  internal func images(_ style: Debuggee.Image.Style,
  ...
  internal var cache: Debuggee.SharedCache {
    get throws(Debuggee.Error) {
      try cache(control: DarwinDebugControl())
    }
  }

  internal func cache(control: borrowing NativeDebugControl)
    ...
    return Debuggee.SharedCache(base: base, identifier: identifier,
                                absent: absent,
                                isolated: all.processDetachedFromSharedRegion,
                                path: absent ? nil : path(at: base))
  }

  internal var image: Debuggee.Image? {
    get throws(Debuggee.Error) {
      try image(control: DarwinDebugControl())
    }
  }

  internal func image(control: borrowing NativeDebugControl)
...
extension ProcessIdentifier {
  private func path(at address: Debuggee.Address) -> String? {
    guard let process = try? native else {
      return nil
    }
    return withUnsafeTemporaryAllocation(of: UInt8.self,
                                         capacity: Int(MAXPATHLEN)) { bytes in
      let count = proc_regionfilename(process, address.rawValue,
                                      bytes.baseAddress, UInt32(bytes.count))
      guard count > 0, count <= bytes.count else {
        return nil
      }
      let path = bytes.prefix(Int(count)).prefix { $0 != 0 }
    ...
    for _ in 0 ..< header.ncmds {
      ...
      if identified == false {
        ...
        case LC_BUILD_VERSION:
          ...
          identified = true
        default:
          if let platform = validated.platform {
