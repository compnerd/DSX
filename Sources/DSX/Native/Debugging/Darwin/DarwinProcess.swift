...
extension ProcessIdentifier {
  internal var stopped: Bool {
    ...
    get throws(Debuggee.Error) {
      ...
      let count =
          proc_pidinfo(identifier, PROC_PIDT_SHORTBSDINFO, 0, &info, size)
      guard count == size else {
        throw Debuggee.Error(unix: errno, invalid: .process)
      }
      // task_for_pid can block for a process already stopped by its tracer.
      // Reject that state before acquiring a task or changing exception ports.
      guard info.pbsi_flags & UInt32(PROC_FLAG_TRACED) == 0 else {
        throw .denied
      }
      return info.pbsi_status == SSTOP
    }
  }
  ...
  internal var info: Debuggee.Process.Info {
    get throws(Debuggee.Error) {
      ...
      let parent = status.pbi_ppid > 0
          ? ProcessIdentifier(rawValue: UInt64(status.pbi_ppid)) : nil
      let command = withUnsafeBytes(of: &status.pbi_comm) {
        String(native: $0)
      }
