...
internal struct DarwinARM64RegisterState: ~Copyable {
  ...
  private let exception: arm_exception_state64_t
}

extension DarwinARM64RegisterState {
  internal static func access(_ register: RegisterIdentifier) -> RegisterAccess {
    switch register.rawValue {
    case 162 ... 164: .immutable
    default: .mutable
    }
  }
  ...
  internal consuming func commit(_: ProcessThreadIdentifier)
    ...
    // ARM_EXCEPTION_STATE64 writes are ignored by XNU. Fault observations
    // remain read-only, just as on Intel Darwin.
  }
}
#endif
