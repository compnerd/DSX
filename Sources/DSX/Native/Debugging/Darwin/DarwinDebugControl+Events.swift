    ...
    while true {
      if let exceptions, let record = try exceptions.next() {
        ...
        for index in 0 ..< exceptions.count {
          let thread = try ThreadIdentifier(mach: exceptions[index].thread)
        ...
        for index in 0 ..< exceptions.count {
          ...
          case let .stopped(stop) where stop.thread.thread == stepping: 2
          case .stopped where interrupt == false: 1
          default: 0
          }
        ...
        if stale {
          // A previous interrupt lost the race with another stop. Consume its
          // exception without cancelling the current step or thread holds.
      ...
      self.status = nil
      // Mach exceptions own stops. BSD continuation notifications are not
      // stops either; only terminal wait statuses become process events.
      guard case .some = UnixWaitStatus(status).exit else {
        continue
      }
