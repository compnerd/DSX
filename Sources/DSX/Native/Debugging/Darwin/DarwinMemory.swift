...
internal enum DarwinMemory {
  ...
  internal static func write(_ process: ProcessIdentifier,
    ...
    return try bytes.withUnsafeBytes { bytes throws(Debuggee.Error) in
      let count = min(bytes.count, Int(mach_msg_type_number_t.max))
      guard let mapping = try mapping(task.handle,
                                      address: address.rawValue) else {
        throw .memory
      }
      ...
      guard offset <= mapping.size,
          UInt64(count) <= mapping.size - offset else {
        throw .memory
      }
