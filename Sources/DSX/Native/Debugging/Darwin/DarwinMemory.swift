...
internal enum DarwinMemory {
  ...
  internal static func write(_ process: ProcessIdentifier,
    ...
    try bytes.withUnsafeBytes { bytes throws(Debuggee.Error) in
      while count < bytes.count {
        ...
        guard let mapping = try task.mapping(at: raw, depth: &depth) else {
          throw .memory
        }
  ...
  internal static func allocate(_ process: ProcessIdentifier, size: UInt64,
                                ...
                                control: inout DarwinDebugControl)
    ...
    let protection = (permissions.contains(.read) ? VM_PROT_READ : 0)
                   | (permissions.contains(.write) ? VM_PROT_WRITE : 0)
                   | (permissions.contains(.execute) ? VM_PROT_EXECUTE : 0)
