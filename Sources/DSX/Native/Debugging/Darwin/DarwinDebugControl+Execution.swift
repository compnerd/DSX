...
extension DarwinDebugControl {
  ...
  internal mutating func resume(_ actions: borrowing Debuggee.Continuations)
    ...
    if plan.signal == SIGKILL {
      return try terminate(process)
    }
    ...
    if let exceptions, exceptions.pending {
      ...
      // Releasing a signal exception can terminate the task in the kernel,
      // even while user execution is suspended. Finish native state updates
      // and relinquish our suspension before releasing the exception thread.
      try exceptions.resume()
      return try exceptions.reply(signal)
    }
    try step(actions, process: process, threads: threads)
    let identifier = try process.native
  ...
  internal mutating func restore(_ threads: borrowing DarwinThreadList)
    ...
    while pending > 0 {
      ...
      for index in 0 ..< threads.count {
        let thread = threads[index]
        guard try ThreadIdentifier(mach: thread) == record.thread else {
          continue
        }
        let status = record.count > 0 ? thread_resume(thread)
...
private func awaken(_ process: pid_t) -> Bool {
  ...
  var resumed = false
  while true {
    guard let count = try? task.suspension, count > 0 else {
      return resumed
    }
