...
extension DarwinDebugControl {
  internal mutating func launch(_ config: borrowing Debuggee.Launch)
    ...
    let flags = POSIX_SPAWN_START_SUSPENDED
              ...
              | (config.aslr ? 0 : _POSIX_SPAWN_DISABLE_ASLR)
    ...
    if capture {
      for (path, descriptor) in [
        ...
        try actions.duplicate(descriptors.writer, to: descriptor,
                              failure: Debuggee.Error.init(unix:))
      }
      try actions.close(descriptors.reader, failure: Debuggee.Error.init(unix:))
      try actions.close(descriptors.writer, failure: Debuggee.Error.init(unix:))
    }
    let process = try spawn(executable, arguments: config.arguments.span,
                            environment: config.environment.span,
                            actions: &actions.value, attributes: &attributes)
    ...
    guard ptrace(PT_ATTACHEXC, process, nil, 0) == 0 else {
      let code = errno
      try? UnixProcess.terminate(identifier)
...
private func spawn(_ executable: String, arguments args: borrowing Span<String>,
    ...
    -> pid_t {
  let command = try UnixCommand(args, env: env, prefix: executable)
