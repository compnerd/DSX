...
extension NativeMemory {
  internal static func search(_ process: ProcessIdentifier,
                              address: Debuggee.Address, length: UInt64,
                              pattern: borrowing Span<UInt8>)
    ...
    return try withUnsafeTemporaryAllocation(of: UInt8.self, capacity: capacity,
      ...
      while consumed < length {
        ...
        let region = try region(process, address: next)
        ...
        if region.readable {
          ...
          try read(process, address: next, size: requested, mapping: region,
                   into: &output)
#else
          try read(process, address: next, size: requested, into: &output)
#endif
...
private func match(_ bytes: borrowing Span<UInt8>,
  ...
  for offset in 0 ... bytes.count - pattern.count {
    var matched = true
    for index in 0 ..< pattern.count where matched {
      matched = bytes[offset + index] == pattern[index]
    }
    if matched {
      return offset
    }
