...
extension ProcessIdentifier {
  /// The thread responsible for the latest wait stop.
  internal var stopped: ThreadIdentifier {
    get throws(Debuggee.Error) {
      let owner = try native
#if os(FreeBSD)
      var state = ptrace_lwpinfo()
      let request = PT_LWPINFO
#else
      var state = ptrace_state()
      let request = PT_GET_PROCESS_STATE
#endif
      let size = CInt(MemoryLayout.size(ofValue: state))
      let result = withUnsafeMutablePointer(to: &state) { state in
        let pointer = UnsafeMutableRawPointer(state)
          .assumingMemoryBound(to: CChar.self)
        return ptrace(request, owner, pointer, size)
      }
      guard result == 0 else {
        throw Debuggee.Error(unix: errno)
      }
#if os(FreeBSD)
      let thread = state.pl_lwpid
#else
      let thread = state.pe_tid
#endif
      guard thread > 0 else {
        throw .thread
      }
      return ThreadIdentifier(rawValue: UInt64(thread))
    }
  }
