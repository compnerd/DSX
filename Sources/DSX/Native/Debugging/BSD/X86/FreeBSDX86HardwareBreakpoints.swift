...
extension BSDDebugControl {
  ...
  internal func hit(_ stop: borrowing Debuggee.Stop,
    ...
    for slot in 0 ..< 4 where status & (1 << slot) > 0 {
      if registers[slot] == site.address.rawValue,
          control.matches(registers[7], slot: slot) {
        return true
      }
    }
    return false
  ...
  private static func configure(_ thread: pid_t, site: borrowing BreakpointSite,
                                enabled: Bool) throws(Debuggee.Error) {
    guard let encoded = try? X86BreakpointControl(site) else {
      if enabled {
        throw .breakpoint
      }
      return
    ...
    let existing =
        slot(registers, address: site.address.rawValue, control: encoded)
    let available = enabled ? (0 ..< 4).first(where: { slot in
        X86BreakpointControl.active(registers[7], slot: slot) == false
    }) : nil
  ...
  private static func slot(_ registers: borrowing InlineArray<16, UInt64>,
                           address: UInt64, control: X86BreakpointControl)
      -> Int? {
    (0 ..< 4).first { slot in
      control.matches(registers[7], slot: slot) && registers[slot] == address
    }
  }
