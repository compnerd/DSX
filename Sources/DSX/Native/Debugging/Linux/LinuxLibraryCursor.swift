...
private enum DynamicEntry {
  internal static let size = ABI.width.bytes * 2
  internal static let d_tag = 0
  internal static let d_un = ABI.width.bytes
}

private enum AuxiliaryEntry {
  internal static let size = ABI.width.bytes * 2
  internal static let a_type = 0
  internal static let a_val = ABI.width.bytes
}

private enum DebugRendezvous {
  internal static let r_map = ABI.width.bytes
}

private enum LinkMap {
  internal static let prefix = ABI.width.bytes * 4
  internal static let l_addr = 0
  internal static let l_name = ABI.width.bytes
  internal static let l_ld = ABI.width.bytes * 2
  internal static let l_next = ABI.width.bytes * 3
}
...
internal struct LinuxLibraryCursor {
  ...
  internal init(_ process: ProcessIdentifier) throws(Debuggee.Error) {
    ...
    let location = try process.rendezvous
    let address = try location == 0 ? 0 : process.word(address: location)
    link = try address == 0 ? 0 : process.word(address: address,
                                               offset: DebugRendezvous.r_map)
  }
  ...
  internal mutating func next() throws(Debuggee.Error) -> Debuggee.Library? {
    while link > 0 {
      ...
      // An empty l_name identifies the executable. Preserve its loader
      // coordinates, including when that executable was loaded by ld.so.
...
extension ProcessIdentifier {
  ...
  internal var address: Debuggee.Address {
    get throws(Debuggee.Error) {
      let location = try rendezvous
      guard location > 0 else {
        throw .unsupported
      }
      return Debuggee.Address(rawValue: location)
    }
  }

  fileprivate var rendezvous: UInt64 {
    get throws(Debuggee.Error) {
      ...
      if let debug = try debug(address: address, size: dynamic.size) {
        return debug
      }
      // A directly executed loader need not contain DT_DEBUG. The program
      // it loads still supplies the rendezvous in its own dynamic table.
      let mappings = try LinuxProcFS.contents("/proc/\(process)/maps")
      var maps = LinuxMemoryMapReader(mappings.span)
      while let map = maps.next() {
        guard map.offset == 0, maps.absolute(map),
            let name = maps.path(map) else {
          continue
        }
        let path = String(decoding: name, as: UTF8.self)
        guard let file =
            try? UnixMappedFile("/proc/\(process)/root\(path)"),
            let module = try? ELFModule(file.span()),
            let dynamic = try? module.dynamic(at: map.start.rawValue) else {
          continue
        }
        let size = dynamic.upperBound - dynamic.lowerBound
        guard let debug = try? debug(address: dynamic.lowerBound, size: size),
            let value = try? word(address: debug), value > 0 else {
          continue
        }
        return debug
      }
  ...
  private func debug(address: UInt64, size: UInt64) throws(Debuggee.Error)
    ...
    for index in 0 ..< Int(size / stride) {
      ...
      if tag == DT_DEBUG {
        // qShlibInfoAddr identifies the pointer slot, not struct r_debug.
        return entry + UInt64(DynamicEntry.d_un)
      }
    }
    return nil
  }
  ...
  fileprivate func word(address: UInt64, offset: Int = 0) throws(Debuggee.Error)
      -> UInt64 {
