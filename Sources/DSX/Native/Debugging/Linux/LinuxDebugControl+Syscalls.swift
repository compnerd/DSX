...
extension LinuxDebugControl {
  internal mutating func syscall(_ process: ProcessIdentifier,
    ...
    // Include a completion trap when the ISA cannot hardware-single-step.
    let encoded = instruction.span.withUnsafeBytes {
      $0.loadUnaligned(as: UInt32.self)
    }
    var replacement = word
    withUnsafeMutableBytes(of: &replacement) { replacement in
      replacement.storeBytes(of: encoded, as: UInt32.self)
    }
    try location.poke(thread, word: replacement, request: PTRACE_POKETEXT,
                      failure: Debuggee.Error.init(unix:))
    var restorable = true
#if arch(arm)
    let request = PTRACE_CONT
#else
    let request = PTRACE_SINGLESTEP
#endif
    do throws(Debuggee.Error) {
      try injected.commit(thread)
      guard ptrace(request, thread, nil, nil) == 0 else {
      ...
      while true {
        ...
        // A barrier SIGSTOP may remain pending after another stop won the
        // race. Drain only our own signal, as the ordinary event loop does.
        // Preserve external stops and explicit client interrupts.
        let barrier = if signal == SIGSTOP, requested == false {
          try LinuxDebugControl.generated(thread) == true
        } else {
          false
        }
        if signal == SIGCHLD || barrier {
          if signal == SIGCHLD, threads[thread]?.signal == nil {
        ...
        let pc = completed.pc
        DSX.log("syscall stopped before completion: code=\(code) pc=\(pc)",
...
extension LinuxRegisterState {
  fileprivate func restore(address: UInt64, word: CLong)
...
extension siginfo_t {
  ...
  internal func completes(_ registers: borrowing LinuxGeneralRegisters,
    ...
    return UInt64(registers.pc) == address
  }

  @inline(__always)
  internal func pending(_ registers: borrowing LinuxGeneralRegisters,
                        at address: UInt64) -> Bool {
    let pc = UInt64(registers.pc)
    return completes(registers, at: address) || kernel && pc == address
  }
