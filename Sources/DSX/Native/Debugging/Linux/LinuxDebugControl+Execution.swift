...
extension LinuxDebugControl {
  internal mutating func discard(_ event: borrowing Debuggee.Event)
    ...
    case let .started(thread): thread
    default: nil
    } else {
      return
    }
    ...
    try resume(native, request: request(native), signal: 0)
  }
  ...
  internal mutating func resume(_ actions: borrowing Debuggee.Continuations)
    ...
    for index in 0 ..< actions.count {
      ...
      guard action.delivery == .process, let signal = action.signal,
          case let .process(owner) = action.selection else {
        continue
      }
  ...
  internal mutating func resume(_ thread: pid_t, request: CInt, signal: CInt)
    ...
    var result: CLong =
        if var pending = threads[thread]?.signal, pending.si_signo == signal {
          withUnsafeMutablePointer(to: &pending) { information in
    ...
    if result == 0 {
      ...
      result = ptrace(request, thread, nil, data)
    }
    if result < 0 {
      let failure = errno
      guard failure == ESRCH, threads[thread]?.stopped == true else {
        throw Debuggee.Error(unix: failure)
      }
      // SIGKILL or exit_group can destroy a tracee in ptrace-stop. Retain
      // its wait status: death need not be waitable yet when ptrace fails.
      // Verify wait ownership before treating ESRCH as this transition.
      var information = siginfo_t()
      guard waitid(P_PID, id_t(thread), &information,
                   WEXITED | WNOHANG | WNOWAIT | __WALL) == 0 else {
        throw Debuggee.Error(unix: failure)
      }
      threads[thread]?.exiting = true
    }
    threads[thread]?.signal = nil
    threads[thread]?.reported = false
    threads[thread]?.stopped = false
    threads[thread]?.stepping = result == 0 && request == PTRACE_SINGLESTEP
  }
  ...
  internal mutating func complete(_ event: borrowing Debuggee.Event)
    ...
    case let .executed(thread): thread
    case let .forked(fork): fork.parent
    case let .stopped(stop): stop.thread
    case .exited, .image, .output, .started, .terminated: nil
    } else {
      return
    }
