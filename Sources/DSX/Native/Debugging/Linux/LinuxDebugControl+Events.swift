...
extension LinuxDebugControl {
  ...
  internal mutating func event(blocking: Bool = false, output: Bool = true,
    ...
    while true {
      if let deferred {
        return try deliver(deferred, output: output)
      }
      guard let process else {
        throw .state
      }
      if let event = collect() {
        return try deliver(event, output: output)
      }
      ...
      do throws(Debuggee.Error) {
        event = try receive(status, thread: native, process: process,
                            launch: launch, signals: signals)
      } catch {
        // SIGKILL and exit_group can supersede a collected ptrace-stop.
        // ESRCH can precede the final wait status: a dead group leader is
        // not waitable until its siblings exit. Verify wait ownership and
        // retain the thread until its terminal status can be consumed.
        var information = siginfo_t()
        guard decoded.stopped,
            waitid(P_PID, id_t(native), &information,
                   WEXITED | WNOHANG | WNOWAIT | __WALL) == 0,
            error == .process || information.si_signo == SIGCHLD else {
          throw error
        }
      ...
      // SIGCHLD coalesces. An internally handled status is not evidence that
      // no other traced thread has a pending status.
    }
  }

  internal mutating func deliver(_ event: Debuggee.Event, output: Bool)
      throws(Debuggee.Error) -> Debuggee.Event {
    deferred = event
    // Output can arrive between the pre-wait read and the waitpid result.
    // Drain it before publishing the stop or closing the exited process.
    if output, event.completion {
      if self.output == nil {
        self.output = try Debuggee.Output(reader, closed: &exhausted)
      }
      if self.output != nil, let process {
        return .output(process)
      }
    }
    if case let .exited(process, _) = event {
      try depart(process)
    }
    deferred = nil
    return event
  ...
  private mutating func receive(_ status: CInt, thread native: pid_t,
    ...
    if case let .stopped(stop) = translated,
        case let .signal(signal) = stop.reason, signals.contains(signal),
        launch == false {
    ...
    if translated.completion {
      threads[native]?.stopped = true
      switch translated {
      case let .stopped(stop) where stop.reason == .interrupt:
        requested = false
      default:
        if requested {
          obsolete = true
  ...
  private mutating func wait(_ reader: CInt?, blocking: Bool)
    ...
    while true {
      if let output = try Debuggee.Output(reader, closed: &exhausted) {
