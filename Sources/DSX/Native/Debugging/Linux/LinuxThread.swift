...
extension ProcessIdentifier {
  internal var threads: Array<ProcessThreadIdentifier> {
    get throws(Debuggee.Error) {
      try threads(living: true)
    }
  }

  internal func threads(living: Bool) throws(Debuggee.Error)
    ...
    while true {
      ...
      if let identifier, identifier > 0, identifier <= UInt64(pid_t.max) {
        ...
        let include = try living == false ? true : pair.alive
        if include {
          threads.append(pair)
        }
      }
  ...
  private var state: UInt8 {
    get throws(Debuggee.Error) {
      ...
      return try withUnsafeTemporaryAllocation(of: UInt8.self, capacity: 256,
        ...
        guard let status = LinuxProcessStatus(output.span) else {
          throw .thread
        }
        return status.state
      })
