...
@_transparent
internal var SFD_CLOEXEC: CInt { CInt(DSXShims.SFD_CLOEXEC) }

@_transparent
internal var SFD_NONBLOCK: CInt { CInt(DSXShims.SFD_NONBLOCK) }

#if arch(arm) || arch(i386)
@_transparent
internal var SYS_mmap2: UInt64 { UInt64(DSXShims.SYS_mmap2) }
#else
@_transparent
internal var SYS_mmap: UInt64 { UInt64(DSXShims.SYS_mmap) }
#endif

@_transparent
internal var SYS_munmap: UInt64 { UInt64(DSXShims.SYS_munmap) }
...
#if arch(i386)
/// Fixed-size FXSAVE requests from Linux UAPI `<asm/ptrace-abi.h>`.
@_transparent
internal var PTRACE_GETFPXREGS: CInt { 18 }

@_transparent
internal var PTRACE_SETFPXREGS: CInt { 19 }
#elseif arch(arm)
/// ELF note type from Linux UAPI `<asm/ptrace.h>`.
@_transparent
internal var NT_ARM_VFP: Int { 0x400 }
#elseif arch(arm64)
/// ELF note types from Linux UAPI `<asm/ptrace.h>`.
@_transparent
internal var NT_ARM_TLS: Int { 0x401 }

@_transparent
internal var NT_ARM_HW_BREAK: Int { 0x402 }

@_transparent
internal var NT_ARM_HW_WATCH: Int { 0x403 }

@_transparent
internal var NT_ARM_SVE: Int { 0x405 }

@_transparent
internal var NT_ARM_PAC_MASK: Int { 0x406 }

@_transparent
internal var SVE_PT_REGS_SVE: UInt16 { 0x0001 }
#endif

/// Linux's `ADDR_NO_RANDOMIZE` personality flag from `<sys/personality.h>`.
@_transparent
internal var ADDR_NO_RANDOMIZE: CUnsignedLong { 0x0004_0000 }
...
@_transparent
internal var PTRACE_SEIZE: CInt {
  0x4206
}

@_transparent
internal var PTRACE_INTERRUPT: CInt {
  0x4207
}
...
@_transparent
internal var PTRACE_SETSIGINFO: CInt {
  0x4203
}
...
@_transparent
internal var SI_USER: CInt {
#if os(Android)
  CInt(Android.SI_USER)
#else
  CInt(Glibc.SI_USER)
#endif
}
...
@_transparent
internal var BUS_ADRALN: CInt {
  1
}

@_transparent
internal var BUS_ADRERR: CInt { 2 }

@_transparent
internal var BUS_OBJERR: CInt {
  3
}

@_transparent
internal var BUS_MCEERR_AR: CInt { 4 }

@_transparent
internal var BUS_MCEERR_AO: CInt { 5 }

@_transparent
internal var SEGV_MAPERR: CInt {
  1
}

@_transparent
internal var SEGV_ACCERR: CInt { 2 }

@_transparent
internal var SEGV_BNDERR: CInt { 3 }

@_transparent
internal var SEGV_PKUERR: CInt {
  4
}

@_transparent
internal var SEGV_MTESERR: CInt {
  9
}

@_transparent
internal var SEGV_MTEAERR: CInt { 8 }

@_transparent
internal var SEGV_CPERR: CInt { 10 }
...
@_transparent
internal var NT_X86_XSTATE: Int { 0x202 }
