...
internal struct LinuxRegisterState: Sendable {
  ...
  internal var extended: LinuxXState?
#endif
#if arch(arm64)
  internal var tls: UInt64
  internal var masks: LinuxPACMask?
  internal var scalable: LinuxScalableRegisters?
#endif

  internal init(_ identifier: ProcessThreadIdentifier,
                control _: borrowing NativeDebugControl = NativeDebugControl())
      throws(Debuggee.Error) {
    ...
    masks = try? LinuxPACMask(thread)
#else
    floating = try LinuxFloatingRegisters(thread)
#if arch(i386) || arch(x86_64)
    extended = try LinuxXState(thread)
#endif
#endif
  }
  ...
  private consuming func commit() throws(Debuggee.Error) {
    ...
    if extended != nil {
      ...
      try extended?.commit(thread, floating: floating)
      return
    }
#endif
    try floating.commit(thread)
