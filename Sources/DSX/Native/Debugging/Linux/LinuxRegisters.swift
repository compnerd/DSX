...
internal enum LinuxRegisters {
  ...
  internal static func syscall(_ identifier: ProcessThreadIdentifier)
    ...
    let state = try snapshot(identifier)
#if arch(arm64)
    return state.general.values[8]
#elseif arch(arm)
    return UInt64(state.general.values[7])
#elseif arch(i386) || arch(x86_64)
    return UInt64(state.general.origin)
#endif
  }
...
private func transfer<Value>(_ request: CInt, note: Int, thread: pid_t,
  ...
  guard result == 0 else {
    throw UnixError.register(errno)
  }
