...
internal enum LinuxMemory {
  ...
  internal static func allocate(_ process: ProcessIdentifier, size: UInt64,
    ...
    let protection = (permissions.contains(.read) ? UInt64(PROT_READ) : 0)
                   | (permissions.contains(.write) ? UInt64(PROT_WRITE) : 0)
                   | (permissions.contains(.execute) ? UInt64(PROT_EXEC) : 0)
    let flags = UInt64(MAP_PRIVATE | MAP_ANONYMOUS)
    var arguments = InlineArray<7, UInt64> { _ in 0 }
#if arch(arm) || arch(i386)
    arguments[0] = SYS_mmap2
#else
    arguments[0] = SYS_mmap
#endif
    arguments[2] = size
  ...
  internal static func read(_ process: ProcessIdentifier,
    ...
    try output.withUnsafeMutableBufferPointer { data, offset throws(Failure) in
      let pointer = try UnsafeMutableRawPointer(bitPattern: address.native)
      guard let base = data.baseAddress else { return }
      let requested = min(size, data.count - offset, Int(available))
      var local = iovec(iov_base: base.advanced(by: offset),
                        iov_len: numericCast(requested))
      var remote = iovec(iov_base: pointer, iov_len: numericCast(requested))
      ...
      try address.read(thread, size: requested, into: data, offset: &offset)
    }
  }
  ...
  internal static func write(_ process: ProcessIdentifier,
    ...
    count = try bytes.withUnsafeBytes { bytes throws(Debuggee.Error) in
      ...
      let thread = try thread(process, fallback: identifier)
      return try address.write(thread, bytes: bytes)
    }
  }
  ...
  internal static func patch(_ process: ProcessIdentifier,
    ...
    guard let thread else {
      return try write(process, address: address, bytes: bytes, count: &count)
    }
    let target = try LinuxMemory.target(process, thread: thread)
    count = try bytes.withUnsafeBytes { bytes throws(Debuggee.Error) in
      try address.write(target, bytes: bytes)
    }
  }
  ...
  internal static func region(_ process: ProcessIdentifier,
    ...
    while let map = maps.next() {
      ...
      let name: String? = if let path = maps.path(map) {
      ...
      return Debuggee.MemoryRegion(address: map.start,
                                   ...
                                   executable: map.executable, name: name)
    }
...
extension Debuggee.Address {
  fileprivate func read(_ process: pid_t, size: Int,
    ...
    while copied < size {
      ...
      do throws(Debuggee.Error) {
        var word = try Debuggee.Address(rawValue: aligned).peek(process)
  ...
  fileprivate func write(_ process: pid_t, bytes: UnsafeRawBufferPointer)
    ...
    while copied < bytes.count {
      ...
      do throws(Debuggee.Error) {
        var word: CLong = if displacement == 0, count == width {
          ...
          try Debuggee.Address(rawValue: aligned).peek(process)
        }
        ...
        try Debuggee.Address(rawValue: aligned).poke(process, word: word)
        copied += count
      } catch {
        if copied > 0 {
          return copied
