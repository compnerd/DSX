...
extension LinuxDebugControl {
  ...
  internal static func configure(_ thread: pid_t,
    ...
    let control = try X86BreakpointControl.encode(site)
#if arch(i386)
    guard site.address.rawValue <= UInt64(UInt32.max) else {
      throw .breakpoint
    }
#endif
    var status = try read(thread, register: kDebugControlRegister)
    let existing =
        try slot(thread, address: site.address.rawValue, status: status)
    ...
    if enabled {
      ...
      try write(thread, register: index, value: 0)
    }
  ...
  private static func slot(_ thread: pid_t, address: UInt64,
    ...
    for slot in 0 ..< kDebugSlotCount
        where X86BreakpointControl.active(status, slot: slot) {
