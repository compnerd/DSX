...
/// Offset of `u_debugreg` in Linux's x86 `struct user`.
private var kUserDebugRegisterOffset: Int {
#if arch(i386)
  0xfc
#else
  0x350
...
extension LinuxDebugControl {
  ...
  internal static func configure(_ thread: pid_t,
    ...
    let available = enabled ? slots.first(where: { candidate in
        X86BreakpointControl.active(status, slot: candidate) == false
    }) : nil
  ...
  private static func read(_ thread: pid_t, register: Int)
    ...
    if value == -1, errno > 0 {
      throw Debuggee.Error(breakpoint: errno)
    }
