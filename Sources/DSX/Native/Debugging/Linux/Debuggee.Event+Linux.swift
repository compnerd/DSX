...
extension Debuggee.Event {
  internal func fault(_ thread: pid_t) throws(Debuggee.Error)
      -> Debuggee.Event {
    guard case let .stopped(stop) = self,
        case let .signal(signal) = stop.reason else {
      return self
    }
  ...
  internal func trap(_ thread: pid_t, stepping: Bool) throws(Debuggee.Error)
      -> Debuggee.Event {
    guard case let .stopped(stop) = self else {
      return self
    }
