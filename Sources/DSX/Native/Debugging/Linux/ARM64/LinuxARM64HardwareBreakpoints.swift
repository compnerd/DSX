...
extension LinuxDebugControl {
  internal func watchpoints(_ process: ProcessIdentifier) throws(Debuggee.Error)
    ...
    return try LinuxDebugControl.capacity(pid_t(process.rawValue),
                                          note: NT_ARM_HW_WATCH)
  }
  ...
  internal func hit(_ stop: borrowing Debuggee.Stop,
    ...
    if let fault = stop.fault {
      ...
      case .watchpoint:
        let encoded = try ARM64BreakpointControl.encode(expected)
  ...
  internal static func configure(_ thread: pid_t,
    ...
    let adjusted = try adjust(site)
    let encoded = try ARM64BreakpointControl.encode(adjusted)
    let bank = try ARM64BreakpointBank(adjusted.kind)
    let note = bank.breakpoint ? NT_ARM_HW_BREAK : NT_ARM_HW_WATCH
    var state = try snapshot(thread, note: note)
    let existing = state.slot(encoded.address, control: UInt32(encoded.control))
    if enabled, case .none = existing,
        state.occupied(encoded.address) {
      throw .breakpoint
    }
    ...
    try commit(thread, note: note, state: &state)
  }
...
extension LinuxDebugControl {
  internal static func capacity(_ thread: pid_t, note: Int)
      throws(Debuggee.Error) -> Int {
    try snapshot(thread, note: note).count
  }

  private static func snapshot(_ thread: pid_t, note: Int)
      throws(Debuggee.Error) -> LinuxARM64DebugRegisters {
    var state = LinuxARM64DebugRegisters()
    ...
    guard state.info >> 8 & 0xff >= kMinimumDebugArchitecture else {
      throw .unsupported
    }
    let stride = MemoryLayout<LinuxARM64DebugRegister>.stride
    let available = Int(vector.iov_len - 8) / stride
    let count = min(Int(state.info & 0xff), available, 16)
    guard count > 0 else {
      throw .unsupported
    }
    state.info = state.info & ~0xff | UInt32(count)
    return state
  }

  private static func commit(_ thread: pid_t, note: Int,
                             state: inout LinuxARM64DebugRegisters)
      throws(Debuggee.Error) {
