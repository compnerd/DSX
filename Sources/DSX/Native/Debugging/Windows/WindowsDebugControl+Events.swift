...
extension WindowsDebugControl {
  ...
  internal mutating func event(blocking: Bool = false, output: Bool = true,
    ...
    while true {
      ...
      if event.dwDebugEventCode == EXCEPTION_DEBUG_EVENT,
          let index = threads.index(forKey: event.dwThreadId) {
        let thread = threads[index].value
        if thread.suspended,
            threads.values.contains(where: { $0.execution == .stepping }) {
          // A queued exception must not interrupt another thread's step.
          try `continue`(disposition: .deferred)
          continue
        }
        let code = event.u.Exception.ExceptionRecord.ExceptionCode
        if WindowsDebugControl.exception(code) == .trace,
            let reply = thread.reply {
          // The hardware stop was published before its event arrived.
          switch reply {
          case .deferred:
            try `continue`(disposition: .deferred)
          case .handled, .unhandled:
            try prepare(thread)
            try `continue`(disposition: reply)
            threads.values[index].reply = nil
          }
          continue
        }
      }
  ...
  private mutating func wait(blocking: Bool, polling: Bool)
    ...
    case (true, true):
      DWORD(Tuning.Debuggee.polling.rounded(to: .milliseconds(1), rule: .up))
    }
  ...
  internal mutating func translate(_ event: inout DEBUG_EVENT,
    ...
    case EXCEPTION_DEBUG_EVENT:
      ...
      let raw = hardware ? UInt64(record.ExceptionInformation.1) : location
      let message =
  ...
  private mutating func capture() throws(Debuggee.Error) -> Bool {
    ...
    let capacity = min(Int(available), Tuning.Output.capacity)
