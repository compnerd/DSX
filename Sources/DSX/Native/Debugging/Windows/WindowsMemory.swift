    ...
    try bytes.withUnsafeBytes { bytes throws(Debuggee.Error) in
      while count < bytes.count {
        ...
        do throws(Debuggee.Error) {
          try transfer(handle, address: location, bytes: remaining,
...
private func store(_ handle: borrowing WindowsHandle, address: Debuggee.Address,
                   bytes: UnsafeRawBufferPointer, flush: Bool,
                   count written: inout Int) throws(Debuggee.Error) {
  var count: SIZE_T = 0
...
extension WindowsMemory {
  internal static func region(_ process: ProcessIdentifier,
    ...
    let execute = accessible && executable(protection)
