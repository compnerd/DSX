...
extension WindowsMemory {
  ...
  internal static func allocate(_ process: ProcessIdentifier, size: UInt64,
    ...
    let handle = try WindowsHandle(process: process, access: access)
    let protection = protect(permissions)
...
private func protect(_ permissions: MemoryPermissions) -> DWORD {
  switch (permissions.contains(.read), permissions.contains(.write),
          permissions.contains(.execute)) {
  case (true, true, true): PAGE_EXECUTE_READWRITE
  case (true, false, true): PAGE_EXECUTE_READ
  case (false, false, true): PAGE_EXECUTE
  case (true, true, false): PAGE_READWRITE
  case (true, false, false): PAGE_READONLY
  case (false, true, _): 0
  case (false, false, false): PAGE_NOACCESS
  }
}
#endif
