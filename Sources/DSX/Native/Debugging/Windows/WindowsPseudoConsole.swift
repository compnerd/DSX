...
internal enum WindowsPseudoConsole {
  ...
  internal static func launch(_ command: inout WindowsCommand,
    ...
    guard terminal.columns <= Int16.max, terminal.rows <= Int16.max else {
      throw Debuggee.Error(process: ERROR_INVALID_PARAMETER)
    }
    var input: HANDLE?
    var writer: HANDLE?
    var reader: HANDLE?
    ...
    guard status >= 0 else {
      ...
      throw Debuggee.Error(hresult: status)
    }
    ...
    let information =
        try command.create(&startup, console: console, flags: flags)
    cleanup = false
