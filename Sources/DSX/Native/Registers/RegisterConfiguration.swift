...
internal struct RegisterConfiguration: Sendable {
  ...
  internal func index(_ index: Int, count: Int) -> Int {
    ...
    // The exception registers follow the optional AVX bank.
    if vector == false, index >= count - RegisterConfiguration.vectors - 3 {
      return index + RegisterConfiguration.vectors
    }
#endif
    return index
  }
...
#if os(anyAppleOS) && arch(x86_64)
    if storage.set.rawValue == 3 {
      let omitted = vector ? 0 : RegisterConfiguration.vectors
      return storage.layout(bits: Int(storage.bits),
                            offset: Int(storage.offset) - 16 - omitted * 16,
                            bias: 2 + omitted)
    }
#endif
#if arch(i386) || arch(x86_64)
