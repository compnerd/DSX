...
internal struct GDBServerCommand {
  ...
  internal func check(_ connection: consuming Connection)
    ...
    switch (pipe, named) {
    case (_?, nil), (nil, _?):
      guard case let .network(_, _, reverse) = connection else {
        throw .notification
      }
      if reverse {
        throw .notification
  ...
  internal consuming func run() throws(DSX.Error) {
    ...
    try DSX.run(configuration, logging: channels, to: log, isolate: setsid,
                daemonize: daemonize, events: .signals)
  }
}
...
internal enum CommandConfigurationError: Error, CustomStringConvertible {
  ...
  internal var description: String {
    ...
    case .notifications: "--pipe and --named-pipe are mutually exclusive"
    case .debuggee: "--attach and a program launch are mutually exclusive"
    }
  }
}
