...
internal struct PacketGenerator {
  private let names: Array<PacketLeaf>
  private let ordinals: Dictionary<PacketLeaf, UInt8>
  private let root: PacketNode

  internal init(_ source: String) throws(DSXCodeGenError) {
    let definitions: Array<PacketDefinition>
    do {
      definitions =
          try YAMLDecoder().decode(Array<PacketDefinition>.self, from: source)
    ...
    var names = Array<PacketLeaf>()
    var ordinals = Dictionary<PacketLeaf, UInt8>()
  ...
  internal func generate() throws(DSXCodeGenError) -> String {
    var source = header()
    source.append("")
    source += leaves()
    source.append("")
  ...
  private func leaves() -> Array<String> {
    ...
    for (index, leaf) in names.enumerated() {
      source.append("    case \(index + 1): self = \(leaf.expression)")
    }
  ...
  private func classifier() throws(DSXCodeGenError) -> Array<String> {
    let root = PacketCompactNode(root)
    ...
    source.append("internal struct GDBPacketMatch: Sendable {")
    source.append("  internal let leaf: GDBPacketLeaf")
    source.append("  internal let payload: Int")
    source.append("  private let flags: UInt8")
    source.append("")
  ...
  private func interpreter() -> Array<String> {
    [
      ...
      "  }",
      "",
      "  internal var route: GDBPacketRoute {",
      ...
      "  }",
      "",
      "  internal var availability: UInt8 {",
      "    flags >> 3 & 0x07",
      "  }",
      "",
      "  internal var request: GDBPacketEncoding {",
      "    flags & 0x40 > 0 ? .binary : .text",
      "  }",
      "",
      "  internal var response: GDBPacketEncoding {",
      "    flags & 0x80 > 0 ? .binary : .text",
      "  }",
      "",
      "  internal init(_ packet: borrowing Span<UInt8>) {",
      "    var node = 0",
      "    var index = 0",
      "    while true {",
      "      let flags = GDBPacketMatch.kTrie[node]",
      "      if index == packet.count {",
      "        guard flags & 0x03 > 0 else {",
      "          self = GDBPacketMatch()",
      "          return",
      "        }",
      "        self = GDBPacketMatch(node, flags: flags, payload: index)",
      "        return",
      "      }",
      "      let count = Int(GDBPacketMatch.kTrie[node + 2])",
      "      var edge = node + 3",
      "      var found = false",
      "      for _ in 0 ..< count {",
      "        let length = Int(GDBPacketMatch.kTrie[edge])",
      "        let start = edge + 3",
      "        if GDBPacketMatch.matches(packet, from: index, bytes: start,",
      "                                  count: length) {",
      "          node = GDBPacketMatch.offset(edge + 1)",
      "          index += length",
      "          found = true",
      "          break",
      "        }",
      ...
      "  }",
      "",
      "  private init(_ node: Int, flags: UInt8, payload: Int) {",
      "    let raw = GDBPacketMatch.kTrie[node + 1]",
      "    guard let leaf = GDBPacketLeaf(raw) else {",
      ...
      "  internal func allows(_ match: GDBPacketMatch) -> Bool {",
      "    switch match.availability {",
      "    case 0: true",
      "    case 1: compatibility == .gdb",
      "    case 2: compatibility == .lldb",
      "    case 3: negotiation.supported.contains(.stopthreads)",
      "    case 4: negotiation.supported.contains(.threadsuffix)",
      "    default: false",
      "    }",
      "  }",
      "",
...
private final class PacketCompactNode {
  ...
  fileprivate init(_ source: PacketNode) {
    ...
    for byte in source.children.keys.sorted() {
      ...
      edges.append(PacketCompactEdge(bytes: bytes,
                                     target: PacketCompactNode(child)))
    }
  }
