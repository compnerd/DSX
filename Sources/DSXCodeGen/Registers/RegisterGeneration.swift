...
extension GeneratedProfile {
  internal init(_ profile: RegisterProfileDefinition) {
    ...
    let custom = Set(named.map(\.0))
    var predefined = Array<String>()
    for register in profile.registers {
      if let name = register.type, custom.contains(name) == false,
          predefined.contains(name) == false {
        predefined.append(name)
      }
    }
    let builtin = predefined.enumerated().map {
      ($0.element, UInt16(named.count + $0.offset))
    }
    let identifiers = Dictionary(uniqueKeysWithValues: named + builtin)
    ...
    for register in profile.registers {
      ...
      let range = (containers ..< invalidates, invalidates ..< relations.count)
      ranges.append(range)
    }
    ...
    let types = profile.features.flatMap { feature in
      feature.types.map { (feature.id, $0, feature.platforms) }
    }
    var fields = Array<RegisterFieldDefinition>()
    var bounds = Array<Range<Int>>()
    for (_, type, _) in types {
    ...
    // Protocol names need not be Swift identifiers; numeric IDs still work.
    let symbols = profile.registers.filter { identifier($0.name) }
    if symbols.isEmpty == false {
      lines.append("internal enum \(profile.profile)Register: UInt32 {")
      for register in symbols {
        if register.platforms.isEmpty == false {
          lines.append("#if \(expression(register.platforms))")
        }
        lines.append("  case `\(register.name)` = \(register.id)")
        if register.platforms.isEmpty == false {
          lines.append("#endif")
        }
      }
    ...
    lines.append("  internal var configuration = RegisterConfiguration()")
    ...
    for set in profile.sets {
      let entry = [
        "    RegisterSetRecord(identifier: " +
            "RegisterSetIdentifier(rawValue: \(set.id)),",
    ...
    for (index, feature) in profile.features.enumerated() {
      ...
      let entry = [
        "    RegisterFeatureRecord(identifier: RegisterFeatureIdentifier(" +
            "rawValue: \(feature.id)),",
    ...
    lines += array("  internal static let kFeatures: " +
                   "InlineArray<_, RegisterFeatureRecord> = [",
                   entries: records)
    lines.append("")
    records.removeAll(keepingCapacity: true)
    for (index, entry) in types.enumerated() {
      ...
      let value = "\(range.lowerBound) ..< \(range.upperBound)"
      let record = [
        "    RegisterTypeRecord(identifier: " +
            "RegisterTypeIdentifier(rawValue: \(index)),",
        "                       feature: " +
            "RegisterFeatureIdentifier(rawValue: \(entry.0)),",
        "                       name: \(literal(entry.1.name)),",
        "                       kind: .\(entry.1.kind.rawValue), " +
            "bits: \(optional(entry.1.bits)),",
        "                       element: \(optional(entry.1.element)), " +
            "count: \(optional(entry.1.count)),",
        "                       fields: \(value)),",
      ]
      records.append((entry.2, record))
    }
    lines += array("  internal static let kTypes: " +
                   "InlineArray<_, RegisterTypeRecord> = [", entries: records)
    lines.append("")
    ...
    lines += positions(profile.registers)
    lines += spelling(profile.registers)
    lines += aliases(profile.registers)
    lines += references(builtin)
    lines += groups(profile.sets)
    lines.append("}")
    lines.append("#endif")
    lines.append("")
    self.init(name: type, architecture: profile.architecture,
              source: lines.joined(separator: "\n"))
  }
}

private func positions(_ registers: Array<RegisterDefinition>)
    -> Array<String> {
  let groups = conditions(registers.map(\.platforms))
  var lines = [""]
  for (index, group) in groups.enumerated() {
    let directive = index == 0 ? "#if" : "#elseif"
    lines.append("\(directive) \(expression(group))")
    let selected = registers.filter {
      $0.platforms.isEmpty || Set($0.platforms) == Set(group)
    }
    lines += positions(selected,
                       pc: selected.firstIndex { $0.role == .program })
  }
  if groups.isEmpty == false {
    lines.append("#else")
  }
  let selected = registers.filter { $0.platforms.isEmpty }
  lines += positions(selected, pc: selected.firstIndex { $0.role == .program })
  if groups.isEmpty == false {
    lines.append("#endif")
  }
  return lines
...
private func positions(_ registers: Array<RegisterDefinition>, pc: Int?)
    -> Array<String> {
  let positions = registers.indices.filter { registers[$0].expedited }
  let indices = positions.map(String.init).joined(separator: ", ")
  let record = pc.map { "record(\($0))" } ?? "nil"
  let frame = registers.firstIndex { $0.role == .frame }
    .map { "record(\($0))" } ?? "nil"
  let declaration =
      "  internal static let expedited: InlineArray<\(positions.count), UInt16>"
  let inline = "\(declaration) = [\(indices)]"
  var lines = if inline.count <= 80 {
    [inline]
  } else {
    ["\(declaration) = ["] + positions.map { "    \($0)," } + ["  ]"]
  }
  lines += [
    "",
    "  internal var pc: RegisterRecord? { \(record) }",
    "  internal var fp: RegisterRecord? { \(frame) }",
  ]
  return lines
}

private func spelling(_ registers: Array<RegisterDefinition>) -> Array<String> {
  var lines = [
    "",
    "  internal func name(_ register: RegisterRecord,",
    "                     compatibility: CompatibilityMode) -> RegisterText {",
  ]
  let overrides = registers.filter { $0.gdb != nil && $0.gdb != $0.name }
  if overrides.isEmpty == false {
    lines.append("    if compatibility == .gdb {")
    lines.append("      switch register.identifier.rawValue {")
    for register in overrides {
      let name = register.gdb!
      lines.append("      case \(register.id):")
      if name == register.alternate {
        lines.append("        return alias(register) ?? name(register)")
      } else {
        let range = UInt64(name.utf8.count) << 32
        lines.append("        return RegisterText(storage: \(literal(name)),")
        lines.append("                            range: UInt64(\(range)))")
      }
    }
    ...
    lines.append("      }")
    lines.append("    }")
    lines.append("    return name(register)")
  } else {
...
private func aliases(_ registers: Array<RegisterDefinition>) -> Array<String> {
  var lines = [
    "",
  ...
  for register in registers {
    guard let alternate = register.alternate else {
      continue
    }
    if register.platforms.isEmpty == false {
      lines.append("#if \(expression(register.platforms))")
    }
    let range = UInt64(alternate.utf8.count) << 32
    lines += [
      "    case \(register.id):",
      "      RegisterText(storage: \(literal(alternate)),",
      "                   range: UInt64(\(range)))",
    ]
    if register.platforms.isEmpty == false {
      lines.append("#endif")
...
private func references(_ builtin: Array<(String, UInt16)>) -> Array<String> {
  var lines = [
    "",
    "  @inline(__always)",
    "  internal func type(_ register: RegisterRecord) -> StaticString? {",
    "    guard let identifier = register.type else {",
    "      return nil",
    "    }",
  ]
  if builtin.isEmpty {
    lines.append("    return type(identifier)?.name")
  } else {
    lines.append("    return switch identifier.rawValue {")
    for (name, identifier) in builtin {
      lines.append("    case \(identifier): \(literal(name))")
    }
    lines.append("    default: type(identifier)?.name")
    lines.append("    }")
  }
  lines.append("  }")
  return lines
...
private func groups(_ sets: Array<RegisterSetDefinition>) -> Array<String> {
  var lines = [
    "",
    "  internal func group(_ identifier: RegisterSetIdentifier,",
    "                      compatibility: CompatibilityMode)" +
        " -> StaticString? {",
    "    if compatibility == .lldb {",
  ]
  lines += titles(sets)
  lines += ["    }", "    return switch identifier.rawValue {"]
  for set in sets {
    if set.platforms.isEmpty == false {
      lines.append("#if \(expression(set.platforms))")
    }
    lines.append("    case \(set.id): \(literal(xml(set.group)))")
    if set.platforms.isEmpty == false {
      lines.append("#endif")
    }
  }
...
private func titles(_ sets: Array<RegisterSetDefinition>) -> Array<String> {
  var lines = Array<String>()
  let escaped = sets.filter { xml($0.title) != $0.title }
  for set in escaped {
    if set.platforms.isEmpty == false {
      lines.append("#if \(expression(set.platforms))")
    }
    lines.append("      if identifier.rawValue == \(set.id) {")
    lines.append("        return \(literal(xml(set.title)))")
    lines.append("      }")
    if set.platforms.isEmpty == false {
      lines.append("#endif")
    }
  }
  lines.append("      return set(Int(identifier.rawValue))?.name")
  return lines
}
...
private func record(_ register: RegisterDefinition, set: UInt16,
  ...
  let location = UInt64(register.bits) | UInt64(register.offset) << 16
               | UInt64(type) << 32
               | (register.expedited ? 0x0001_0000_0000_0000 : 0)
...
private func relation(_ ranges: Array<(Range<Int>, Range<Int>)>,
  ...
  if conditions.isEmpty {
    ...
    for (index, condition) in conditions.enumerated() {
      let directive = index == 0 ? "#if \(expression(condition))"
...
private func array(_ declaration: String,
  ...
  var lines = Array<String>()
  for (index, group) in groups.enumerated() {
    let directive = index == 0 ? "#if \(expression(group))"
...
private func storage(_ name: String, values: Array<String?>) -> Array<String> {
  ...
  let compact = storage.utf8.count <= UInt16.max
  let word = compact ? "UInt32" : "UInt64"
  var lines = [
    "  private static let k\(name): StaticString = \(literal(storage))",
    "  private static let k\(name)Ranges: InlineArray<_, \(word)> = [",
  ]
  var offset = 0
  for value in values {
    guard let value else {
      lines.append("    \(word).max,")
      continue
    }
    let count = value.utf8.count
    let range = UInt64(offset) | UInt64(count) << (compact ? 16 : 32)
...
private func accessors(_ type: String, relations: Bool) -> Array<String> {
  ...
  return [
    ...
    "    configuration.count(\(type).kRecords.count / 3)",
    "  }",
    ...
    "    }",
    "    let index =",
    "        configuration.index(index, count: \(type).kRecords.count / 3)",
    "    return record(index)",
    "  }",
    "",
    "  /// Reads a physical slot, applying runtime availability and layout.",
    "  internal func record(_ index: Int) -> RegisterRecord? {",
    "    let base = index * 3",
    "    let storage = RegisterStorage(\(type).kRecords[base],",
    "                                  \(type).kRecords[base + 1],",
    "                                  \(type).kRecords[base + 2],",
    "                                  relations: \(relations))",
    "    guard let storage = configuration.layout(storage) else {",
    "      return nil",
    "    }",
...
private func role(_ value: RegisterRole?) -> UInt8 {
  ...
  case .stack: 6
  case .thread: 7
  case let .argument(value): 0x80 | value
  }
}
