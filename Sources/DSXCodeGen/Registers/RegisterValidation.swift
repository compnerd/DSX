...
internal func validate(_ profile: RegisterProfileDefinition)
  ...
  for set in profile.sets {
    try text(set.title, attribute: false)
    try text(set.group, attribute: false)
  }
  for feature in profile.features {
    try text(feature.name)
  }
  ...
  let referenced = Set(profile.registers.compactMap(\.type))
  let count = types.count + referenced.subtracting(names).count
  guard count <= Int(UInt16.max) else {
    throw .schema("profile has too many register types")
  }
  ...
  for register in profile.registers {
    try text(register.name)
    if let alternate = register.alternate {
      try text(alternate)
    }
    if let name = register.gdb {
      try text(name)
    }
    ...
    if register.expedited, register.bits > UInt64.bitWidth {
      throw .schema("expedited register '\(register.name)' is too wide")
    }
...
internal func identifier(_ value: String) -> Bool {
...
/// Protocol identifiers are emitted verbatim; display text is escaped at build.
private func text(_ value: String, attribute: Bool = true)
    throws(DSXCodeGenError) {
  guard !value.isEmpty else {
    throw .schema("register metadata contains empty text")
  }
  for scalar in value.unicodeScalars {
    let code = scalar.value
    let valid = code == 9 || code == 10 || code == 13 ||
        code >= 0x20 && code <= 0xd7ff ||
        code >= 0xe000 && code <= 0xfffd || code >= 0x10000
    guard valid else {
      throw .schema("register metadata contains an invalid XML character")
    }
    if attribute,
        code <= 0x20 || scalar == "&" || scalar == "<" || scalar == "\"" {
      throw .schema("register identifier is not safe for protocol output")
    }
  }
}
...
private func numbers(_ registers: Array<RegisterDefinition>,
  ...
  let numbers = registers.compactMap { register in
    register[keyPath: key].map { ($0, register.platforms) }
  }
  try unique(numbers, label: "\(label) register number")
}
