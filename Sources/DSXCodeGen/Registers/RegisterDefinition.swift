...
internal struct RegisterSetDefinition: Decodable {
  ...
  internal init(from decoder: Decoder) throws {
    let values = try decoder.container(keys: [
        "id", "name", "title", "group", "platforms"
    ])
...
extension RegisterProfileDefinition {
  internal init(_ source: String) throws(DSXCodeGenError) {
    do {
      self =
          try YAMLDecoder().decode(RegisterProfileDefinition.self, from: source)
