...
def platform(paths: list[pathlib.Path], label: str, build: str,
             shards: list[str], result: str, scope: str = "") -> tuple[str, bool]:
    ...
    for name, entries in expected.items():
        ...
        try:
            ...
            complete = (data.get("completed", False) and selected > 0 and
                        finished == selected and attempted == selected and
            ...
            continue
        states[name] = ("Incomplete" if not complete else
    ...
    if scope:
        lines.append(f"Test path override: {scope}.")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--scope", default="", help="test path override")
