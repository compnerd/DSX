...
def configuration(arguments: argparse.Namespace, root: pathlib.Path):
    ...
    if arguments.trace:
        # Keep the client's step decisions alongside the server's stop packets.
        # dotest retains these logs for failures, discarding successful cases.
        options.extend(("--channel", "lldb step", "--channel",
                        "gdb-remote packets"))
    if arguments.android:
...
def android_server(adb: str, arguments: argparse.Namespace,
    ...
    command = (f"cd {parent} && echo DSX_PID:$$ "
               f"&& export LD_LIBRARY_PATH={parent} "
