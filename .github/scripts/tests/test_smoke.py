...
class Smoke(unittest.TestCase):
    ...
    def test_exchange(self):
        bodies = (b"triple:693338362d77696e646f7773;ptrsize:4;endian:little;",
                  ...
                  b"pid:41;port:1235;", b"multiprocess+", b"T05thread:p2a.2b;",
                  b"0011xx", b"QCp2a.2b", b"OK", b"OK",
                  b"F0")
        ...
        packet = b"vKill;2a"
        framed = b"+$" + packet + b"#%02x" % (sum(packet) & 0xff)
        self.client.sendall.assert_any_call(framed)
    ...
    def test_launch_failure(self):
        for reply, message in ((b"", "closed before its reply"),
                               (b"E01", "Invalid launch stop")):
            with self.subTest(reply=reply):
                bodies = [b"pid:41;port:1235;", b"multiprocess+", reply, b"OK"]
                self.client.recv.side_effect = [
                    b"+$" + body + b"#%02x" % (sum(body) & 0xff)
                    if body else b"" for body in bodies
                ]
                with patch.object(smoke.socket, "create_connection",
                                  return_value=self.client):
                    with self.assertRaisesRegex(RuntimeError, message):
                        smoke.launch(self.client, b"647378", 1)
                packet = b"qKillSpawnedProcess:41"
                framed = b"+$" + packet + b"#%02x" % (sum(packet) & 0xff)
                self.client.sendall.assert_any_call(framed)
