...
class Platforms(unittest.TestCase):
    ...
    def summary(self, shards=("commands", "tools"), result="success", scope=""):
    ...
    def test_scope(self):
        ...
        self.assertIn("Test path override: commands/register.", text)
...
class Harness(unittest.TestCase):
    ...
    def test_android(self):
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            with patch.object(harness, "available_port", return_value=1234), \
                ...
                self.assertEqual(command[:2], ["adb", "shell"])
                self.assertIn("export LD_LIBRARY_PATH=/data/local/tmp && exec",
                              command[2])
    ...
    def test_android_environment(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            result = subprocess.run(["/bin/sh", "-c", command], check=True,
                                    capture_output=True, text=True,
                                    env={"PATH": "/usr/bin:/bin"})
            self.assertIn(f"parent:{root}\nchild:{root}\n", result.stdout)
...
class Upstream(unittest.TestCase):
    ...
    def test_android_configuration(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            compiler = root / "ndk/bin/x86_64-linux-android28-clang"
            args = argparse.Namespace(
                source=self.source, tools=root, arch="x86_64",
                triple="x86_64-unknown-linux-android28", compiler=compiler,
                android=True, android_api=28, working_directory=None,
            ...
            self.assertEqual(parsed.channels, ["lldb step", "gdb-remote packets"])
            self.assertIn("API_LEVEL=28", config.environment["MAKEFLAGS"])
