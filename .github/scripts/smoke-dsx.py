...
def launch(platform: socket.socket, executable: bytes, timeout: float) -> None:
    body = exchange(platform, b"qLaunchGDBServer;host:127.0.0.1;", timeout)
    fields = dict(field.split(b":", 1) for field in body.split(b";")
                  if b":" in field)
    if not {b"pid", b"port"}.issubset(fields):
        raise RuntimeError(f"Invalid qLaunchGDBServer: {body!r}")
    try:
        with socket.create_connection(("127.0.0.1", int(fields[b"port"])),
                                      timeout=timeout) as client:
            exchange(client, b"qSupported:multiprocess+", timeout)
            body = exchange(client, b"vRun;" + executable + b";76657273696f6e",
                            timeout)
            if not re.fullmatch(rb"T[0-9a-fA-F]{2}.*", body):
                raise RuntimeError(f"Invalid launch stop: {body!r}")
            body = exchange(client, b"g", timeout)
            if not re.fullmatch(rb"(?:[0-9a-fA-Fx]{2})+", body):
                raise RuntimeError(f"Invalid register snapshot: {body!r}")
            body = exchange(client, b"qC", timeout)
            thread = re.fullmatch(rb"QCp([0-9a-fA-F]+)\.[0-9a-fA-F]+", body)
            if thread is None:
                raise RuntimeError(f"Invalid launch thread: {body!r}")
            body = exchange(client, b"vKill;" + thread[1], timeout)
            if body != b"OK":
                raise RuntimeError(f"Invalid vKill: {body!r}")
    finally:
        exchange(platform, b"qKillSpawnedProcess:" + fields[b"pid"], timeout)
...
if __name__ == "__main__":
    ...
    print("DSX startup, files, permissions, launch and register capture passed")
