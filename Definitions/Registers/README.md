...
layout: fixed
sets:
  - { id: 0, name: gpr, title: General Purpose Registers, group: general }
features:
  - id: 0
...
Nullable values use `null`; collections may be empty.
Numbering domains are independent. The canonical identifier is `id`, not any
protocol or debug-information number.

A register set's `name` is its schema identity, `title` is its display name,
and optional `group` is its GDB group spelling (defaulting to `name`). Set IDs
may be reused on disjoint `platforms`; generated lookups preserve those guards.
Titles and groups are XML-escaped at generation time. Protocol identifiers must
be nonempty and contain no ASCII whitespace or XML attribute delimiters.
Invalid XML characters are rejected in all emitted text.
