...
`FrameCursor` walks the supported native frame chain and enforces progress
...
`NativeMappedFile` owns the storage. Windows uses a bounded file mapping; Unix
reads into an owned anonymous mapping so later file truncation cannot invalidate
the parser's memory. The Unix path eagerly materializes the file's bytes.
`ELFModule`, `PEModule`, and `MachOModule` borrow those bytes and validate
format-specific offsets, records, architectures, and identifiers. Format parsing
is separate from host file access: reading an ELF file is not inherently a Linux
operation.
